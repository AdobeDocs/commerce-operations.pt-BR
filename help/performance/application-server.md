...
1. Confirme se a variável de ambiente `CRYPT_KEY` está definida para sua instância. Você pode verificar o status dessa variável no Portal de projetos na nuvem (interface de integração).
1. Clonar o projeto do Commerce Cloud.
...
Os desenvolvedores e comerciantes de extensões devem primeiro verificar se sua extensão e código de personalização seguem as diretrizes técnicas descritas em [Diretrizes técnicas](https://developer.adobe.com/commerce/php/coding-standards/technical-guidelines/).
