...
A inclusão de módulos do JavaScript para obter melhor desempenho tem a ver com a redução de dois fatores:

1. O número de solicitações do servidor.
1. O tamanho dessas solicitações de servidor.

Em um aplicativo modular, o número de solicitações de servidor pode chegar a centenas. Por exemplo, a captura de tela a seguir mostra apenas o início da lista de módulos do JavaScript carregados na página inicial de uma instalação limpa.

![Nenhum pacote](../assets/performance/images/noBundling.png)

## Mesclagem e agrupamento

O Commerce oferece suporte ao agrupamento para reduzir o número de solicitações do servidor. O agrupamento está desativado por padrão. Você pode ativá-lo em **[!UICONTROL Stores]** > **Configurações** > **[!UICONTROL Configuration]** > **[!UICONTROL Advanced]** > **[!UICONTROL Developer]** > **[!UICONTROL JavaScript Settings]** ou na linha de comando.

Consulte [Dicas de agrupamento](configuration.md#bundling-tips) em *Práticas recomendadas de configuração* para ferramentas de terceiros, HTTP/2, e orientações sobre mesclagem de JS e CSS obsoletas.
...
Este é um mecanismo nativo do Commerce que combina todos os ativos presentes no sistema e os distribui entre pacotes do mesmo tamanho (bundle_0.js, bundle_1.js ... bundle_x.js):

![Agrupamento Commerce](../assets/performance/images/magentoBundling.png)

Melhor, mas o navegador ainda carrega TODOS os pacotes JavaScript, não apenas os necessários.

O agrupamento Commerce reduz o número de conexões por página, mas, para cada solicitação de página, carrega todos os pacotes, mesmo quando a página solicitada depende apenas de arquivos dentro de um ou dois dos pacotes. O desempenho melhora depois que o navegador armazena os pacotes em cache. Mas, como o navegador carrega esses pacotes de forma síncrona, a primeira visita do usuário a uma loja da Commerce pode demorar um pouco para renderizar e prejudicar a experiência do usuário.

### Mesclagem básica (não recomendado)

>[!NOTE]
>
>Não recomendamos usar **[!UICONTROL Merge JavaScript Files]**. Esta configuração foi projetada apenas para JavaScript carregada de forma síncrona na seção HEAD da página e pode fazer com que o agrupamento e a lógica do [!DNL RequireJS] funcionem incorretamente. Ele é mantido somente para compatibilidade com versões anteriores e não fornece benefício de desempenho quando HTTP/2 está habilitado.
...
Esse comando mescla todos os arquivos síncronos do JavaScript em um único arquivo. Habilitar mesclagem sem também habilitar agrupamento não é útil porque o Commerce usa [!DNL RequireJS]. Se você não habilitar o agrupamento, o Commerce mesclará somente [!DNL RequireJS] e sua configuração. Quando você habilita a combinação e a mesclagem, o Commerce cria um único arquivo JavaScript:
...
Na conectividade Slow 3G, leva cerca de 44 segundos para carregar todos os pacotes da página inicial de uma instalação limpa do Commerce.
...
Com uma abordagem mais avançada ao agrupamento JavaScript, podemos melhorar esses tempos de carregamento.

## Agrupamento avançado

Lembre-se de que o objetivo do agrupamento JavaScript é reduzir o número e o tamanho dos ativos solicitados para cada página carregada no navegador. Para fazer isso, queremos criar nossos pacotes para que cada página em nossa loja só precise baixar um pacote comum e um pacote específico da página para cada página acessada.

Uma maneira de fazer isso é definir seus pacotes por tipos de página. É possível categorizar as páginas do Commerce em vários tipos de página, incluindo Categoria, Produto, CMS, Cliente, Carrinho e Check-out. Cada página categorizada em um desses tipos de página tem um conjunto diferente de [!DNL RequireJS] dependências de módulo. Ao agrupar os módulos do [!DNL RequireJS] por tipo de página, você terá apenas alguns pacotes que cobrem as dependências de qualquer página da sua loja.
...
Uma instalação limpa do Commerce permite atingir um desempenho bom o suficiente dividindo pacotes por tipos de página, mas algumas personalizações podem exigir uma análise mais profunda e outras distribuições de ativos.
...
![[!DNL RequireJS] configuração](../assets/performance/images/RequireJSConfig.png)
...
1. [!DNL RequireJS] comando no console do navegador.

#### Para usar [!DNL PhantomJS]:

No diretório raiz do Commerce, crie um novo arquivo chamado `deps.js` e copie no código abaixo. Este código usa [!DNL [!DNL PhantomJS]] para abrir uma página e aguardar o navegador carregar todos os ativos da página. Em seguida, ele gera todas as dependências [!DNL RequireJS] de uma determinada página.
...
Abra um terminal no diretório raiz do Commerce e execute o script em cada página da loja que represente um tipo de página específico:
...
- `include`— uma matriz de ativos (sequências de caracteres) incluídos como dependências para a página. [!DNL RequireJS] rastreia todas as dependências e as inclui no conjunto, a menos que excluídas.
...
As etapas abaixo descrevem o processo básico para gerar pacotes Commerce mais eficientes. Você pode automatizar esse processo da maneira que quiser, mas ainda precisará usar `nodejs` e `r.js` para realmente gerar seus pacotes. E se seus temas tiverem personalizações relacionadas ao JavaScript e não puderem reutilizar o mesmo arquivo `build.js`, talvez seja necessário criar várias configurações de `build.js` por tema.
...
#### &#x200B;4. Configurar [!DNL RequireJS] para usar conjuntos

Para fazer [!DNL RequireJS] usar seus pacotes, adicione um retorno de chamada `onModuleBundleComplete` após o nó `modules` no arquivo `build.js`:
...
Abra `requirejs-config.js` no diretório `pub/static/frontend/Magento/luma/en_US` para verificar se [!DNL RequireJS] anexou o arquivo com chamadas de configuração de agrupamento:
...
O tempo de carregamento de uma página inicial vazia agora é duas vezes mais rápido do que o uso do pacote nativo do Commerce. Mas podemos fazer ainda melhor.

#### &#x200B;7. Otimizar os pacotes

Mesmo se forem compactados, os arquivos JavaScript ainda serão grandes. Reduza-os com [!DNL RequireJS], que usa um modificador para minificar o JavaScript para um bom resultado.
...
Os tempos de carregamento agora são três vezes mais rápidos do que com o pacote nativo do Commerce.
