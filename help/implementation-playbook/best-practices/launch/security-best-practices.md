...
Estabelecer e manter um ambiente seguro para projetos da Adobe Commerce implantados em infraestrutura em nuvem é uma responsabilidade compartilhada entre clientes da Adobe Commerce, parceiros de soluções e a Adobe. O objetivo deste guia é fornecer práticas recomendadas para as responsabilidades do cliente.

Embora não seja possível eliminar todos os riscos de segurança, a aplicação dessas práticas recomendadas fortalece a postura de segurança das instalações do Commerce. Um site e uma infraestrutura seguros reduzem o risco de ataques mal-intencionados, protegem informações confidenciais e minimizam incidentes de segurança dispendiosos.
...
- [Segurança para o administrador do Commerce](https://experienceleague.adobe.com/pt-br/docs/commerce-admin/systems/security/security-admin)

- [Conexões SSH seguras](https://experienceleague.adobe.com/pt-br/docs/commerce-on-cloud/user-guide/project/multi-factor-authentication) (infraestrutura em nuvem)
...
- [Configure uma URL de administrador não padrão](https://experienceleague.adobe.com/pt-br/docs/commerce-admin/stores-sales/site-store/store-urls#use-a-custom-admin-url) em vez de usar o `admin` padrão ou um termo comum, como `backend`. Essa configuração reduz a exposição a scripts que tentam obter acesso não autorizado ao site.

- [Definir configurações de segurança avançadas](https://experienceleague.adobe.com/pt-br/docs/commerce-admin/systems/security/security-admin) — Adicione uma chave secreta a URLs, exija senhas que diferenciam maiúsculas de minúsculas e limite a duração da sessão do Administrador, o tempo de vida da senha e as tentativas de logon. Para maior segurança, configure o tempo de inatividade do teclado antes que a sessão atual expire e exija que o nome de usuário e a senha diferenciem maiúsculas de minúsculas.

- [Habilite o reCAPTCHA](https://experienceleague.adobe.com/pt-br/docs/commerce-admin/systems/security/captcha/security-google-recaptcha) para proteger o Administrador contra ataques automatizados de força bruta.

- Siga o princípio do privilégio mínimo ao atribuir [Permissões de administrador](https://experienceleague.adobe.com/pt-br/docs/commerce-admin/systems/user-accounts/permissions) a funções e funções para contas de usuário administrador.
...
Use o [serviço de Verificação de Segurança da Commerce](https://experienceleague.adobe.com/pt-br/docs/commerce-admin/systems/security/security-scan) para monitorar todos os sites da Adobe Commerce quanto a riscos de segurança e malware conhecidos, e inscreva-se para receber atualizações de patches e notificações de segurança.
...
![Lista de verificação](/help/assets/icons/Smock_CheckmarkCircleOutline_18_N.svg) **Escolha um parceiro com consciência de segurança ou um integrador de soluções (SI)**—Garanta integrações seguras e código personalizado selecionando organizações com um histórico de desenvolvimento seguro.
...
![Lista de verificação](/help/assets/icons/Smock_CheckmarkCircleOutline_18_N.svg) **Código de auditoria**—Revise o servidor e o repositório de código-fonte para ver se há sobras de desenvolvimento. Certifique-se de que não existam arquivos de log acessíveis, diretórios .git, túneis SQL, dumps de banco de dados ou arquivos php info.

## Atualizar para a versão mais recente

A Adobe lança continuamente componentes atualizados da solução para melhorar a segurança e proteger melhor os clientes contra possíveis comprometimentos. Atualizar para a versão mais recente do aplicativo Adobe Commerce, serviços instalados e extensões e aplicar os patches atuais é o principal método de proteção contra ameaças à segurança.
...
- [Aviso de segurança e conformidade](../../../release/security-enforcement-policy.md) (Adobe Commerce nas versões 2.4.4 a 2.4.9)
- [Guia de atualização](../../../upgrade/overview.md)
- [Como aplicar patches](../../../upgrade/patches/overview.md)

>[!TIP]
>
>Obtenha as informações de segurança mais recentes e resolva problemas de segurança conhecidos assinando o [Serviço de Notificação de Segurança da Adobe](https://www.adobe.com/subscription/adobesecuritynotifications.html).
...
- [Gerenciamento de configuração de loja para o Adobe Commerce na infraestrutura em nuvem](https://experienceleague.adobe.com/pt-br/docs/commerce-on-cloud/user-guide/configure-store/store-settings)
...
Esta seção resume as práticas recomendadas para manter a segurança do local e da infraestrutura de uma instalação do Adobe Commerce. Muitas dessas práticas recomendadas se concentram na proteção da infraestrutura do computador em geral, de modo que algumas das recomendações já estão implementadas.

![Lista de verificação](/help/assets/icons/Smock_CheckmarkCircleOutline_18_N.svg) **Bloquear acesso não autorizado**—Para bloquear o acesso não autorizado ao site da Commerce e aos dados do cliente, trabalhe com seu parceiro de hospedagem para configurar um túnel VPN. Para bloquear o acesso não autorizado ao aplicativo do Commerce, configure um túnel SSH.
...
As instalações do Adobe Commerce implantadas na infraestrutura em nuvem podem usar os serviços WAF integrados disponíveis com a [integração do Fastly services](https://experienceleague.adobe.com/pt-br/docs/commerce-on-cloud/user-guide/cdn/fastly)

![Lista de Verificação](/help/assets/icons/Smock_CheckmarkCircleOutline_18_N.svg) **Definir configurações avançadas de segurança de senha**—Configure senhas fortes e altere-as pelo menos a cada 90 dias, conforme recomendado pelo Padrão de Segurança de Dados PCI na seção 8.2.4. Consulte [Definir configurações de segurança do administrador](https://experienceleague.adobe.com/pt-br/docs/commerce-admin/systems/security/security-admin).

![Lista de verificação](/help/assets/icons/Smock_CheckmarkCircleOutline_18_N.svg) **Usar HTTPS** — Se o site do Commerce for recém-implementado, inicie o site inteiro usando HTTPS. O Google usa HTTPS como um fator de classificação, e muitos usuários consideram comprar de um site somente se ele estiver protegido por HTTPS.

## Proteção contra malware

Ataques de malware direcionados a sites de comércio eletrônico são frequentes e os agentes de ameaça desenvolvem continuamente novas maneiras de coletar informações pessoais e de cartões de crédito das transações.

No entanto, a Adobe descobriu que a maioria dos comprometimentos do site não se deve a um invasor inovador. Em vez disso, os agentes de ameaças geralmente tiram proveito das vulnerabilidades existentes e sem patch, senhas ruins e configurações fracas de propriedade e permissão no sistema de arquivos.
...
Após ações como envio de formulário ou modificação de campo, o skimmer serializa os dados e os envia para endpoints de terceiros. Normalmente, esses endpoints são outros sites comprometidos que atuam como uma retransmissão para enviar os dados ao destino final.


>[!TIP]
>
>Se um ataque de malware afetar um site do Commerce, siga as práticas recomendadas da Adobe Commerce para [responder a um incidente de segurança](../maintenance/respond-to-security-incident.md).
...
- **Botnets**—O servidor Commerce do cliente se torna parte de uma botnet que envia email de spam. Embora os dados do usuário normalmente não sejam comprometidos, os filtros de spam não incluem na lista de bloqueios o nome de domínio do cliente, impedindo a entrega de qualquer email do domínio. Como alternativa, o site do cliente se torna parte de uma botnet que causa um ataque de negação de serviço distribuído (DDoS) em outros sites. A botnet bloqueia o tráfego IP de entrada para o servidor do Commerce, impedindo que os clientes possam comprar.
...
- Identifique e proteja todos os pontos em que a instalação do Commerce pode ser acessada de fora.
  ...
  Você pode proteger o acesso ao Administrador, que requer mais proteção, seguindo as [recomendações de prioridade](#priority-recommendations) da Adobe ao configurar seu projeto do Commerce.
