...
Use uma CDN para otimizar o desempenho do download de ativos. O Adobe Commerce utiliza o Fastly. Se tiver uma implementação local do Adobe Commerce, você também deve considerar adicionar uma camada de CDN.
...
Um núcleo de CPU pode atender cerca de 2 a 4 solicitações de Adobe Commerce sem cache eficaz. Para determinar quantos nós/núcleos da Web são necessários para processar todas as solicitações recebidas sem colocá-las em fila, use a equação:
...
- **ByteCode** — para obter a velocidade máxima do Adobe Commerce no PHP 7, você deve ativar o  `opcache` módulo e configurá-lo corretamente.

- **APCU** — Recomendamos habilitar a extensão do APCu PHP e configurar o Composer para otimizar o desempenho máximo. Essa extensão armazena em cache locais de arquivos abertos, o que aumenta o desempenho para chamadas de servidor do Adobe Commerce, incluindo páginas, chamadas Ajax e endpoints.

- **Realpath_cacheconfiguration** — A otimização  `realpath_cache` permite que os processos PHP armazenem em cache caminhos em arquivos, em vez de pesquisá-los sempre que uma página é carregada.
...
Otimize a configuração `innodb_buffer_pool_instances` para evitar problemas com vários threads que tentam acessar a mesma instância. O valor do parâmetro `max_connections` deve correlacionar-se com o número total de threads PHP configurados no servidor de aplicativos. Use a seguinte fórmula para calcular o melhor valor para `innodb-thread-concurrency`:
...
Recomendamos o uso do Varnish para o cache de página completo na sua loja do Adobe Commerce. O módulo `PageCache` ainda está presente na base de código, mas deve ser usado somente para fins de desenvolvimento.
...
>[!NOTE]
>
> O Adobe Commerce on cloud Infrastructure já aplica todas as otimizações de infraestrutura e arquitetura acima, exceto pela pesquisa DNS, pois está fora do escopo.

### Pesquisar

O Elasticsearch é necessário a partir do Adobe Commerce versão 2.4, mas também é uma prática recomendada habilitá-lo para versões anteriores à versão 2.4.
...
Temos uma seção separada dedicada ao detalhamento do que é [headless](../../architecture/headless/adobe-commerce.md) e opções diferentes. Em resumo, ele separa a camada da loja da própria plataforma. Ainda é o mesmo back-end, mas o Adobe Commerce não processa mais solicitações diretamente e, em vez disso, suporta apenas vitrines personalizadas por meio da API GraphQL.

### Manter o Adobe Commerce atualizado

O Adobe Commerce sempre tem melhor desempenho ao executar a versão mais recente. Mesmo que não seja possível manter o Adobe Commerce atualizado após cada nova versão ser lançada, ainda é recomendável atualizar quando o Adobe Commerce introduz otimizações significativas de desempenho.
