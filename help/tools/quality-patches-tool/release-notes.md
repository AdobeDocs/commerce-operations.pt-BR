...
>[!INFO]
>
>Consulte [Aplicar patches](https://experienceleague.adobe.com/docs/commerce-operations/tools/quality-patches-tool/usage.html#apply-individual-patches) para obter instruções sobre como aplicar patches aos projetos do Adobe Commerce. Consulte [[!DNL Quality Patches Tool]: Procurar patches](https://experienceleague.adobe.com/tools/commerce-quality-patches/index.html) no Guia de Atualização de Software para revisar uma lista completa de patches lançados.
...
* **ACSD-55566** (para Adobe Commerce e Magento Open Source >=2.4.3 &lt;2.4.7) - Corrige o problema em que a variável `mergeCart` falha com um caractere &quot;*Erro interno do servidor*&quot; no [!DNL GraphQL] resposta ao mesclar carrinhos de origem e destino que têm os mesmos itens do pacote.
* **ACSD-56546** (para Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Corrige o problema em que os produtos configuráveis e agrupados são exibidos como **Sem estoque** na loja quando a variável **exibição fora da configuração do produto** é *Desabilitado*.
* **ACSD-56635** (para Adobe Commerce >=2.4.6 &lt;2.4.7) - corrige o problema em que o cliente importado é duplicado com o mesmo endereço de email, quando uma importação é usada com **compartilhamento de conta** definir como *Global*.
* **ACSD-56741** (para Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.7) - Corrige a mensagem de erro &quot;*Tentando acessar o deslocamento da matriz no valor do tipo nulo*&quot; que é exibido durante `setup:upgrade` quando o banco de dados contiver uma tag personalizada [!DNL MySQL] acionador não relacionado ao mecanismo de indexação e [!DNL MView].
* **ACSD-57315** (para Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7) - Corrige o problema quando uma nova transação é criada no [!DNL PayPal Payflow Pro] cada vez que a variável [!UICONTROL Fetch] é clicado no botão **[!UICONTROL View transaction]** na tela Admin.
* **ACSD-57337** (para Adobe Commerce >=2.4.4 &lt;2.4.6) - Corrige o problema em que um usuário administrador com restrições de acesso a sites específicos pode ver empresas de todos os sites na **[!UICONTROL Companies]** grade.
...
* **ACSD-57565** (para Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.7) - Corrige o problema em que a variável **[!UICONTROL Order]** o painel exibe informações incorretas sobre o pedido até que o período seja atualizado. O painel agora exibe as estatísticas de ordem corretas na primeira carga.
* **ACSD-57854** (para Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.7) - Corrige o problema quando o [!DNL GraphQL] as solicitações retornaram categorias desabilitadas nas agregações de categorias.
* **ACSD-58008** (para Adobe Commerce >=2.4.5 &lt;2.4.7) - Corrige o problema em que a atualização programada removia a versão anterior de um item preparado, se nenhuma data final fosse especificada.
...
* **ACSD-56790** (para Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.7) - Corrige o problema em que um usuário é redirecionado para o Painel de administração ao classificar produtos de categoria usando o **Mover do Estoque para o inferior** e a opção `Invalid security or form key. Please refresh the page` aparece na parte superior da tela.
...
* **ACSD-56193** (para Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.4) - Corrige o problema em que o cache Vernish/Fastly não é atualizado quando um bloco agendado é usado na descrição da categoria usando o Page Builder.
* **ACSD-56158** (para Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.7) - Corrige o problema em que a consulta de &quot;carrinho&quot; retorna o valor total do imposto para cada regra de imposto.
* **ACSD-56023** (para Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7) - Corrige o problema em que o conteúdo do widget não é atualizado na página CMS quando o cache é ativado.
* **ACSD-55427** (para Adobe Commerce >=2.4.5 &lt;2.4.7) - Corrige o problema em que o usuário administrador não pode cancelar a atribuição de um produto de um catálogo compartilhado na página de produto no Administrador.
* **ACSD-55352** (para Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7) - Corrige o problema em que, após criar um aviso de crédito parcial com pontos de premiação do cliente, o status do pedido muda para Fechado, e as opções de aviso de crédito desaparecem da página Pedido de administrador.
...
* **ACSD-54283** (para Adobe Commerce >=2.4.3 &lt;2.4.4) - Corrige o problema em que os Produtos/Categorias não atribuídos ao Catálogo compartilhado para o padrão (Grupo geral) ainda estão incluídos no Mapa do site XML.
...
* **ACSD-53722** (para Adobe Commerce >=2.4.4 &lt;2.4.7) - corrige o problema em que o preço das opções de produto agrupadas muda para $0 quando atualizações programadas para escopos diferentes são ativadas.
* **ACSD-53643** (para Adobe Commerce >=2.4.3 &lt;2.4.7) - corrige o problema em que o pedido tem um total incorreto ao fazer um pedido com produtos desativados ou indisponíveis. É corrigido ocultando o *[!UICONTROL Place Order]* para essas ordens de compra.
...
* **ACSD-55414** (para Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.6) - Melhora o desempenho quando o MariaDB tenta converter a entity_id de EAV de string em inteiro.
...
* **ACSD-53118** (para Adobe Commerce >=2.4.0 &lt;2.4.7) - Corrige o problema em que a variável *[!UICONTROL Cart Price Rule]* é aplicado usando o código do cupom enquanto o produto tem um atributo vazio.
...
* **ACSD-54040** (para Adobe Commerce >=2.4.4-p5 &lt;2.4.5 || >=2.4.5-p4 &lt;2.4.6) - Corrige o problema em que a variável *[!UICONTROL Created]* O campo está em branco para exibir detalhes da ordem quando os módulos B2B estão ativados.
...
* **ACSD-53583** (para Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Melhora o desempenho do reindexação parcial para *Categoria Produtos* e *Categorias de produto* indexadores.
* **ACSD-54026** (para Adobe Commerce >=2.4.6 &lt;2.4.7) - Corrige uma mensagem de erro incorreta de um `updateCompanyRole` Solicitação GraphQL para um usuário não autorizado.
...
* **ACSD-52219** (para Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.7) - Corrige o problema em que os filtros salvos das grades de administração não funcionam como esperado ao alternar frequentemente entre exibições de marcadores.
...
* **ACSD-54660** (para Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.6) - Adição de um novo atributo de entrada *sort* para classificar pedidos de clientes no GraphQL por `sort_field` e `sort_direction`.
* **ACSD-54776** (para Adobe Commerce >=2.4.5 &lt;2.4.7) - Corrige o problema em que estava desmarcado *[!UICONTROL Use Default Value]* Os valores de campo de produto e não padrão não são salvos para a segunda exibição de site, loja e loja.
* **ACSD-53998** (para Adobe Commerce e Magento Open Source >=2.4.4-p2 &lt;2.4.5 || >=2.4.5-p1 &lt;2.4.7) - Corrige o problema em que uma **[!UICONTROL Dynamic Block]** com base em um **[!UICONTROL Customer Segment]** não funciona corretamente depois de fazer logoff de uma conta de cliente.
* **ACSD-53204** (para Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.7) - Correções *Não é possível salvar o produto.* erro ao fazer solicitações simultâneas para adicionar imagens à galeria de produtos usando o `rest/V1/products/<sku>/media` terminal.
* **ACSD-47657** (para Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Adição de um mecanismo de cache para credenciais do AWS. Um provedor de credenciais agora usa o cache de Magento para armazenar em cache credenciais recuperadas do AWS para configuração EC2.
...
* **ACSD-52613** (para Adobe Commerce e Magento Open Source >=2.4.6 &lt;2.4.7) - Corrige o problema em que os caches e índices são atualizados mesmo quando não são feitas atualizações no `Inventory_source` por API REST.
...
* **ACSD-53148** (para Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Corrige o problema em que duas solicitações paralelas no GraphQL para adicionar o mesmo produto configurável ao carrinho resultaram em dois itens separados no carrinho com o mesmo SKU de produto.
* **ACSD-52606** (para Adobe Commerce e Magento Open Source >=2.4.0 &lt;2.4.7) - Corrige o problema em que a mensagem de erro *Seu pedido não está pronto para retirada* é exibido quando o usuário clica em **[!UICONTROL Notify Order is Ready for Pickup]**.
...
* **ACSD-52085** (para Adobe Commerce e Magento Open Source >=2.4.5 &lt;2.4.7) - corrige o problema em que um produto configurável com um preço especial não é visível no carrossel do produto.
* **ACSD-53795** (para Adobe Commerce e Magento Open Source >=2.4.2 &lt;2.4.7) - Corrige o problema com tipo de dados inválido no `indexer_update_all_views` trabalho cron.
...
* **ACSD-53750** (para Adobe Commerce e Magento Open Source >=2.4.4 &lt;2.4.7) - Corrige a *Pipe quebrado ou conexão fechada* erro durante multithread `catalog_product_price` reindexar.
* **ACSD-49843** (para Adobe Commerce e Magento Open Source >=2.3.7 &lt;2.4.0 || >=2.4.1 &lt;2.4.7) - Corrige o problema em que o link no download do produto não está disponível depois que o item solicitado é faturado automaticamente pelo método de pagamento online com o **[!UICONTROL Payment Action]** = **[!UICONTROL Sale]** configuração ativada.
* **ACSD-47054** (para Adobe Commerce >=2.4.2 &lt;2.4.6) - Corrige o problema em que o reindex de visualização executa o reindex para todos os armazenamentos, causando lentidão.
