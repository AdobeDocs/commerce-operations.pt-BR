...
O patch ACSD-55381 corrige o problema em que um erro de servidor interno ocorre durante consultas GraphQL para os campos `configurable_product_option_uid` e `configurable_product_option_value_uid` de uma lista de requisições B2B. Este patch está disponível quando o [[!DNL Quality Patches Tool (QPT)]](/help/tools/quality-patches-tool/quality-patches-tool-to-self-serve-quality-patches.md) 1.1.42 está instalado. A ID do patch é ACSD-55381. Observe que o problema está programado para ser corrigido no Adobe Commerce 2.4.7.
