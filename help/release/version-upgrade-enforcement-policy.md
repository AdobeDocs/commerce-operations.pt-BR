...
description: Saiba mais sobre a imposição de atualização de versão para o Adobe Commerce na nuvem (PaaS) — por que o Adobe impõe atualizações, datas de imposição, desativação e ações necessárias. Consulte a política de ciclo de vida para obter disposições transitórias e caminhos de migração.
...
# Política de imposição de atualização de versão para Adobe Commerce na nuvem (PaaS)

Quando o suporte regular e o suporte estendido terminam para uma versão do Adobe Commerce, a Adobe se reserva o direito de desativar os ambientes do Adobe Commerce na nuvem (PaaS) que ainda estejam executando essa versão não compatível. A aplicação de atualização de versão se aplica somente a ambientes PaaS hospedados pela Adobe; os clientes locais gerenciam sua própria infraestrutura.
...
Quando o suporte de segurança para dependências de software subjacentes é oficialmente encerrado pelos fornecedores, a Adobe não pode mais fornecer o nível necessário de cobertura de segurança e suporte à plataforma. Continuar a administrar lojas em infraestruturas não compatíveis coloca os clientes, seus compradores e a Adobe em riscos inaceitáveis. O Adobe está introduzindo uma política formal de imposição de atualização de versão imposta que define quando os ambientes da nuvem hospedados pela Adobe que executam versões do Commerce não compatíveis serão desativados.
...
**Data de imposição da atualização** = Data de disponibilidade geral + 3 anos de suporte regular + 1 ano de suporte estendido
