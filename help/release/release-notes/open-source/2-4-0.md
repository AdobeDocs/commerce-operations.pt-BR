...
Embora o código desses recursos esteja incluído nas versões trimestrais, vários desses projetos (por exemplo, Inventory management e Progressive Web Applications (PWA) Studio) também são lançados independentemente. As correções de erros para esses projetos estão documentadas nas informações de versão separadas e específicas do projeto, disponíveis na documentação de cada projeto.
...
* **Suporte ao MariaDB 10.4**. O suporte para MySQL 8.0 oferece a oportunidade para os comerciantes implantarem o MariaDB 10.4 com o Magento Open Source. Embora os comerciantes ainda possam usar o MariaDB 10.2 com 2.4.0, recomendamos a atualização para o MariaDB 10.4 para melhorar o desempenho e a confiabilidade. Não há mais suporte para MariaDB 10.0 e 10.1 (como resultado da remoção do suporte para MySQL 5.6 nesta versão).
...
Essa extensão substitui nossa integração principal do Braintree. Ele fornece os mesmos recursos que a integração principal do Braintree. Consulte [Braintree Payments](https://commercemarketplace.adobe.com//paypal-module-braintree.html).
...
A instalação do Magento Open Source agora é concluída com êxito, e os armazenamentos são criados conforme esperado, quando a configuração de armazenamento é predefinida em `config.php`.
...
* O comando `php bin/magento setup:cron:run` foi removido. As referências a isso nos crontabs devem ser removidas ao atualizar.
...
* Os pedidos concluídos agora aparecem no sistema de pagamento e no Magento Open Source. Anteriormente, os pedidos apareciam no sistema de pagamento, mas não no Magento Open Source. [GitHub-25862](https://github.com/magento/magento2/issues/25862)
...
* O Elasticsearch 7.6.x agora é o mecanismo de pesquisa de catálogo padrão do Adobe Commerce e do Magento Open Source. Não é possível instalar ou atualizar para a versão 2.4.0 sem instalar também o Elasticsearch 7.6.x. O código do Elasticsearch versão 2.x foi removido. As versões 5.x e 6.x do Elasticsearch foram substituídas e não são mais compatíveis. Consulte [pré-requisitos do mecanismo de pesquisa](../../../installation/prerequisites/search-engine/overview.md).
...
* O Magento Open Source `cron` agora é usado para reindexação após a criação de dados de teste, e onde aplicável, `cache:flush config` substitui `cache:flush`. _Correção enviada por Lukasz Bajsarowicz na solicitação pull [26990](https://github.com/magento/magento2/pull/26990)_. [GitHub-26989](https://github.com/magento/magento2/issues/26989)
...
* Se um membro da comunidade tiver fornecido uma correção para esta versão, identificaremos a correção na seção Problema Corrigido dessas notas com a frase &quot;*Correção fornecida pelo membro da comunidade @member_name*&quot;.
