...
description: Saiba mais sobre novos recursos, melhorias, correções de erros e problemas conhecidos no Magento Open Source versão 2.4.2.
...
O Magento Open Source 2.4.2 apresenta aprimoramentos no desempenho e na segurança, além de melhorias significativas na plataforma. As melhorias de segurança incluem a expansão do suporte para a `SameSite` para todos os cookies. Elasticsearch 7.9.x e Redis 6.x agora são compatíveis.
...
* As operações do sistema de arquivos em todos os componentes foram padronizadas e rígidas para evitar uploads mal-intencionados.
...
Esta versão contém aprimoramentos na qualidade principal, que melhoram a qualidade da Estrutura e dessas áreas funcionais: Conta do cliente, Catálogo, CMS, OMS, Importação/Exportação, Promoções e Direcionamento e Carrinho e Check-out.
...
* Adição de suporte para sindicatos no GraphQL. [GitHub-29425](https://github.com/magento/magento2/issues/29425) <!-- ENGCOM-8291 -->
...
* O aplicativo não gera mais um erro quando você tenta alterar `backend-frontname` usando o `ssh` após instalar o Magento Open Source. [GitHub-26762](https://github.com/magento/magento2/issues/26762)
...
* A lógica foi removida do controlador WYSIWYG OnInsert (`Magento\Cms\Controller\Adminhtml\Wysiwyg\Images\OnInsert::execute()`) e adicionados à `Model\Wysiwyg\Images\PrepareImage::execute()` modelo. [GitHub-1504](https://github.com/magento/adobe-stock-integration/issues/1504)
...
* A aplicação de uma regra de preço com um cupom usando o GraphQL agora funciona como esperado quando a variável **Desconto de valor fixo para o carrinho inteiro** é usada.
...
* O aplicativo não lista mais um produto disponível para download na guia Meus produtos de download depois que o pedido ao qual ele pertence foi parcialmente reembolsado. [GitHub-28388](https://github.com/magento/magento2/issues/28388)
...
* O resolvedor de URL do GraphQL agora lida com parâmetros de consulta corretamente. Anteriormente, retornava NULL ao manipular URLs compatíveis com SEO com parâmetros.

<!-- ENGCOM-8422 -->

* O resolvedor de URL do GraphQL agora lida com a hierarquia de página do CMS corretamente. Anteriormente, retornava NULL. [GitHub-30474](https://github.com/magento/magento2/issues/30474)

<!-- MC-38209-->

* Consultas de um produto `categories.breadcrumbs` Os dados não retornam mais dados de navegação estrutural para categorias cuja categoria principal está desativada. [GitHub-30468](https://github.com/magento/magento2/issues/30468)

<!-- MC-38407-->

* O caminho para uma imagem de categoria agora é retornado corretamente quando as regravações de URL são desativadas.
...
* As posições dos produtos agora são atualizadas conforme esperado nos resultados da consulta do GraphQL quando os resultados da consulta são armazenados em cache.
...
* O `FlushCacheByTags` A classe foi atualizada para usar após plug-ins em vez de plug-ins. [GitHub-29558](https://github.com/magento/magento2/issues/29558)
...
* O gerador de reescrita de URL agora define o objeto de categoria `url_key`, `url_path`e  `store_id` para armazenar valores padrão ao salvar substituições de URL de categoria para o escopo global. Anteriormente, ele era salvo `url_key`, `url_path`e `store_id` com os valores da última ID de armazenamento processada. [GitHub-29585](https://github.com/magento/magento2/issues/29585)### Produtos agrupados
...
* O tempo de resposta do GraphQL para adicionar operações ao carrinho foi aprimorado.
...
* O GraphQL agora respeita as permissões de catálogo. O aplicativo restringe quais itens são retornados para um `products` query com base no grupo de clientes do comprador.
...
* Correção de problemas com a GraphQL `testRequestCacheTagsForCategoryListOnMultipleIds` teste. [GitHub-29372](https://github.com/magento/magento2/issues/29372)

<!-- ENGCOM-7762 -->

* Os valores do cabeçalho que incluem URLs agora são analisados corretamente em `ApiFunctional TestFramework`. [GitHub-26425](https://github.com/magento/magento2/issues/26425)
...
**Problema**: O aplicativo cria um pedido no Braintree, como esperado, quando um comprador clica **Pagar com Venmo**, mas não cria a ordem no Administrador. Consulte a [2.4.2 problema conhecido: O pagamento Braintree Venmo não funciona](https://support.magento.com/hc/en-us/articles/360054662652/) Artigo da Base de conhecimento.  <!-- BUNDLE-2894-->

**Problema**: Não é possível usar a variável `addConfigurableProductToCart` Mudança GraphQL para adicionar um produto configurável a um carrinho em uma visualização de loja não padrão em uma implantação de várias lojas que executa Inventário. (As vitrines de Luma não são afetadas.) o aplicativo exibe este erro: `Could not add item to cart. Please check required options and try again`. **Solução alternativa**: Use o `addProductsToCart` em vez disso. [GitHub-31660](https://github.com/magento/magento2/issues/31660)<!-- PWA-1298-->
...
Você pode instalar o Magento Open Source 2.4.2 usando [Composer](../../../installation/composer.md).
