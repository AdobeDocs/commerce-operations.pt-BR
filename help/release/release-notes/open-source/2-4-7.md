...
* **Suporte para 3DS para[!DNL Google Pay]**—Suporte para verificação 3DS incluído para [!DNL Google Pay] placas não tokenizadas. Consulte a [documentação do Braintree](https://developer.paypal.com/braintree/docs/guides/3d-secure/advanced-options/javascript/v3#verify-google-pay-card-using-3ds) para obter mais informações.
...
* **Suporte para cancelamento de pedido**. A mutação [cancelOrder](https://developer.adobe.com/commerce/webapi/graphql/schema/orders/mutations/cancel-order/) permite que um cliente cancele um pedido, transmitindo seu identificador e um motivo de cancelamento. <!-- LYNX-197 -->
  ...
  * Os novos campos de resposta `order_cancellation_enabled` e `order_cancellation_reasons.description` no `storeConfig`
    suporte a consultas solicitações de cancelamento de pedidos iniciadas pelo usuário. Consulte [Consultar a configuração de cancelamento de pedido de um armazenamento](https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/store-config#query-a-stores-order-cancellation-configuration)
...
* [Payment Services](https://experienceleague.adobe.com/docs/commerce/payment-services/guide-overview.html?lang=pt-BR)
...
* Os produtos do pacote agora são importados com êxito sem duplicar SKUs nas opções do produto. Anteriormente, a Magento Open Source criava vários SKUs duplicados nas opções do produto quando você importava produtos agrupados com entradas duplicadas.
...
* As respostas das operações GET do GraphQL em páginas do CMS que contêm blocos do CMS agora são apagadas conforme esperado no cache do Fastly. As atualizações de programação subsequentes agora mostram um conteúdo preciso e atualizado dessas páginas.
...
* O Magento Open Source agora exibe mensagens informativas quando uma ação complementar é acionada. Anteriormente, o `The requested qty is not available` nem sempre era exibido conforme necessário na loja.
...
* O Magento Open Source agora exibe o nome correto após classificar produtos por nome em Administração > **[!UICONTROL Category]** > **[!UICONTROL Products in category]** em implantações de várias lojas onde os nomes de produtos variam de acordo com a loja. [GitHub-36208](https://github.com/magento/magento2/issues/36208)
...
* Os comerciantes agora podem adicionar uma imagem do GIF à galeria de imagens de um produto do Administrador. Anteriormente, o Magento Open Source exibia este erro: `imagecolorsforindex(): Argument #2 ($color) is out of range`.
...
* O Magento Open Source agora retorna um resultado válido para a data personalizada `01/01/1970`. Anteriormente, o atributo personalizado retornava este erro: `Invalid input datetime format of value '1/01/1970`. [GitHub-37274](https://github.com/magento/magento2/issues/37274)
...
* Código refatorado em toda a base de código que foi criada quando a prática de codificação ditava que qualquer seção de cache incluída no arquivo `app/etc/env.php` incluiria uma seção de front-end. Essa refatoração aborda possíveis problemas ao atualizar implantações anteriores à versão 2.4.4 do Magento Open Source. [GitHub-38363](https://github.com/magento/magento2/issues/38363)
...
* O Magento Open Source não lança mais uma exceção no arquivo de registro quando um usuário tenta imprimir uma etiqueta de remessa. Anteriormente, se uma extensão modificava cabeçalhos de resposta que continham um anexo, o sistema lançava uma exceção no arquivo de log.
...
* A importação de fontes de estoque e endereços de clientes usando a opção **[!UICONTROL Customers and Addresses (single file)]** agora é concluída com êxito.
...
* Agora o Magento Open Source lida corretamente com erros 502 do PayPal, impedindo a criação de pedidos quando esse erro ocorre. Anteriormente, os pedidos ainda eram criados apesar de um erro 502 do PayPal, que resultava em inconsistências entre os registros do Magento Open Source e do PayPal.
...
* Os compradores agora podem selecionar um método de envio ao aplicar o código do cupom durante o checkout. Anteriormente, sob essas condições, a Magento Open Source emitia um erro e atribuía um frete com taxa uniforme ao pedido.
...
* O teste `\Magento\Test\Integrity\Di\CompilerTest::testConstructorIntegrity` agora verifica todas as classes PHP de módulos registrados por `ComponentRegistrar()`.
...
* O atributo `crossorigin` foi adicionado a `linkTypes` para `HEAD` elementos.[GitHub-32509](https://github.com/magento/magento2/issues/32509)
...
* Mensagens de erros aprimoradas que ocorrem durante a renderização do modelo. O Magento Open Source agora exibe uma mensagem informativa. Anteriormente, o Magento Open Source exibia vazamentos de HTML quando ocorria um erro de PHP.
...
* O Magento Open Source agora retorna com êxito o atributo `content` (código de imagem base64) em `GET V1/products/sku/media` respostas da API de mídia do produto. O atributo `content` não apareceu anteriormente na resposta.
