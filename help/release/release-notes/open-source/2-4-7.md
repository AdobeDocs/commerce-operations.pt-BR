...
O Magento Open Source 2.4.7 ainda é compatível com o PHP 8.1 apenas para fins de atualização. O PHP 8.1 não é suportado e não é recomendado. O código núcleo Magento Open Source 2.4.7, todas as extensões agrupadas e todas as extensões pertencentes ao Adobe e serviços SaaS são compatíveis com o PHP 8.3. <!-- AC-9668 -->

* **Suporte ao RabbitMQ 3.13**. Esta versão é compatível com a versão mais recente do RabbitMQ 3.13. A compatibilidade permanece com o RabbitMQ 3.11 e 3.12, que são compatíveis até agosto de 2024 e dezembro de 2024, respectivamente, mas o Adobe é recomendado usando o Magento Open Source 2.4.7 somente com o RabbitMQ 3.13. <!-- AC-9659 -->

* **Composer 2.7.x**. A compatibilidade com o Composer 2.2.x permanece. <!-- AC-9663 -->

* **Suporte ao cache de verniz 7.4**. Esta versão é compatível com a versão mais recente do Varnish Cache 7.4. A compatibilidade permanece com as versões 6.0.x e 7.2.x, mas recomendamos usar somente o Magento Open Source 2.4.7 com o Varnish Cache versão 7.4 ou versão 6.0 LTS. <!-- AC-9662 -->
...
* Os módulos de envio do Temando foram removidos da base de código do Magento Open Source principal. Este recurso foi descontinuado no Magento Open Source 2.4.4. <!-- AC-8130 -->
...
* `bin/magento setup:install` agora é concluído com êxito depois que `app/etc/config.php` é excluído. Anteriormente, o arquivo ausente não era gerado novamente durante a instalação e o Magento Open Source gerava um erro. [GitHub-37805](https://github.com/magento/magento2/issues/37805)
...
* Adição de uma declaração ausente `return` ao controlador de ativação de uso do Admin. Anteriormente, não `return` havia nenhuma declaração, e o controlador na `FrontendController` classe era tratado incorretamente. [GitHub-31374](https://github.com/magento/magento2/issues/31374)

<!-- AC-9390 -->

* Substituição do MySQL `CREATE TEMPORARY TABLE ... LIKE` por reformulação `CREATE TEMPORARY TABLE` para conformidade com o MySQL 8.x. [GitHub-37926](https://github.com/magento/magento2/issues/37926)

<!-- ACP2E-1462 -->

* Magento Open Source não inclui mais o caminho completo para a raiz do servidor na mensagem de erro que ele exibe quando uma solicitação HTTP malformada é enviada ao servidor. Somente as cadeias de caracteres de caracteres agora são passadas para o `explode` método como um segundo argumento.

<!-- AC-8384 -->

* `restricted_classes.php` substituiu `Zend_Validate_File_Upload`. Anteriormente, a validação falhava durante a upload de um arquivo de importação maior do que o valor configurado em `php.ini `. [GitHub-37281](https://github.com/magento/magento2/issues/37281)
...
* `product` as consultas com dados de agregação agora retornam os valores de rótulo corretos para atributos de produto do tipo booleano. [GitHub-29123](https://github.com/magento/magento2/issues/29123)

<!-- AC-2618 -->

* Adicionadas Ucraniano regiões à `directory_country_region` tabela. [GitHub-35187](https://github.com/magento/magento2/issues/35187)

<!-- https://github.com/magento/magento2/pull/29173 -->

* Os arquivos CSS não estão mais corrompidos quando unidos quando `var/tmp` `pub/static` estão em diferentes sistemas de arquivos e **[!UICONTROL CSS Merging Enabled]** são ativados. Arquivos temporários foram criados no mesmo diretório estático que o arquivo de Direcionamento para garantir que ambos os arquivos estejam no mesmo sistema de arquivos. Como resultado, a chamada para renomeá-la é atômica. [GitHub-29172](https://github.com/magento/magento2/issues/29172)
...
* O Magento Open Source não lança mais um erro ao criar uma segunda fatura parcial para um pedido pago pelo Braintree quando o **[!UICONTROL Enable Vault for Card Payments]** está habilitado. Anteriormente, quando um comerciante tentava criar uma segunda fatura, o Magento Open Source exibia este erro: `Your payment could not be taken. Please try again or use a different payment method. Payment method token is invalid`.
...
* Magento Open Source agora exibe o valor correto para o valor **[!UICONTROL Regular Price]** quando uma opção personalizada é selecionada para um produto que tem um valor **[!UICONTROL Special Price]** quando **[!UICONTROL Price Display Settings]** está definido para incluir imposto.
...
* O formulário de registro do cliente agora é carregado conforme esperado quando inclui vários atributos selecionados do cliente. Anteriormente, o Magento Open Source gerou esta exceção: `[2023-03-05T05:57:40.032602+00:00] report.CRITICAL: Exception: Deprecated Functionality: explode(): Passing null to parameter #2 ($string) of type string is deprecated in /magento2ee/app/code/Magento/CustomAttributeManagement/Block/Form/Renderer/Multiselect.php on line 53 in /lib/internal/Magento/Framework/App/ErrorHandler.php:62`. [GitHub-37189](https://github.com/magento/magento2/issues/37189)
...
* O Magento Open Source não gera mais regravações de URL de produto para categorias raiz e para produtos definidos como **[!UICONTROL Not Visible Individually]** em configurações de várias lojas ao gerar regravações de URL ausentes para lojas personalizadas.
...
* As operações assíncronas de API REST em massa para atualizar produtos com preços de camada e diferentes conjuntos de atributos foram concluídas com êxito. Anteriormente, os produtos com diferentes conjuntos de atributos não podiam ser atualizados dessa forma, e o Magento Open Source exibia este erro: `The product was unable to be saved. Please try again`.
