...
* O comando `bin/magento config:set graphql/session/disable 1` permite que os comerciantes desabilitem completamente a criação de cookies de sessão para todas as operações do GraphQL. Por padrão, o aplicativo Open Source cria esses cookies e depende deles para autorização, o que afeta o desempenho. A partir de agora, recomendamos usar tokens como a única forma de autorização para solicitações do GraphQL. Não recomendamos usar cookies de sessão sozinhos ou em conjunto com tokens de autorização. Consulte [Autorização do GraphQL](https://devdocs.magento.com/guides/v2.4/graphql/authorization-tokens.html). <!-- B2B-2204 -->
...
* O cache de página inteira não é mais exibido como desabilitado no Administrador quando o cache de Magento Open Source é liberado e o `use_stale_cache` está habilitado.
...
* O Magento Open Source agora exibe uma mensagem de erro ao definir um domínio de cookie inválido (**Armazenamento** > **Configurações** > **Web** > **Domínio de Cookie Padrão**). Anteriormente, o site travava. [GitHub-35048](https://github.com/magento/magento2/issues/35048)

<!-- AC-2765-->

* O Magento Open Source não lança mais um erro quando um administrador com funções no escopo de um único site adiciona o produto aos Elementos de conteúdo usando o PageBuilder. Anteriormente, o applicationOpen Source gerava um erro de SQL.
...
* O Magento Open Source não gera mais o seguinte erro durante a criação de uma regra de catálogo no Administrador após a atualização: `A technical problem with the server created an error. Try again to continue what you were doing. If the problem persists, try again later`.
...
* As sessões de Magento Open Source não terminam mais após a realização de uma solicitação GraphQL. Anteriormente, o plug-in `ClearCustomerSessionAfterRequest` desconectou o comprador. [GitHub-34550](https://github.com/magento/magento2/issues/34550)
...
* O Magento Open Source agora exibe registros do armazenamento solicitado na página de grade de avisos de crédito em implantações executando o PHP 7.4. Anteriormente, o Source applicationOpen gerou o seguinte erro depois de criar um memorando de crédito e tentar exibi-lo: `The store that was requested wasn't found. Verify the store and try again`.
...
* A página **Armazenamento** > **Configuração** > **Serviços** agora exibe informações da API da Web Magento Open Source, conforme esperado, quando o Acesso a Recursos está definido como **Personalizado** na guia Recursos de Função. [GitHub-35506](https://github.com/magento/magento2/issues/35506)
...
* As consultas Elasticsearch agora funcionam como esperado quando `int` está configurado como um atributo `type` de back-end pesquisável. Anteriormente, o Source applicationOpen gerava uma exceção `Elasticsearch\Common\Exceptions\BadRequest400Exception`.
...
* As solicitações de PUT e DELETE em massa da REST API agora funcionam como esperado quando o módulo `Magento_ReCaptchaWebapiRest` é habilitado. [GitHub-35348](https://github.com/magento/magento2/issues/35348)
