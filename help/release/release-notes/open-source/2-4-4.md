...
A DHL apresentou o schema versão 6.2 e descontinuará o schema versão 6.0 em breve. Adobe Commerce 2.4.4 e versões anteriores que suportam a integração DHL suportam apenas a versão 6.0. Os comerciantes que implantarem essas versões devem aplicar o `AC-3022.patch` o mais rápido possível para continuarem oferecendo a DHL como transportadora. Consulte o [Aplicar um correção de continuar oferecendo o DHL como artigo da Knowledge Base da operadora](https://support.magento.com/hc/en-us/articles/7707818131597-Apply-a-patch-to-continue-offering-DHL-as-shipping-carrier) de transporte para obter informações sobre como baixar e instalar o correção.

## Outras informações da versão

Embora o código desses recursos seja agrupado com versões trimestrais do Magento Open Source código principal, vários desses projetos também são lançados de forma independente. As correções de erros desses projetos estão documentadas nas informações de versão separadas e específicas do projeto, disponíveis na documentação de cada projeto.

>[!NOTE]
>
>Todas as extensões empacotadas por fornecedores, com exceção de Braintree, foram removidas do Magento Open Source 2.4.4.

>[!NOTE]
>
>O Composer 2.2 introduziu um recurso de segurança que exige que os comerciantes identifiquem plug-ins confiáveis em seu `composer.json` arquivo antes que os plug-ins sejam executados. Atualmente, os plug-ins incluídos no arquivo `composer.json`, mas não marcados como confiáveis, são instalados automaticamente. No entanto, a partir de julho de 2022, o Composer não carregará plug-ins, a menos que tenham sido explicitamente permitidos. Consulte os problemas dos plug-ins do [Composer ao atualizar para o Adobe Systems Comércio artigo da base de conhecimento 2.4.4](https://support.magento.com/hc/en-us/articles/6215997614093-Composer-plugins-issues-when-upgrading-to-Adobe-Commerce-2-4-4) para obter instruções sobre como modificar o `composer.json` arquivo para permitir explicitamente plug-in carregamento.
...
Consulte o [Boletim de Segurança do Adobe](https://helpx.adobe.com/br/security/products/magento/apsb22-13.html) para obter a discussão mais recente sobre esses problemas corrigidos.
...
* O HTTPS agora está habilitado por padrão para a loja de Magento Open Source. As configurações **Usar URLs Seguras na Loja** e **Usar URLs Seguras no Administrador** estão habilitadas por padrão, e todos os cookies internos agora estão definidos como seguros.  <!-- AC-1173-->
...
* Os desenvolvedores agora podem configurar o limite de tamanho de arrays aceitos pelos endpoints RESTful do Magento Open Source com base no endpoint. Consulte [Segurança de API](https://developer.adobe.com/commerce/webapi/get-started/api-security/). <!-- AC-465-->
...
Com exceção de [Braintree](https://experienceleague.adobe.com/pt-br/docs/commerce-admin/stores-sales/payments/braintree), todas as extensões agrupadas por fornecedor foram removidas da base de código Magento Open Source 2.4.4. Os comerciantes devem migrar para as extensões oficiais, que estão disponíveis no [Commerce Marketplace](https://marketplace.magento.com/extensions.html).

[Pagamento do Amazon](https://experienceleague.adobe.com/pt-br/docs/commerce-admin/stores-sales/payments/payments)
...
* Alternativas textuais foram adicionadas ao ícone de lápis que aparece quando um comerciante edita a entrada para o menu Search Otimização de mecanismos.

<!-- AC-1272-->

* O ícone da lupa usado para executar pesquisas em toda a interface do produto recebeu um nome acessível e uma alternativa textual.

<!-- AC-1286-->

* O página do produto **Novo Exibir** campo de entrada de texto agora tem um nome acessível.
...
* O Magento Open Source não emite mais um erro quando um comprador clica em **Atualizar carrinho** depois de alterar a quantidade de um produto no minicarrinho. Anteriormente, o applicationOpen Source exibia este erro: `The quote item isn't found. Verify the item and try again`.
...
* O Magento Open Source não gera mais este erro quando um comprador adiciona um endereço de cobrança que não tem um campo de rua: `Uncaught TypeError: Unable to process binding "text: function(){return currentBillingAddress().street.join(', ') }"`. [GitHub-33826](https://github.com/magento/magento2/issues/33826)
...
* Magento Open Source não gera mais um erro após usar a `addConfigurableProductsToCart` mutação para adicionar um produto configurável a um carrinho diretamente depois de alterar o armazenamento visualização.

<!-- ACP2E-176-->

* Magento Open Source não gera mais um erro quando você usa a `addConfigurableProductsToCart` mutação para adicionar um filho desativado de um produto configurável ao carrinho. Anteriormente, o applicationOpen Origem lançou um erro de violação de restrição de integridade.
...
* As regras de preço do carrinho que usam uma condição de SKU com **Se um item for ENCONTRADO/NÃO ENCONTRADO no carrinho com Todas/Quaisquer dessas condições verdadeiras** agora funcionam como esperado com produtos configuráveis.

<!-- ACP2E-225-->

* As regras de preço do carrinho agora são aplicadas corretamente para produtos de pacote de preço fixo. Anteriormente, carrinho regras de preço não se aplicavam a esses produtos quando pelo menos um produto filho tinha o mesmo conjunto de atributos especificado na condição regra.

<!-- ACP2E-285-->

* Um produto com o preço de 0 pode ser adicionado com êxito ao carrinho quando Magento Open Source é configurado com o **desconto de Valor fixo para carrinho** carrinho preço inteiro regra. Anteriormente, o applicationOpen Origem exibido este erro:  `report.ERROR: Warning: Division by zero in /home/mer33515/public_html/ee24develop/app/code/Magento/SalesRule/Helper/CartFixedDiscount.php on line 85`.

<!-- ACP2E-306-->

* Correção de uma condição em que o `cart` query retornava um valor de desconto impreciso quando carrinho de compras regras com descontos de quantia fixa eram ativadas.

### Catálogo

<!-- MC-41796-->

* Magento Open Source não gera mais uma exceção ao executar uma ação de atualização de atributo em massa na grade do produto quando um produto tem um `datetime` atributo.

<!-- MC-42571-->

* As consultas GraphQL agora retornam resultados para produtos secundários de um produto configurável de acordo com as configurações de filtro de visibilidade do(s) produto(s) filho.

<!-- MC-41853-->

* Magento Open Source não modifica mais os preços dos produtos relacionados quando os atributos de produto configuráveis são alterados. Anteriormente, o Preço Mínimo Anunciado (MAP) de um produto configurável sobrescrevia o preço de produtos relacionados no armazenamento frente.
...
* Magento Open Source não remove mais as regras de preço do catálogo em produtos configuráveis durante a reindexação parcial. Anteriormente, somente os subprodutos eram reindexados quando os produtos configuráveis eram reindexados.
...
* As listas de produtos agora incluem corretamente produtos secundários de um produto em estoque quando o sinalizador global para exibir produtos não estoque está habilitado. Anteriormente, a lista de produtos não incluía o produto filho se uma das várias opções de produto filho estivesse indisponível quando esse sinalizador era ativado.
...
* Clicar na pré-visualização de qualquer modelo de email agora abre a pré-visualização do modelo em uma janela separada, conforme esperado.  Anteriormente, o aplicativo Abrir Source abria a pré-visualização do modelo simultaneamente a partir da grade Modelos de email e em uma janela pop-up.
...
* Novas opções necessárias foram adicionadas aos arquivos `.htaccess` e `nginx.conf` que são distribuídos com Magento Open Source para suporte ao PHP 8.x. As opções desatualizadas também foram removidas. [GitHub-34358](https://github.com/magento/magento2/issues/34358)
...
* por padrão, o Magento Open Source redimensiona imagens de forma síncrona durante o salvamento do produto. Os comerciantes agora podem redimensionar imagens como um processo assíncrono em segundo plano por pequenas edições em um arquivo `di.xml`.
...
* Magento Open Source agora habilita os campos Salvar **botão e preenchimento automático quando você inscreve uma URL incompleta ao adicionar um vídeo (Catálogo** de administração **>** Produtos **>** Adicionar Vídeo **).** Anteriormente, o **Salvar** botão era desativado e os campos não eram preenchidos.
...
* O Magento Open Source agora remove marcas HTML como esperado da página **Conta** > **Meus Pedidos** > **Exibir pedido** da vitrine. Anteriormente, o applicationOpen Source exibia as tags HTML na seção de comentário de pedido do cliente da loja.
...
* As opções de Navegação em camadas para a faixa de preço agora funcionam como esperado com atributos de preço personalizados. O aplicativo Open Origem usa a configuração do preço navegação etapa ao filtrar atributos de preço personalizados. Anteriormente, o applicationOpen Origem usado a configuração de etapa manual.

<!-- MC-42545-->

* A vitrine agora mostra todas as subcategorias do categoria atual em camadas navegação independentemente do número de categorias disponíveis no catálogo.

<!-- MC-41706-->

* Elasticsearch pesquisas no catálogo agora não distinguem entre maiúsculas e minúsculas. Anteriormente, pesquisas por termos sem acento resultou em resultados diferentes do que pesquisas no mesmo termo com acento.

<!-- ACP2E-8-->

* Search peso agora é definida corretamente em atributos SKU que contêm caracteres especiais.

<!-- ACP2E-36-->

* `category_name` foi adicionado aos códigos de atributo de produto reservados ao sistema. O aplicativo Abrir Source agora mostra um erro se um usuário tentar criar um atributo de produto com o código de atributo `category_name`. Anteriormente, os produtos não eram listados na categoria esperada e um erro de Elasticsearch era registrado.
...
* O Magento Open Source agora corta os caracteres de espaço não separáveis do campo de entrada de email do formulário Fale conosco. Anteriormente, o formulário era enviado sem remover os caracteres de espaço sem quebra da entrada de email (se fornecida), o que causava erros nos arquivos de log. O campo **Responder para** também estava ausente da mensagem de email de contato gerada para o administrador de armazenamento.
...
* Erro mensagens nos detalhes do produto página não desapareçam mais rapidamente quando os **produtos de widgets sincronizados com back-end armazenamento** configuração é ativada.

<!-- ACP2E-70-->

* A validação para valores negativos foi adicionada ao **campo Quantidade a retornar** no Criar Novo formulário Return.

<!-- ACP2E-105-->

* Magento Open Source não exibe mais endereços duplicado quando um consumidor clica **em Alterar endereço** na página de Revisão e Pagamentos em um implantação em que a **configuração de configuração Search** de Endereço está habilitada.

<!-- ACP2E-136-->

* A **botão Adicionar produtos** agora está ativada conforme o esperado para administradores com permissões restritas enquanto trabalha em escopo de armazenamento-visualização no Admin Categoria página.

<!-- ACP2E-169-->

* Exibir pontos de interrupção são consistentes em todas as Magento Open Source.
...
* Adicionar um novo desserializador à API REST não remove mais outros desserializadores. Anteriormente, a API REST aceitava uma nova tipo de conteúdo, mas os tipos de conteúdo definidos por CORE para APIs não funcionavam mais e retornava um erro 400. [GitHub-26433](https://github.com/magento/magento2/issues/26433)
...
* Magento Open Source agora gera uma `customertoken` solicitação de API por GraphQL ou REST conforme esperado, após várias falhas consecutivas fazer logon tentativas. Anteriormente, o applicationOpen Origem não verificava se o valor de `lock_expires_at` entrada `oauth_token_request_log` era maior do que a data e hora atuais, e sempre retornava o número de tentativas com falha, o que impedia o cliente de se fazendo logon. [GitHub-34067](https://github.com/magento/magento2/issues/34067)

<!-- AC-1190-->

* Adicionar uma ID de cotação mascarada a `QuoteRepository::getList` não causa mais um loop infinito. Anteriormente, usar o processador de pagamentos que confiava na REST API para atualizar pedidos desencadeava esse comportamento. [GitHub-33675](https://github.com/magento/magento2/issues/33675)
...
* Correção de um erro que ocorria quando **a Code Adicionar armazenamento a URLs** era definida **como Sim** e uma chamada REST não incluía um código de armazenamento. Embora a carga fosse válida, a chamada retornou o seguinte erro: `Fatal error: Allowed memory size of **&#x200B;**&#x200B;**&#x200B;**&#x200B;**&#x200B; bytes exhausted (tried to allocate &#x200B;**&#x200B;** bytes) in vendor/magento/module-store/Model/StoreRepository.php on line 75`.
...
* o Magento Open Source não processa mais uma lista de desejos na barra lateral da categoria quando a opção **Mostrar na barra lateral** da lista de desejos está desabilitada. Anteriormente, o applicationOpen Source ignorava essa opção.
