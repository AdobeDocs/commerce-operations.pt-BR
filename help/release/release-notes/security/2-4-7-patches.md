...
>
>Após essa data, o Adobe Commerce 2.4.7 não fornecerá compatibilidade ou
...
>
>Todos os clientes locais do Adobe Commerce que executam a versão 2.4.7 são altamente
>aconselhados a migrar seus servidores de banco de dados para uma versão compatível do MariaDB.

## 2.4.7-p10

A versão de segurança do Adobe Commerce 2.4.7-p10 fornece correções de bugs de segurança para vulnerabilidades identificadas em versões anteriores do 2.4.7.
...
#### Compatibilidade com MariaDB 11.8

O Adobe Commerce 2.4.7-p10 apresenta compatibilidade com o MariaDB versão 11.8. O MariaDB 11.8 apresenta alterações no comportamento SQL, padrões, descontinuações e otimizações de desempenho. Essa atualização soluciona proativamente possíveis problemas para manter a estabilidade da plataforma e a disponibilidade futura.

#### Suporte à versão secundária mais recente do OpenSearch 3

O Adobe Commerce 2.4.7 agora é compatível com a versão secundária mais recente do OpenSearch 3 no Adobe Commerce em infraestrutura em nuvem, Cloud Native e implantações locais. A compatibilidade com o OpenSearch 2 é mantida.

#### Suporte ao Valkey 8.1 LTS

O Adobe Commerce 2.4.7 agora é compatível com o Valkey 8.1 LTS, fornecendo uma opção de back-end de cache de suporte de longo prazo que é compatível com o Adobe Commerce na infraestrutura em nuvem.

#### Suporte ao RabbitMQ 4.2

O Adobe Commerce 2.4.7 agora é compatível com o RabbitMQ 4.2, que aborda a data de fim de suporte do RabbitMQ 4.1 programada para fevereiro de 2026. A compatibilidade com o Apache AtiveMQ Artemis é mantida, e o AtiveMQ permanece o serviço de fila de mensagens padrão para essa linha de versão somente de segurança.
...
Para lidar com a aposentadoria do Laminas MVC, a Adobe Commerce agora usa uma bifurcação de propriedade da Magento de `laminas-mvc` (publicada como `magento/magento-zf-mvc`). Essa bifurcação garante a correção contínua e a conformidade de segurança a longo prazo para o Adobe Commerce 2.4.7.

## 2.4.7-p9

A versão de segurança 2.4.7-p9 do Adobe Commerce fornece correções de bugs de segurança para vulnerabilidades identificadas em versões anteriores da 2.4.7.
...
#### Adicionar compatibilidade com a versão mais recente do Composer

O Adobe Commerce 2.4.7 foi atualizado para oferecer suporte ao Composer 2.9.x, permanecendo compatível com o Composer 2.2 LTS.
...
* **Atualizar [configurações de senha única](https://experienceleague.adobe.com/pt-br/docs/commerce-admin/config/security/2fa) para o Google Authenticator**-Esta atualização é necessária para resolver um erro introduzido por uma [alteração incompatível com versões anteriores](https://developer.adobe.com/commerce/php/development/backward-incompatible-changes/highlights/#new-system-configuration-validation-for-two-factor-authentication-otp_window-value) na versão 2.4.7. A descrição do campo **[!UICONTROL OTP Window]** agora fornece uma explicação precisa da configuração e o valor padrão foi alterado de `1` para `29`.
