...
* O comando `bin/magento config:set graphql/session/disable 1` permite que os comerciantes desabilitem completamente a criação de cookies de sessão para todas as operações do GraphQL. Por padrão, o Adobe Commerce cria esses cookies e depende deles para autorização, o que afeta o desempenho. A partir de agora, recomendamos usar tokens como a única forma de autorização para solicitações do GraphQL. Não recomendamos usar cookies de sessão sozinhos ou em conjunto com tokens de autorização. Consulte [Autorização do GraphQL](https://devdocs.magento.com/guides/v2.4/graphql/authorization-tokens.html). <!-- B2B-2204 -->
