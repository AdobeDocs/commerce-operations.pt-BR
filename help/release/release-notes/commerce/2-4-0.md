...
>[!NOTE]
>
>**integração de pagamento Braintree**: antes da versão 2.4.0, era recomendável que os comerciantes instalassem e configurassem a extensão de integração de pagamento Braintree oficial do Commerce Marketplace para substituir a integração principal. Com esta versão (2.4.0), a extensão agora está incluída na versão. Os comerciantes devem seguir etapas adicionais para garantir que a Braintree funcione adequadamente em uma implantação 2.4.0. Consulte [Braintree](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/payments/braintree) para obter mais informações sobre como migrar para a 2.4.0.

Esta versão inclui todas as melhorias na qualidade principal incluída no 2.3.5-p1, mais de 100 novas correções no código principal e 30 melhorias de segurança. Inclui a resolução de 226 problemas do GitHub por nossos membros do comunidade. Essas comunidade contribuições variam de uma pequena limpeza do código principal a aprimoramentos significativos no Gerenciamento de inventário e no GraphQL.

>[!NOTE]
>
>Versões secundárias trazem melhorias substanciais no código. Antes de atualizar para a versão 2.4.0, confirme que sua ambiente atende aos requisitos mínimos [](../../../installation/system-requirements.md)da pilha técnica.

{{bics}}

>[!NOTE]
>
>Os nomes dos pacotes das versões de segurança normalmente são anexados com -p1. No entanto, não foi possível evitar o desvio dessas convenções de nomenclatura com a versão 2.3.5, que, por sua vez, teve um efeito de cascata temporário nos nomes subsequentes do pacote de segurança. Especificamente, o recurso completo versão 2.3.5 é 2.3.5-p1. A versão de segurança que lançaremos quando a versão 2.4.0 do GA for 2.3.5-p2. Esperamos retornar às convenções de nomenclatura habituais em versões futuras.
...
Os comerciantes agora podem instalar correções de segurança sensíveis ao tempo sem aplicar as centenas de correções e aprimoramentos funcionais que uma versão trimestral completa (por exemplo, 2.3.5-p2) fornece. O patch 2.3.5.2 (Pacote do Composer 2.3.5-p2) é um correção de segurança que fornece correções para vulnerabilidades identificadas em nossa versão trimestral anterior, 2.3.5-p1. Todos os hotfixes aplicados à versão 2.3.5 estão incluídos nesta correção de segurança. (Um *hotfix* fornece uma correção para uma versão lançada que aborda um problema ou bug específico.)

Para obter informações gerais sobre patches de segurança, consulte [Introdução à versão Novo de correção de segurança](https://community.magento.com/t5/Magento-DevBlog/Introducing-the-New-Security-Patch-Release/ba-p/141287). Para obter instruções sobre como baixar e aplicar patches de segurança (incluindo correção 2.3.5-p2), consulte [Quick start instalação no local](../../../installation/composer.md). Os patches de segurança incluem apenas correções de bugs de segurança, não as melhorias de segurança adicionais incluídas no patch completo.
...
* **O modo estrito do filtro de modelo agora está habilitado por padrão**. Componentes (incluindo páginas e blocos CMS) que usam o filtro de modelo no modo legado podem ser vulneráveis à execução do código remoto (RCE).Ativar o modo restrito por padrão garante que os ataques RCE não possam ser deliberadamente ativados.<!-- MC-22982-->
...
* **Suporte para security.txt arquivo**. Esse arquivo é um arquivo padrão do setor no servidor que ajuda pesquisadores de segurança a relatar possíveis problemas de segurança para os administradores do site.

* **Melhorias no suporte à política de segurança de conteúdo (CSP).** `SecureHtmlRenderer` foi adicionado à Estrutura e está disponível em `.phtml` modelos para a lista de permissões em linha `style` e `script` tags. Scripts e estilos incorporados não são permitidos com a configuração CSP padrão, que pode ser substituída por arquivos XML. <!-- MC-34433-->

{{cve-notice}}

### Atualizações de Platform

As atualizações de plataforma a seguir ajudam a melhorar a segurança e o desempenho do site. Versões compatíveis do PHP e PHPUnit, Elasticsearch, MySQL e outras dependências estão listadas em [requisitos de pilha de tecnologia](../../../installation/system-requirements.md).
...
* O aplicativo não lança mais um erro quando você exclui uma imagem de uma página ao criá-la de **Administrador** > **Conteúdo** > **Páginas** >  **Adicionar nova página**. Anteriormente, o aplicativo exibia este erro no console do desenvolvedor: `500 (Internal Server Error)`. _Correção enviada por Eden Duong em obter solicitação [25924](https://github.com/magento/magento2/pull/25924)_. [GitHub-25893](https://github.com/magento/magento2/issues/25893)

### Produtos do pacote

<!-- MC-25251-->

* O desempenho da `catalog_product_price` operação de reindexação para produtos pacote foi aprimorado.

<!-- MC-29633-->

* Os administradores não podem mais inserir manualmente uma classe tributária no Admin para um produto pacote quando as configurações de Classe **Fiscal e** Preço **dinâmico do** produto pacote são desabilitadas pela armazenamento padrão visualização. Anteriormente, quando um administrador desmarcava a opção **Usar padrão Valor** ao **lado da Classe** Fiscal, o aplicativo permitia a opção, permitindo que um administrador inserisse outro valor e salvasse o produto.

<!-- MC-30257-->

* Os preços dos produtos em conjunto agora são calculados corretamente nas páginas dos produtos.

<!-- MC-29745-->

* Agora é possível adicionar vários pacote produtos ao seu carrinho de compras sem erro. Anteriormente, quando você adicionava um produto pacote ao seu carrinho e navegava para o carrinho, o aplicativo exibia esse erro: `Please correct the quantity for some products`.

### Cache

<!-- ENGCOM-7114-->

* O `cache.xsd` schema foi refatorizado para que um tipo de `Magento\Framework\Cache` cache sem um `instance` atributo não cause mais exceções quando você desativar um módulo por meio do gerenciamento de cache do Admin. Anteriormente, quando você criava um tipo de cache usando `cache.xml` sem o `instance` argumento, o aplicativo lançou uma exceção quando você tentou desativá-lo.  _Correção enviada por Andrii Beziazychnyi em obter solicitação [27307](https://github.com/magento/magento2/pull/27307)_. [GitHub-26224](https://github.com/magento/magento2/issues/26224)
...
* O desalinhamento corrigido do **My faturamento e endereço de entrega é a mesma** caixa de seleção na seção de página Revisão e Pagamentos do fluxo de Trabalho de checkout.  _Correção enviada por divyajyothi5321 em obter solicitação [26169](https://github.com/magento/magento2/pull/26169)_. [GitHub-26168](https://github.com/magento/magento2/issues/26168)

<!-- ENGCOM-6581-->

* Remoção de espaço em branco desnecessário entre os rótulos de campo e os asteriscos na seção Endereço de envio da fluxo de Trabalho de check-out.  _Correção enviada por Daniel Ruf em obter solicitação [26285](https://github.com/magento/magento2/pull/26285)_. [GitHub-26275](https://github.com/magento/magento2/issues/26275)

<!-- ENGCOM-6480-->

* Correção de desalinhamento dos botões de opção na seção de métodos de envio dos fluxo de Trabalho de checkout.  _Correção enviada pelo Hitesh na solicitação de pull [25966](https://github.com/magento/magento2/pull/25966)_. [GitHub-25962](https://github.com/magento/magento2/issues/25962)
...
* Correção do desalinhamento dos ícones mini-carrinho Editar e Remover em visualização móveis.  _Correção enviada por Vasilii Burlacu em obter solicitação [27493](https://github.com/magento/magento2/pull/27493)_. [GitHub-26652](https://github.com/magento/magento2/issues/26652)

<!-- ENGCOM-6967-->

* Correção da desalinhamento da taxa de imposto cep/postagem intervalo de código e caixa de seleção na caixa de seleção Adicionar taxa de imposto Novo/CEP Code página (**Administração**  >  **Store**  >  **Impostos** >  **Zonas e Taxas** Fiscais). _Correção enviada pelo Sathish Subramanian em obter solicitação [26932](https://github.com/magento/magento2/pull/26932)_. [GitHub-26917](https://github.com/magento/magento2/issues/26917)

<!-- ENGCOM-7237-->

* O link do Assistente de configuração para o Administrador agora funciona conforme o esperado. _Correção enviada por Oleh Usik em obter solicitação [26100](https://github.com/magento/magento2/pull/26100)_. [GitHub-24990](https://github.com/magento/magento2/issues/24990)

<!-- ENGCOM-6531-->

* A classificação foi desabilitada na coluna de status da grade Gerenciamento de cache no Administrador. _Correção enviada pelo Subramaniano Sathish em obter solicitação [26215](https://github.com/magento/magento2/pull/26215)_. [GitHub-26208](https://github.com/magento/magento2/issues/26208)

<!-- ENGCOM-7015-->

* Correção da desalinhamento de elementos de página na **caixa de diálogo Admin** >  **Stores**  >  **Attributes**  > **Avaliações** >  **Adicionar nova caixa de diálogo de classificações** . _Correção enviada pelo Sathish Subramanian em obter solicitação [27014](https://github.com/magento/magento2/pull/27014)_. [GitHub-26992](https://github.com/magento/magento2/issues/26992)

<!-- ENGCOM-6485-->

* Correção da desalinhamento do ícone de calendário na página Adicionar alteração de design. _Correção enviada por Eduard Chitoraga em obter solicitação [26063](https://github.com/magento/magento2/pull/26063)_. [GitHub-20379](https://github.com/magento/magento2/issues/20379)
...
* As dependências no Zend Framework foram migradas para o [projeto Laminas](https://getlaminas.org/about/foundation) para refletir a transição do Zend Framework para o projeto Laminas da Fundação Linux. O Zend Framework foi descontinuado. _Correção enviada por Ihor Sviziev em obter solicitação [26436](https://github.com/magento/magento2/pull/26436)_. [GitHub-26335](https://github.com/magento/magento2/issues/26335)

<!-- MC-25257-->

* As configurações especiais de faixa de preço (de/a datas) agora funcionam corretamente para contas de administrador que usam uma localidade holandesa.

<!-- MC-29388-->

* `php bin/magento cron:run` não processa mais itens a partir da tabela de log de alterações várias vezes. Anteriormente, quando você tinha mais de 100000 novas versões na tabela de log de alterações, as ações podiam ser chamadas várias vezes para o mesmo `entity id`.

* O `php bin/magento setup:cron:run` comando foi removido. Além disso, remova as referências a esse arquivo nos crontabs ao atualizar.
...
* O módulo do cliente não tem mais dependência do Review módulo. Anteriormente, não era possível desativar o módulo de revisão devido a essa dependência.
...
* A aplicativo agora renderiza o **atributo Sim/Não** no Categoria página quando **Usar na Navegação em camadas: Filtre (com resultados)** para propriedades de vitrine.

<!-- ENGCOM-6807-->

* Redundante `\Magento\Sales\Model\Order\Email\Sender\ShipmentSender` foi obsoleto em favor de  `\Magento\Sales\Model\Order\Shipment\Sender\EmailSender`. _Correção enviada por Adarsh Manickam em obter solicitação [26714](https://github.com/magento/magento2/pull/26714)_. [GitHub-14885](https://github.com/magento/magento2/issues/14885)

### Log

<!-- MC-29636-->

* As alterações de status do pedido agora são registradas como esperado **nos Logs** de ação do Sistema **>**> **Relatório**.

<!-- MC-29637-->

* A aplicativo agora registra ações de envio de vendas no log Admin Action, conforme esperado.

<!-- MC-29892-->

* As ações de salvamento página CMS agora estão registradas nos Logs de ação administrativa. Anteriormente, apenas visualização ações eram registradas.

<!-- MC-30618-->

* Salvar ações nas páginas cms agora são registradas como esperado nos logs de ação do administrador quando **Selecionar todas as ações a serem registradas** está ativada na guia de registro de ações do administrador (**Admin** > **Stores** > **Configuration** > **Avançado**).

<!-- ENGCOM-7274-->

* As modificações página CMS agora são registradas conforme esperado no log de Ação. Agora, o MFTF cobre esse recurso.  _Correção enviada por Lukasz Bajsarowicz em obter solicitação [27597](https://github.com/magento/magento2/pull/27597)_. [GitHub-171](https://github.com/magento/magento2/issues/171)
...
* Agora é possível criar uma subpasta na Galeria de mídia que tem o mesmo nome da pasta primária. Anteriormente, não era possível salvar a subpasta com o mesmo nome. _Correção enviada por Vadim Malesh em obter solicitação [27976](https://github.com/magento/magento2/pull/27976)_. [GitHub-1270](https://github.com/magento/adobe-stock-integration/issues/1270)

### Armazenamento de mídia

<!-- MC-32593-->

* `var/resource_config.json` não é mais regenerado sempre que uma imagem é solicitada por `get.php`. Anteriormente, esse arquivo era reescrito em cada chamada para `get.php`.
...
A tabela a seguir destaca as contribuições feitas pelos parceiros. Esta tabela lista o Parceiro que contribuiu com a obter solicitação, o obter externo solicitação e o número de edição do GitHub associado a ele (se disponível).

{{$include /help/_includes/release-notes/engcomm-2-4-0-partner.md}}

### Contribuições colaborador individuais

A tabela a seguir identifica as contribuições de nossos membros comunidade. Esta tabela lista as solicitações de obter externas, o número de edição do GitHub associado a ela (se disponível) e o membro comunidade que contribuiu com o obter solicitação.
