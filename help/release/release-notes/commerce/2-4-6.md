---
title: Adobe Systems Comércio notas de versão 2.4.6
description: Saiba mais sobre novos recursos, melhorias, correções de erros e problemas conhecidos na versão 2.4.6 Adobe Systems Comércio.
...
# Adobe Systems Comércio notas de versão 2.4.6

Adobe Systems Comércio 2.4.6 apresenta suporte para PHP 8.2. O PHP 8.1 continua completamente compatível, mas o suporte para PHP 7.4 foi removido. Inclui aprimoramentos significativos de desempenho e escalabilidade. As operações graphQL para operações de carrinho em massa e categoria resposta de renderização de árvore foram otimizadas. Esta versão apresenta a Adobe Systems Comércio metapackage de Extensão.
...
Embora o código para esses recursos seja agrupado com versões do Adobe Systems Comércio código principal, vários desses projetos (por exemplo, B2B, Page Builder e Progressive Web Applications (PWA) Studio) também são lançados de forma independente. As correções de erros desses projetos estão documentadas nas informações de versão separadas e específicas do projeto, disponíveis na documentação de cada projeto.
...
* As bibliotecas JavaScript desatualizadas foram atualizadas para suas versões mais recentes e as dependências desatualizadas foram removidas. Essas alterações têm compatibilidade retroativa. <!-- AC-2036-->
...
* **Desempenho de processamento de ordem de alta taxa de transferência aprimorado por meio do balanceamento de carga**. O Adobe Commerce 2.4.6 em clientes de infraestrutura em nuvem que têm um alto volume de pedidos simultâneos (1000 pedidos por minuto) agora pode melhorar o desempenho do processamento de pedidos, equilibrando a carga em seu banco de dados em diferentes nós. Os comerciantes podem reequilibrar essa carga habilitando conexões secundárias para o banco de dados MySQL e instâncias Redis. Consulte [Processamento de pedido de alta produtividade](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/high-throughput-order-processing.html#load-balancing) <!-- ACPT-495 -->


### Metappackage da extensão do Adobe Commerce

Esta versão apresenta o Adobe Systems Comércio Extension metapackage v1.1.0, que automaticamente compila extensões de Adobe Systems Comércio selecionadas com esta versão principal. As versões dessas extensões incluídas neste metapackage são instaladas quando `composer update` são executadas, simplificando o processo de atualização dessas extensões ao atualizar para a versão principal mais recente. Essas extensões ainda manterão agendamentos de versões independentes.

O Adobe Systems Comércio Extension metapackage para Adobe Systems Comércio 2.4.6 inclui estas duas extensões:

* [Check-out rápido do Bolt](https://experienceleague.adobe.com/docs/commerce/quick-checkout/overview.html)
* [Eventos de E/S Adobe Systems para Adobe Systems Comércio](https://developer.adobe.com/commerce/events/get-started/)

As versões futuras desta metapackage de extensão conterão extensões adicionais.

### Adobe I/O Events para Adobe Commerce

O Adobe I/O Events for Adobe Commerce captura eventos do Commerce e transfere dados do Adobe Commerce para outros sistemas Adobe.Os desenvolvedores podem build aplicativos nativo nuvem no Adobe Systems Developer aplicativo Builder que se inscrevem em eventos de Comércio como atualizações de inventário, alterações de status solicitar e atualizações de preços. Os desenvolvedores podem usar esse recurso para trocar dados para a orquestração de toda a comprador jornada através de seus sistemas de crítico missão.

### Atualizações de acessibilidade

A focalizar desta versão foi criar uma vitrine experiência na Venia (PWA) mais perceptível, operável, compreensível e robusta. Esses aprimoramentos incluem:
...
* Agora, somente usuários com teclado podem acessar todas as página funcionalidade na vitrine. Anteriormente, os compradores não podiam acessar links navegação sub-navegação usando apenas pressionamentos de tecla.


### integração Adobe Systems Comércio com o IMS Adobe Systems
...
O novo `adobe-ims-metapackage` v2.2.0 vem com o Adobe Commerce 2.4.6. Nenhuma funcionalidade foi alterada nesta versão.<!-- CABPI-505-->
...
* O esquema GraphQL de ordem de compra abrange a funcionalidade Comprar Order Management de usuário de empresa B2B, como criar, editar e excluir ordens de compra. Consulte [Ordens de compra (B2B)](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/purchase-order/).
...
**query do cliente agora oferece suporte solicitar classificação**. O `orders` objeto de saída do `customer` query agora contém um campo de classificação. Agora é possível classificar pedidos por solicitar número ou data de criação e em solicitar crescente ou decrescente. <!-- AC-6903-->
...
PWA Studio v.13.0.x é compatível com o Adobe Systems Comércio 2.4.6. Ele inclui vários aprimoramentos para melhorar a acessibilidade. Para obter informações sobre correções de bugs, consulte [PWA Studio versões](https://github.com/magento/pwa-studio/releases). Consulte [Compatibilidade de versão](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) para obter uma lista de versões do PWA Studio e suas versões principais compatíveis do Adobe Commerce.
...
* A grade de produtos não lança mais uma exceção quando pacote produtos têm um preço especial que excede 1000. Anteriormente, ao pesquisar o produto pelo SKU ou navegar até **[!UICONTROL Catalog]** > **[!UICONTROL Products]**, a Adobe Commerce exibia este erro: `Non Numeric value encountered`. [GitHub-36284](https://github.com/magento/magento2/issues/36284))

<!-- ACP2E-1220-->

* As colunas em PDFs de memorando de crédito e fatura para pedidos que contêm uma combinação de pacote e produtos simples agora são renderizadas corretamente. Anteriormente, as colunas estavam distorcidas ou desalinhadas.
...
* O mini carrinho agora exibe as imagens corretas do produto após a execução `bin/magento catalog:image:resize`. Anteriormente, todas as imagens eram substituídas pela imagem padrão de espaço reservado do produto. [GitHub-35535](https://github.com/magento/magento2/issues/35535)

<!-- AC-2376-->

* O check-out pode ser concluído com sucesso quando o endereço de entrega contém um atributo de extensão. Anteriormente, Adobe Systems Comércio jogou este erro: `Object of class Magento\Quote\Api\Data\AddressExtension could not be converted to string`. [GitHub-34202](https://github.com/magento/magento2/issues/34202)
...
* A `setGiftOptionsOnCart` mutação agora requer autorização do cliente para operações de carrinho do cliente.

### Preço do carrinho regra

<!-- AC-2242-->

* O preço do carrinho regra grátis opções de envio definidas durante regra criação agora são aplicadas aos pedidos conforme o esperado. Anteriormente, as opções de frete gratuito aplicadas durante a regra criada eram ignoradas durante o check-out e o frete gratuito não era permitido quando a regra era aplicada. [GitHub-35013](https://github.com/magento/magento2/issues/35013)
...
* O preço do catálogo regra descontos não são mais aplicados às opções de produtos personalizados de preço fixo, mas são aplicados apenas ao preço original do produto.

<!-- ACP2E-1027-->

* Os produtos relacionados agora são obtidos com êxito nos detalhes do produto de vitrine página quando gerados pelo produto relacionado regra para atributos de produtos alfanuméricos. [GitHub-36285](https://github.com/magento/magento2/issues/36285)

<!-- AC-6810-->

* As regras de preço do catálogo agora levam em conta o escopo selecionado e são aplicadas a produtos apenas para os sites para os quais o atributo tem um valor de opção que corresponde ao regra. Anteriormente, o regra não aproveitava conta o escopo selecionado em implantações de vários sites. [GitHub-36049](https://github.com/magento/magento2/issues/36049)
...
* Os clientes agora são notificados sobre a queda do preço do produto quando o cliente está inscrito na queda de preço alerta. Anteriormente, as notificações de queda de preço nem sempre eram enviadas devido a aplicativo nível armazenamento em cache.

<!-- ACP2E-1348-->

* As regras de catálogo baseadas em SKUs com a **[!UICONTROL is not one of]** condição agora funcionam como esperado.

<!-- AC-729-->

* Adobe Systems Comércio não assume mais erroneamente que um categoria é o categoria principal de outro com base em categoria caminho. Anteriormente, quando duas categorias eram adjacentes e uma categoria ID começa com a ID categoria de outra categoria, Adobe Systems Comércio assumia que as categorias tinham o mesmo categoria pai. [GitHub-32464](https://github.com/magento/magento2/issues/32464)
...
* As opções de imagem não são mais exibidas como marcação HTML no aviso de crédito, confirmação de pedido e emails de cliente de fatura (modelo padrão) para produtos com opções personalizadas.
...
* O método `getFilteredTimerIds` agora funciona conforme esperado.  A classe `Magento\Framework\Profiler\Driver\Standard\Stat` agora é compatível com o PHP 8.1. Anteriormente, o Adobe Commerce lançava uma exceção quando esse método era chamado. [GitHub-35307](https://github.com/magento/magento2/issues/35307)
...
* O Adobe Commerce agora exibe um intervalo de tempo preciso quando um usuário administrador seleciona a opção **[!UICONTROL Last 24 Hours]** no painel Estatísticas do administrador. Anteriormente, o Adobe Commerce exibia um intervalo de tempo incorreto. Isso ocorreu porque o fuso horário do banco de dados permaneceu inalterado.
...
* O **[!UICONTROL Grid Filter Condition Type]** campo dos atributos de endereço do cliente e do cliente não está mais visível na edição do atributo página quando a **[!UICONTROL Use in Filter Option]configuração s** é desativada.

<!-- AC-6020-->

* Adobe Systems Comércio agora exibe uma mensagem de erro informativa se um comerciante não inserir um valor de atributo SKU opcional ao criar um produto. Anteriormente, o aplicativo lançou essa exceção quando o atributo era opcional: `Deprecated Functionality: trim(): Passing null to parameter #1 ($string) of type string is deprecated in /var/www/html/mg244/project-community-edition/vendor/magento/module-eav/Model/Entity/AbstractEntity.php on line 932`. [GitHub-35810](https://github.com/magento/magento2/issues/35810)

<!-- ACP2E-716-->

* O **[!UICONTROL Show my quotes]** filtro no **[!UICONTROL My Quotes]** página agora funciona como esperado. Anteriormente, os resultados do filtro incluíam outros administrador cotações de usuários.
...
* O preço de quebra de presente agora é exibido conforme esperado no bloco de resumo do pedido quando a quebra de presente é aplicada no nível do pedido a um pedido que contém produtos físicos e virtuais. Anteriormente, Adobe Systems Comércio não exibia o preço do invólucro do presente.
...
* A `products` query retorna categoria informações conforme esperado, quando o prefixo do banco de dados tem um valor não nulo. Anteriormente, retornava uma `Internal server error`. [GitHub-36117](https://github.com/magento/magento2/issues/36117)

<!-- AC-1003-->

* Corrigiu o tipo de retorno do `getCustomerIsGuest` método. [GitHub-29529](https://github.com/magento/magento2/issues/29529)

<!-- B2B-2221-->

* O `Magento\CatalogGraphQl\Plugin\DesignLoader` plug-in como refatorado.
...
* Adição do `gift_message` tipo ao `OrderItemInterface`. [GitHub-29418](https://github.com/magento/magento2/issues/29418)

<!-- AC-2454-->

* As opções de produto retornadas nas respostas do GraphQL agora incluem apenas os produtos que estão no catálogo ao qual a usuário pertence. Se a usuário pertence a um catálogo compartilhado, somente produtos em catálogos compartilhados são retornados. Se usuário é um convidado ou não faz parte de um catálogo compartilhado personalizado, somente os produtos no catálogo público são retornados.
...
* Atualização das recomendações sobre o tamanho do logotipo para a guia de fatura do PDF.
...
* Adição de limpeza e `DATETIME` valores em dumps de `DATE` banco de dados para Adobe Systems suporte Comércio. Anteriormente, os campos do tipo datetime não eram limpos para tabelas de clientes/cotações/pedidos na tabela de backup do banco de dados.
...
* A Galeria de mídia herdada agora está desativada por padrão. Anteriormente, **[!UICONTROL Enable old Media gallery]** estava definido como **[!UICONTROL Yes]**.
...
* Adicionar um comentário a um solicitar arquivado não o retorna mais à grade de ordens desarquivadas enquanto mantém seu status arquivado.

<!-- ACP2E-860-->

* Os valores de atributos da empresa agora são exibidos conforme esperado nas seções de faturamento e endereço de entrega do fluxo de Trabalho de checkout. Anteriormente, empresa atributos não eram exibidos corretamente no detalhe solicitar página porque as lojas não eram definidas corretamente durante a criação solicitar. Os valores de atributos eram armazenados corretamente somente quando o **Exibir Company** propriedade foi definido para ****[!UICONTROL optional]**o Site** principal em implantações multi-armazenamento.
...
* As informações de pagamento do Vault não são mais mostradas na seção Pagamento do Administrador solicitar criação fluxo de Trabalho se o consumidor não tiver salvo um token. [GitHub-33954](https://github.com/magento/magento2/issues/33954)

<!-- AC-2688-->

* Os preços personalizados não dobram mais depois que um desconto é aplicado por carrinho regra de preço. [GitHub-35154](https://github.com/magento/magento2/issues/35154)

<!-- ACP2E-1120-->

* Novo solicitar status pendente agora é cumprido conforme esperado quando um solicitar é colocado usando o método de pagamento Zero Subtotal Checkout.

<!-- ACP2E-1121-->

* As configurações de email de convidados agora são respeitadas durante a criação solicitar para um consumidor de hóspedes com o PayPal Express via GraphQL. Anteriormente, o conjunto de endereços de email era substituído pelo endereço de email definido na PayPal conta.
...
* A consulta `customerPaymentTokens` não retorna mais tokens do Braintree quando o cofre é desativado no Administrador.
...
* Os usuários administradores agora podem criar um pedido ou um novo pedido quando o método de pagamento do Braintree está ativado. Anteriormente, o Adobe Commerce exibia este erro: `report.CRITICAL: Error: Call to a member function getMethodInstance() on null in /app/vendor/paypal/module-braintree-core/Block/Form.php:174`.

<!-- BUNDLE-3139-->

* O check-out usando o método de pagamento Braintree agora é concluído com êxito quando o Braintree PayPal é habilitado e o **[!UICONTROL Enable Card Payments]** é desabilitado. Anteriormente, a barra de progresso nunca era resolvida, e o Adobe Commerce exibia este erro de console: `adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66)`. [GitHub-35961](https://github.com/magento/magento2/issues/35961)
...
* A consulta `customerPaymentTokens` não retorna mais tokens do Braintree quando o cofre é desativado do Administrador.

<!-- BUNDLE-3137-->

* Os usuários administradores agora podem criar um pedido ou um novo pedido para clientes quando o método de pagamento do Braintree está ativado. Anteriormente, o Adobe Commerce exibia este erro: `report.CRITICAL: Error: Call to a member function getMethodInstance() on null in /app/vendor/paypal/module-braintree-core/Block/Form.php:174`

<!-- BUNDLE-3139-->

* Os compradores agora podem usar o método de pagamento Braintree em vitrines nas quais **[!UICONTROL Enable Card Payments]** foi desabilitado. Anteriormente, o Adobe Commerce exibia este erro do JavaScript: `_adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66).`
...
* O desempenho do `bin/magento/setup:upgrade` comando foi aprimorado. Anteriormente, esse comando recriava os acionadores do banco de dados, o que causava um tempo de inatividade significativo durante a execução do comando. Além disso, os indexadores não voltaram ao modo programado e permaneceram no modo de salvamento. [GitHub-33386](https://github.com/magento/magento2/issues/33386)

<!-- ACP2E-1016-->

* A atualização dos preços de nível por API agora é executada com mais rapidez. Anteriormente, a atualização dos preços em nível gerava consultas desnecessárias `SELECT` , o que atrasou a execução quando muitos grupos de clientes eram atribuídos a um produto.

<!-- ACP2E-1307-->

* O carregamento de páginas que incluem o lista de produtos de widget de catálogo foi otimizado. Anteriormente, os atributos não globais eram carregados diretamente do banco de dados.

### Relatórios

<!-- ACP2E-852-->

* O relatório Log de administração agora mostra um valor **[!UICONTROL Value before change]** preciso quando uma regra de categoria é atualizada.
...
* A constante `ANDROID_PAY_CARD` em `Braintree/Ui/Component/Report/Listing/Column/PaymentType.php` substituiu `GOOGLE_PAY_CARD`. Isso soluciona problemas com o relatório do Braintree.

### Autorizações de devolução de produto (RMA)

<!-- ACP2E-1285-->

* O campo de comentário RMA (**[!UICONTROL Orders]** > **[!UICONTROL Return]**) não contém mais espaços em branco redundantes. (As marcas `<textarea>` e `<?php` agora estão em uma linha.

<!-- ACP2E-1279-->

* A caixa de seleção **[!UICONTROL Use Config Settings]** do campo **[!UICONTROL Enable RMA]** agora salva seu valor conforme esperado quando um produto é salvo.
...
* As pesquisas de cadeias de caracteres entre aspas agora funcionam como esperado quando a configuração **[!UICONTROL Minimum Terms to Match]** está habilitada. Anteriormente, o Adobe Commerce exibia este erro: `{"error":{"root_cause":[{"type":"parsing_exception","reason":"[match_phrase] query does not support [minimum_should_match]' ...`.
...
* Adicionados os `price_excl_tax` tipos e `price_excl_tax` à saída da `setShippingMethodsOnCart` mutação. [GitHub-31206](https://github.com/magento/magento2/issues/31206)

<!-- AC-1096-->

* As IDs de frete exclusivas agora são passadas nas respostas do GraphQL conforme esperado, com detalhes de envio para pedidos de envio para vários endereços para consultas GraphQL.

### Preparo

<!-- ACP2E-879-->

* O agendamento da data final de uma atualização de preparo não resulta mais em um erro após a atualização ser concluída. Anteriormente, os administradores não podiam abrir páginas de produtos e grades de produtos, e Adobe Systems Comércio jogou este erro: `report.CRITICAL: Exception: Item (Magento\Catalog\Model\Product\Interceptor) with the same ID "1" already exists`.

<!-- ACP2E-776-->

* O painel de preparo agora exibe o número total correto de registros, e painel paginação agora funciona conforme o esperado. Anteriormente. o painel exibiu no máximo 20 registros.

<!-- ACP2E-1213-->

* As visualizações de preparo de conteúdo agora carregam mais rápido em implantações com muitas lojas. Visualização reindexação agora indexa novamente apenas as lojas selecionadas.

<!-- ACP2E-919-->

* Agora, os produtos podem ser adicionados ao carrinho de compras no modo de agendamento pré-visualização. Atualização de links de navegação estrutural no modo de agendamento pré-visualização com estes parâmetros corrigiu este problema: `version`, `timestamp`e `signature`.
...
* A atribuição automática de grupo de clientes agora funciona quando não há ID de IVA associada ao endereço do cliente. Se o endereço não contiver uma ID de IVA, ele será tratado como pertencente a um cliente não registrado para IVA e a Adobe Commerce alterará o grupo de clientes para um grupo não-IVA. Anteriormente, quando **[!UICONTROL Enable Automatic Assignment to Customer Group]** era habilitado, a conta do cliente não era automaticamente atribuída ao `customerGroup` quando a ID de IVA era excluída da conta do cliente.
...
* As visualizações de e-mail agora abrem em uma guia separada em vez de uma janela pop-up, o que fornece uma visualização mais ampla dos conteúdo de email. Anteriormente, os conteúdo de email eram truncados.

<!-- ACP2E-1299-->

* A função de filtro da grade de função da usuário ( **[!UICONTROL System]** Administrador > **[!UICONTROL Permissions]** > **[!UICONTROL User roles]** > **[!UICONTROL Administrators]**) agora funciona como esperado.O valor da matriz agora é tratado separadamente. Anteriormente, quando você tentava inserir uma query nesta grade, Adobe Systems Comércio jogaram este erro de console, e o página não carregou: `Uncaught TypeError: Cannot read properties of null`. [GitHub-35993](https://github.com/magento/magento2/issues/35993)

<!-- ACP2E-328-->

* A validação para valores de atributos foi adicionada ao criar um cliente por meio da REST API. O processo de criação agora falha quando valores de atributos inválidos são fornecidos. Anteriormente, não havia validação para gênero, que não é um campo obrigatório. Durante a criação do cliente, o valor passado para o gênero foi salvo na `customer_entity` tabela.
...
* Alterar a grade de colunas da grade de produtos visualização e selecionar novas colunas para uma visualização personalizada não aciona mais uma ação Selecionar todas as colunas de grade. Anteriormente, a caixa de coluna desaparecia e os produtos da grade eram selecionados ou desmarcados. [GitHub-36302](https://github.com/magento/magento2/issues/36302)

<!-- ACP2E-1245 1405-->

* Agora, os usuários administradores podem limpar o log de tarefas de administração concluídas com sucesso. Anteriormente, o administrador exibia esse erro: `Something went wrong`. [GitHub-36255](https://github.com/magento/magento2/issues/36255)
...
* O serviço de moeda Fixer agora funciona conforme esperado com chaves geradas via ApiLayer e as taxas de moeda são exportadas corretamente. Anteriormente, Adobe Systems Comércio ocorreu um erro quando um comerciante tentou importar taxas de moeda através do Administrador.
...
Você pode instalar o Adobe Commerce 2.4.6 usando o [Composer](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
