...
O Adobe Commerce 2.4.1 apresenta aprimoramentos no desempenho e na segurança, além de adições significativas ao conjunto de recursos B2B. Os aprimoramentos de segurança incluem suporte para o `SameSite` para cookies e a adição da proteção CAPTCHA para pontos de extremidade de API relacionados ao pagamento e ao pedido e da página de loja Inserir pedido . As melhorias B2B focam no processo de aprovação de pedidos, nos métodos de envio B2B, no registro expandido das ações de Administração e na segurança aprimorada na loja.
...
* **CAPTCHA** A proteção foi adicionada às seguintes áreas de produtos:
   ...
   * Página Colocar loja de pedidos e pontos finais REST e GraphQL <!-- MC-36067-->
...
* **Integração da ferramenta de análise em todo o site com o Administrador**. O [ferramenta](https://docs.magento.com/user-guide/reports/site-wide-analysis-tool.html) fornece informações e instrumentação do sistema para instalações de infraestrutura em nuvem do Adobe com monitoramento de desempenho, relatórios e recomendações de autoatendimento 24 horas por dia, 7 dias por semana. Os comerciantes podem usar o novo Administrador [recurso de função](https://docs.magento.com/user-guide/system/permissions-role-resources.html) para acessar com segurança as páginas de detalhes do cliente pelo Administrador. Consulte a [Perguntas frequentes](https://support.magento.com/hc/en-us/articles/360048646671) para obter uma visão geral. <!-- SWAT-807-->
...
O Construtor de páginas agora é compatível com o modo de tela cheia, que oferece suporte para a edição mais fácil do conteúdo e fornece uma experiência consistente de edição do conteúdo no Administrador. Consulte [Workspace](https://docs.magento.com/user-guide/cms/page-builder-workspace.html). <!-- PB-543-->
...
* **Opções de presentes**. Todos os clientes e convidados podem adicionar uma mensagem de presente ao seu pedido. Nas instalações do Adobe Commerce, também é possível adicionar o encapsulamento de presente, os recibos de presente e os cartões impressos no pedido. Consulte [`setGiftOptionsOnCart` mutação](https://devdocs.magento.com/guides/v2.4/graphql/mutations/set-gift-options.html) e [`updateCartItems` mutação](https://devdocs.magento.com/guides/v2.4/graphql/mutations/update-cart-items.html) <!-- MC-32345-->

* **Pontos positivos**. Os clientes podem aplicar ou remover pontos de recompensa aos carrinhos. Eles também podem ver seu histórico do ponto de recompensa. Consulte [`applyRewardPointsToCart`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/apply-reward-points.html) e [`removeRewardPointsFromCart`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/remove-reward-points.html) para uma discussão sobre como gerenciar pontos de recompensa em um carrinho.<!-- MC-23366-->
...
* **Métodos de pagamento armazenados**. Os clientes conectados agora podem armazenar os detalhes do pagamento (incluindo o cartão de crédito do Braintree e o Braintree com PayPal) em Minha conta. <!-- MC-32348 35945 35946-->
...
* **Klarna GraphQL**. Tópicos adicionados ou atualizados no Klarna GraphQL em [Método de pagamento de Klarna](https://devdocs.magento.com/guides/v2.4/graphql/payment-methods/klarna.html) e [`createKlarnaPaymentsSession`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/create-klarna-payments-session.html)
...
* Várias melhorias na experiência da MyAccount (Minha conta) da loja Venia <!-- PWA-247-->
...
A versão 2.4.1 apresenta B2B v1.3.0. Essa versão inclui melhorias para aprovações de pedidos, métodos de envio, carrinho de compras e registro de ações de Administração.

#### Melhorias nas aprovações de pedido

As aprovações de pedidos B2B foram aprimoradas para melhorar a usabilidade e permitir ações em massa em ordens de compra.
As melhorias na aprovação e rejeição de pedidos incluem:

* **Nova página Exibir regra para usuários sem privilégios de edição**. Os compradores B2B agora podem exibir regras que se aplicam à empresa na nova página Exibir Regra quando não têm permissão para editá-las. <!-- BUNDLE-104 -->

* **Ícone de alerta Contagem na guia Exige minha aprovação**. A guia Exige minha aprovação na exibição Minhas ordens de compra agora exibe um contador que indica o número de ações de aprovação pendentes. <!-- BUNDLE-102 -->

* **Aprovações e rejeições de pedidos em massa**. Gerentes B2B e Administradores da empresa agora podem executar rejeição em massa e aprovação de pedidos de compra. Essas alterações permitem que os aprovadores aprovem ou rejeitem várias ordens de compra em uma única ação. <!-- BUNDLE-140 128 111 153 -->

* Os comerciantes agora podem pesquisar o **Aplica-se a** e **Requer aprovação de** campos da exibição Meus pedidos de compra e podem selecionar várias funções de usuário durante a criação da regra. <!-- BUNDLE-105 106-->

* Exemplos de como configurar regras de aprovação de pedido são fornecidos na página Configuração de regra . <!-- BUNDLE-103 -->

Consulte [Regras de aprovação](https://docs.magento.com/user-guide/customers/account-dashboard-approval-rules.html)

#### Melhorias nos métodos de envio B2B

Os comerciantes B2B agora podem controlar os métodos de entrega oferecidos a cada Empresa. Os comerciantes podem configurar o seguinte no Administrador:<!-- BUNDLE-160 161 162 -->

* Um conjunto específico de métodos de envio para contas de empresas B2B
* O uso de métodos de entrega específicos para Todos ou B2B para cada conta Empresa
* Uma lista específica de métodos de envio B2B para cada conta empresarial

#### Melhorias no carrinho de compras

* Os comerciantes agora podem permitir que os usuários limpem o conteúdo de seu carrinho de compras em uma única ação e podem configurar essa capacidade independentemente em cada site. <!-- BUNDLE-108 -->

* Os compradores B2B agora podem adicionar itens individuais ou todo o conteúdo do carrinho de compras diretamente a uma lista de requisições. <!-- BUNDLE-145 144-->

#### Novos recursos de administração

* Os comerciantes B2B podem criar ordens do Administrador em nome de clientes usando o Pagamento em Conta como método de pagamento. <!-- BUNDLE-166 178-->
* Os comerciantes agora podem visualizar diretamente todas as aspas associadas a um usuário na página de detalhes do cliente. <!-- BUNDLE-139 -->
* Os comerciantes agora podem filtrar a grade Clientes online por empresa. <!-- BUNDLE-137 -->
* Agora, os administradores podem filtrar os clientes na Admin por representante de vendas. <!-- BUNDLE-110 -->
...
Para reduzir a criação de contas fraudulentas ou de spam, os comerciantes agora podem habilitar o Google reCAPTCHA no formulário de solicitação da nova empresa na loja. Consulte [reCAPTCHA](https://docs.magento.com/user-guide/configuration/security/google-recaptcha-storefront.html).<!-- BUNDLE-154 -->

#### Registro expandido de ações do Administrador

As ações de administrador executadas nos módulos Empresa agora são registradas no Log de ações do administrador. As ações são registradas em todos os módulos relevantes da empresa: `Company`, `NegotiableQuote`, `CompanyCredit`, `SharedCatalog`.
...
* Agora você pode usar `bin/magento sampledata:deploy` para implantar dados de amostra conforme esperado após a instalação do Adobe Commerce usando o Composer. Anteriormente, o aplicativo exibia esse erro: `Git installations must deploy sample data from GitHub; see https://devdocs.magento.com/guides/v2.3/install-gde/install/sample-data-after-clone.html for more information`. _Correção apresentada por Andrii Beziazychnyi no pedido de pull [27481](https://github.com/magento/magento2/pull/27481)_. [GitHub-19481](https://github.com/magento/magento2/issues/19481)
...
* Em execução `bin/magento setup:upgrade` agora é concluída conforme esperado. Anteriormente, o aplicativo exibia o conteúdo impresso da matriz para caches. _Correção apresentada por Sathish Subramanian no pedido de pull [27567](https://github.com/magento/magento2/pull/27567)_. [GitHub-27091](https://github.com/magento/magento2/issues/27091)
...
* O **Usado em** A seção da página de detalhes da imagem da galeria do Adobe Stock agora identifica com precisão se a imagem está associada a um produto. _Correção apresentada por Nazar Klovanych no pedido de pull [28798](https://github.com/magento/magento2/pull/28798)_. [GitHub-1474](https://github.com/magento/adobe-stock-integration/issues/1474)
...
* O Amazon Pay agora preenche corretamente a variável `store name` em emails e outros locais exibidos. Se a variável **Nome da loja** na configuração do Amazon Pay está vazia, a extensão recupera o nome padrão do armazenamento (ou seja, o nome que você dá ao armazenamento no Administrador).
...
* Agora os administradores com as permissões corretas podem acessar Relatórios avançados e Relatórios de segmento.

<!-- MC-34352-->

* O Adobe Commerce gera com êxito arquivos de dados de relatório avançados e os envia conforme esperado para Inventário em implantações com bancos de dados divididos. Anteriormente, o aplicativo não gerava ou enviava o `quotes.csv` para Inventário e, como resultado, o Inventário não gerou os relatórios esperados.

### Braintree

* O Braintree agora envia o valor correto para o PayPal quando um código promocional é aplicado na página de check-out.

* O Apple Pay agora funciona como esperado quando os Termos e condições estão ativados na página de finalização.

* Os erros do navegador não ocorrem mais durante o check-out em dispositivos de desktop quando Venmo é ativado pelo Administrador.

* O check-out não falha mais quando o comprador insere caracteres especiais no **Nome** campos do fluxo de trabalho de check-out. Anteriormente, a autenticação falhava porque a API Braintree 3DS não suportava caracteres não ASCII.

* O aplicativo agora exibe o nome correto do recipient na seção de envio do fluxo de trabalho de check-out ao fazer um pedido usando PayPal.

* O aplicativo agora atualiza a página Revisão do pedido conforme esperado quando um comprador altera o método de envio mais de uma vez durante o check-out.
...
* A GraphQL agora oferece suporte para fazer um pedido de um produto de pacote com o tipo de opção `radio` e `dropdown` com várias opções. Anteriormente, o aplicativo exibia uma mensagem sobre entrada inválida para `BundleItem.type: radio/dropdown`. _Correção apresentada por Michał Derlatka no pedido de pull [29256](https://github.com/magento/magento2/pull/29256)_. [GitHub-26110](https://github.com/magento/magento2/issues/26110)
...
* O verniz não joga mais um `Connection reset by peer` erro quando um catálogo grande é reindexado na programação. _Correção apresentada por Matthew O&#39;Loughlin no pedido de pull [26256](https://github.com/magento/magento2/pull/8815)_. [GitHub-26255](https://github.com/magento/magento2/issues/8815)
...
* O aplicativo não gera mais um erro fatal ao salvar uma regra de catálogo com as seguintes condições: `If ALL of these conditions are FALSE:, If ALL of these conditions are TRUE:, Attribute set is default`
...
* Atualização do logotipo do Adobe Commerce e remoção de espaços irrelevantes do arquivo README. _Correção apresentada por Rafael Corr̻ êa Gomes em pedido de pull [28891](https://github.com/magento/magento2/pull/28891)_. [GitHub-29056](https://github.com/magento/magento2/issues/29056)
...
* O aplicativo agora gera um erro quando um comerciante cria uma página CMS com o mesmo URL da página Estrutura da empresa. Anteriormente, o aplicativo exibia a página CMS em vez da página Estrutura da empresa .
...
* A configuração do consumidor da fila de mensagens foi estendida com novos parâmetros que ajudam a controlar consumidores e salvar recursos do servidor e que podem diminuir o consumo da CPU na fila do consumidor. Consulte [Configurar filas de mensagem](https://developer.adobe.com/commerce/php/development/components/message-queues/configuration/) para uma descrição do `maxIdleTime`, `sleep`e `onlySpawnWhenMessageAvailable` parâmetros.
...
* Correção de problemas de alinhamento do texto explicativo sobre senhas e do **Cargo** na página Editar informações de conta do cliente .

<!-- MC-33645-->

* O CAPTCHA agora funciona conforme o esperado quando um novo cliente clica no **Criar uma conta** na página de registro do cliente da loja. Anteriormente, o aplicativo não criava a conta do cliente e exibia um erro quando o cliente clicava no botão.

<!-- MC-34024-->

* O fluxo de trabalho de check-out não exibe mais os valores personalizados do atributo de endereço do cliente quando o cliente não inseriu dados.
...
* As condições do segmento do cliente agora funcionam conforme esperado em uma implantação de banco de dados dividido. Anteriormente, o aplicativo exibia um erro ao tentar editar um segmento de cliente adicionando uma condição: `SQLSTATE[42S02]: Base table or view not found`.
...
* Os emails de notificação enviados para representantes de vendas atribuídos a uma empresa agora incluem o logotipo corporativo atribuído. Anteriormente, o email de notificação incluía o logotipo padrão LUMA, não o email de logotipo corporativo carregado.
...
* Os compradores agora podem alterar o número de pedidos exibidos por página quando a lista de Pedidos abrange várias páginas. Anteriormente, o aplicativo exibia essa mensagem quando você navegava até a última página de pedidos e tentava alterar o número de pedidos exibidos por página: `You have placed no orders`. Esse foi um problema conhecido para a versão 2.4.0.
...
* O aplicativo não exibe mais uma página CMS na hierarquia do site se a página estiver atribuída a várias visualizações de loja.
...
* As contas de cartão-presente agora capturam os números de pedido conforme esperado. Anteriormente, o **Mais informações** na guia Histórico da conta de presente selecionada não exibia IDs de pedido.

<!-- MC-34519-->

* Usar uma vírgula como separador decimal agora funciona conforme o esperado. Anteriormente, o separador de vírgulas ignorava valores decimais.

<!-- MC-34469-->

* Os cartões-presente agora são exibidos conforme esperado no minicarrinho. Anteriormente, o aplicativo renderizava objetos HTML como texto.

<!-- MC-33851-->

* Os memorandos de crédito agora refletem corretamente o total geral de pedidos que envolveram produtos com desconto e que foram pagos por uma combinação de cartão-presente e crédito de loja.

### Gerenciador de tags da Google

<!-- MC-33729-->

* O aplicativo não gera mais um erro de JavaScript durante o check-out quando a variável **Modo de restrição de cookie** e o Google Tag Manager estão ativados.

### GraphQL

<!-- ENGCOM-7661 7662 7559 MC-35646 32345-->

* Os compradores podem selecionar mensagem de presente e opções de encapsulamento durante o check-out. O GraphQL agora cobre opções de mensagem de presente para diferentes tipos de itens do carrinho. Consulte [`setGiftOptionsOnCart` mutação](https://devdocs.magento.com/guides/v2.4/graphql/mutations/set-gift-options.html) e [`updateCartItems` mutação](https://devdocs.magento.com/guides/v2.4/graphql/mutations/update-cart-items.html). _Correção enviada por Oleh Usik em solicitações de pull [28519](https://github.com/magento/magento2/pull/28105), [27956](https://github.com/magento/magento2/pull/27956), [28072](https://github.com/magento/magento2/pull/28072), [28072](https://github.com/magento/magento2/pull/28072)e 246 em private repo partner-magento2ee_. [GitHub-253](https://github.com/magento/magento2/issues/28519)
...
* Os clientes podem aplicar ou remover pontos de recompensa aos carrinhos. Eles também podem ver seu histórico do ponto de recompensa. Consulte [`applyRewardPointsToCart`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/apply-reward-points.html) e [`removeRewardPointsFromCart`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/remove-reward-points.html) para uma discussão sobre como gerenciar pontos de recompensa em um carrinho. _Correção apresentada por Petkovski Marjan no pedido de pull 285 em private repo partner-magento2ee e Dmitriy Gallyamov nos pedidos de pull 284 e 281 em private repo partner-magento2ee_. [GitHub-28835](https://github.com/magento/magento2/issues/28835), [GitHub-28833](https://github.com/magento/magento2/issues/28833)
...
* As consultas do GraphQL para produtos relacionados agora retornam valores para produtos relacionados que foram criados a partir de regras de destino. Anteriormente, as consultas de produtos relacionados (venda adicional e venda cruzada) retornavam apenas valores se os produtos relacionados fossem adicionados das configurações do produto. _Correção apresentada pela Ulzii na solicitação de pull 288 em private repo partner-magento2ee_. [GitHub-28566](https://github.com/magento/magento2/issues/28566)
...
* `updateCustomer` não permite mais definir um valor inválido `INT` na variável `gender` argumento . _Correção apresentada por Alexander Taranovsky no pedido de pull [28487](https://github.com/magento/magento2/pull/28487)_. [GitHub-28481](https://github.com/magento/magento2/issues/28481)
...
* O GraphQL padrão `Category` O método agora classifica por posição de categoria conforme esperado. _Correção apresentada por Derrik Nyomo no pedido de pull [29301](https://github.com/magento/magento2/pull/29301)_. [GitHub-104](https://github.com/magento/catalog-storefront/issues/104)

* Adição de suporte para listas de desejos no Magento Open Source. Você pode [adicionar itens](https://devdocs.magento.com/guides/v2.4/graphql/mutations/add-products-to-wishlist.html) para, [atualizar itens](https://devdocs.magento.com/guides/v2.4/graphql/mutations/update-products-in-wishlist.html) em, e [remover itens](https://devdocs.magento.com/guides/v2.4/graphql/mutations/remove-products-from-wishlist.html) de uma lista de desejos. _Correção apresentada por Eduard Chitoraga em pedidos de pull [28205](https://github.com/magento/magento2/pull/28205) e 264 em acordos de recompra privados-magento2ee_. [GitHub-28551](https://github.com/magento/magento2/issues/28551)
...
* Comentários de código desnecessários foram removidos de `app/code/Magento/CatalogInventory/Model/StockState.php`. _Correção enviada pelo Vitaliy Prokopov na solicitação de pull [27758](https://github.com/magento/magento2/pull/27758)_. [GitHub-26702](https://github.com/magento/magento2/issues/26702)
...
* O filtro de navegação em camadas de amostra agora está consistente com os filtros padrão usados no Adobe Commerce. _Correção apresentada por Bartłomiej Szubert no pedido de pull [28015](https://github.com/magento/magento2/pull/28015)_. [GitHub-28011](https://github.com/magento/magento2/issues/28011)
...
* Foram adicionados testes para cobrir a **Usado em** links para uma imagem na galeria de mídia. _Correção apresentada por Nazar Klovanych no pedido de pull [29392](https://github.com/magento/magento2/pull/29392)_. [GitHub-1963](https://github.com/magento/adobe-stock-integration/issues/1693)
...
* Os emails de assinatura do boletim informativo agora usam as mesmas tags de HTML que outros emails relacionados ao boletim informativo. _Correção apresentada por Paweł Tylek no pedido de pull [27357](https://github.com/magento/magento2/pull/27357)_. [GitHub-28166](https://github.com/magento/magento2/issues/28166)

<!-- ENGCOM-7788-->

* Adiciona um teste para excluir o assinante do boletim informativo como um usuário administrador. _Correção enviada por Dmitry Tsimbal na solicitação de pull [28972](https://github.com/magento/magento2/pull/28972)_. [GitHub-29032](https://github.com/magento/magento2/issues/29032)
...
* O `CustomerAddressI` valores para um cliente recém-criado agora são validados entre aspas. _Correção apresentada por Andrii Kalinich no pedido de pull [29139](https://github.com/magento/magento2/pull/29139)_. [GitHub-28793](https://github.com/magento/magento2/issues/28793)
...
* O `description` e `short_description` os atributos do produto foram reordenados para acomodar melhor as alterações planejadas no armazenamento temporário de conteúdo do Page Builder. _Correção enviada por Matt Walters em solicitação de pull [29238](https://github.com/magento/magento2/pull/29238)_. [GitHub-543](https://github.com/magento/magento2-page-builder/issues/543)
...
* A página Retornos agora funciona conforme o esperado depois que você cria um rótulo de envio para uma RMA (Return Merchandise Authorization). Anteriormente, os comerciantes não podiam interagir com nenhum elemento de página na página Retornos depois de criar um rótulo de envio para uma RMA. Esse era um problema conhecido da versão 2.4.0 e `MC-35984-2.4.0-CE-composer.patch` Esse problema foi resolvido para a versão 2.4.0.

<!-- MC-35826-->

* O aplicativo agora exibe uma mensagem de erro informativa ou todos os produtos disponíveis quando um administrador com permissões restritas tenta criar um retorno para produtos que não estão mais no escopo de site atribuído. Anteriormente, o aplicativo exibia uma página em branco.
...
* O aplicativo não atribui mais um status de `Complete` após faturamento para uma ordem que exija pagamento zero.
...
* Os resultados de Elasticsearch agora incluem os valores corretos para cada opção de atributo de exibição de loja. Se um atributo Suspenso ou Seleção Múltipla tiver um valor de opção diferente na exibição de loja não padrão do que na exibição de loja padrão, o Elasticsearch agora indexará esse valor ou retornará o produto com esse valor nos resultados. Anteriormente, o Elasticsearch não indexava esse valor ou retornava o produto com esse valor nos resultados.
...
* Os clientes agora podem remover com êxito os pontos de recompensa na página de revisão do pedido do fluxo de trabalho de finalização ao fazer check-out com vários endereços. Anteriormente, o aplicativo exibia uma `404 Not Found` erro quando um cliente clicou no **Remover** para remover pontos de qualquer endereço. Esse era um problema conhecido na versão 2.4.0.
...
* O **Definir produto como novo** Agora, o rótulo do atributo aparece como esperado nas páginas de detalhes e atualizações programadas de um produto.

<!-- MC-34484-->

* Agora é possível visualizar com êxito o agendamento de preparação de um bloco CMS. Anteriormente, o aplicativo gerava um erro fatal.

<!-- MC-33789-->

* Agora você pode acessar a página Visualização do CMS ao editar uma página do CMS com uma atualização programada e reatribuí-la do armazenamento padrão a outra loja. Anteriormente, o aplicativo exibia um erro 404 ao clicar no link de visualização.

<!-- MC-33572-->

* Agora você pode usar o POST `/V1/products/special-price` para atualizar o preço especial de um produto sem especificar um `price_to` valor do parâmetro. Anteriormente, o tenta definir a atualização de preço sem a variável `price_to` parâmetro especificado sem êxito.
...
* O desempenho da página do produto nas seguintes condições melhorou:
   ...
   * A funcionalidade do produto vinculado (produtos relacionados, de venda agregada e de venda cruzada) é implementada
...
* Remoção de um não usado `AdminAnalytics` teste (`TrackingScriptTest`). _Correção apresentada por Lukasz Bajsarowicz no pedido de pull [28605](https://github.com/magento/magento2/pull/28605)_. [GitHub-28850](https://github.com/magento/magento2/issues/28850)
...
* O aplicativo agora exibe o estoque de origem em vez do estoque de produto padrão quando você classifica produtos no Visual Merchandiser e o Inventory é ativado.

### Estrutura da API da Web

<!-- MC-36084-->

* As faturas criadas usando o REST agora incluem informações de cartão-presente semelhantes às faturas criadas no Administrador. Anteriormente, usando o POST `/V1/order/:id/invoice` para faturar, o pedido não exibiu o código do cartão-presente ou o valor do cartão-presente aplicado.

<!-- MC-35838-->

* Os comerciantes com vários sites agora podem usar a REST API para criar e atualizar produtos, preservando a herança de imagem e de função de imagem. Anteriormente, quando um comerciante usava a REST API para criar e atualizar produtos e um produto era atualizado para exibição de loja, e as funções de imagem padrão eram carregadas e salvas para essa exibição de loja. Como resultado, as funções de imagem de exibição de loja pararam de herdar do escopo padrão após a atualização.
...
* Habilitar a restrição de site não bloqueia mais as solicitações de ESI da Varnish para clientes conectados como convidados. Anteriormente, esses convidados não conseguiam acessar o menu principal da página inicial.

<!-- MC-32369-->

* O aplicativo agora carrega a página inicial do produto conforme esperado quando as restrições do site estão ativadas.

### Lista de desejos

<!-- MC-35810-->

* Agora você pode usar o recurso de pesquisa da lista de desejos para localizar um produto em uma lista de desejos pública em implantações nas quais o suporte para várias listas de desejos esteja ativado. Anteriormente, depois que um cliente usava a pesquisa da lista de desejos para localizar um produto, selecionava-o e clicava em **Adicionar ao carrinho**, o aplicativo não adicionou o produto ao carrinho e exibiu esse erro: `Invalid Form Key. Please refresh the page`.
...
* A página Categoria da loja agora exibe o widget de pesquisa da lista de desejos conforme esperado.

<!-- ENGCOM-7580 7285-->

* Enchimentos de polietileno para `Map`, `WeakMap`, `FormData`e `MutationObserver` agora são carregadas somente conforme necessário. _Correção enviada por Dmitry Tsimbal na solicitação de pull [28330](https://github.com/magento/magento2/pull/28330) e Mateusz Krzeszowiak no pedido de pull [27618](https://github.com/magento/magento2/pull/27618)_. [GitHub-28377](https://github.com/magento/magento2/issues/28377), [GitHub-28382](https://github.com/magento/magento2/issues/28382)
...
* O aplicativo agora valida corretamente o máximo permitido de listas de desejos que um comprador pode criar. _Correção apresentada por Eduard Chitoraga no pedido de pull 247 em private repo partner-magento2ee_.
