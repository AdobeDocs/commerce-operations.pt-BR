...
# Adobe Systems Comércio 2.4.8 notas de versão

{{bics}}

## Destaques

Adobe Systems Comércio 2.4.8 traz segurança aprimorada, compatibilidade com PHP 8.4 e MariaDB 11.4, melhorias abrangentes da API GraphQL para acelerar a migração para um novo [Adobe Systems Comércio Storefront](https://experienceleague.adobe.com/developer/commerce/storefront/) fornecido pela Edge Delivery, e mais de 500 correções e melhorias de qualidade para elevar as funcionalidade existentes. A atualização para o 2.4.8 mantém sua plataforma segura, alinhada com os últimos avanços tecnológicos e apoiada até abril de 2028.
...
<table style="table-layout:auto">
    <tbody>
        ...
        <tr>
            ...
            <td>O <a href="https://developer.adobe.com/commerce/php/development/security/subresource-integrity/">mecanismo SRI</a> introduzido na versão 2.4.7 foi refatorado para melhorar a confiabilidade e o desempenho. Em vez de armazenar hashes em um cache personalizado que poderiam ser limpos durante as operações de limpeza de cache, os <code>pub/static</code> hashes SRI agora são armazenados diretamente no diretório e organizados por área (<code>adminhtml</code>, <code>base</code>, <code>frontend</code>). Essa alteração elimina a necessidade de reimplemente o conteúdo estático após a liberação do cache.</td>
        </tr>
    </tbody>
</table>
...
As atualizações de plataforma a seguir para esta versão garantem que Adobe Systems Comércio continue sendo uma plataforma robusta e confiável, pronta para atender às demandas dos ambientes de comércio modernos:
...
<table style="table-layout:auto">
    <tbody>
        ...
        <tr>
            ...
            Essa versão inclui os seguintes aprimoramentos no banco de dados:
              <ul>
                <li><strong>Coleção</strong> MySQL: Agora o sistema usa o <code>utf8mb4</code> collation para MySQL, garantindo a compatibilidade com o MySQL 8 e provando o futuro contra a descontinuação de <code>utf8mb3</code>. Anteriormente, o sistema padrão era usar <code>utf8mb3</code> a colação, que está obsoleta no MySQL 8.<!-- AC-8828 --></li>
                <li><strong>Restrições</strong> de fey estrangeira MySQL: MySQL 8.4 introduz validação de chave externa mais rigorosas por padrão. A configuração <code>restrict_fk_on_non_standard_key</code> agora está ATIVADA por padrão, o que restringe o uso de chaves não exclusivas ou parciais como chaves estrangeiras. Os comerciantes que estiverem atualizando do MySQL 8.0 para o MySQL 8.4 devem definir explicitamente esse parâmetro como OFF ou usar a opção de servidor <code>--skip-restrict-fk-on-non-standard-key</code> para manter a compatibilidade com estruturas de banco de dados existentes. Consulte o <a href="/help/upgrade/prepare/prerequisites.md#mysql"><em>Guia de Atualização</em></a> para obter mais detalhes.<!-- AC-14257 --></li>
              </ul>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>Os seguintes componentes e dependências de <a href="/help/release/packages/adobe-commerce.md">terceiros foram atualizados</a> para as versões estáveis mais recentes para melhorar a estabilidade e o desempenho da plataforma:
              <ul>
        ...
        <tr>
            ...
            <td>Adobe Systems Comércio agora está otimizada para o OpenSearch 2.19 e não é mais compatível com o Elasticsearch. Todos os Elasticsearch 7 e 8 módulos e classes estão agora obsoletos na base de código. Adobe Systems recomenda a transição para o OpenSearch para implantações infraestrutura em nuvem e local para garantir suporte contínuo e compatibilidade. Consulte <a href="/help/upgrade/prepare/opensearch-migration.md">Migrando para OpenSearch</a>.
              <ul>
...
<table style="table-layout:auto">
    <tbody>
        ...
        <td>
            <ul>
              ...
              <li><!--LYNX-542-->Atualização da <a href="https://developer.adobe.com/commerce/webapi/graphql-api/index.html#mutation-updateCartItems"><code>updateCartItems</code></a> mutação para retornar respostas de sucesso com detalhes de erro em vez de exceções. Melhoria no mapeamento de erros para melhorar a clareza das notificações usuário.</li>
              <li><!--LYNX-541-->Adição de um novo <code>InsufficientStockError</code> tipo para lidar com casos em que os níveis de estoque são insuficientes. Ajustado o schema para suportar novos tipos de erros, melhorando os recursos de relatórios os erros.</li>
        ...
        <tr>
           ...
           <td>Esta versão inclui os seguintes aprimoramentos de gerenciamento de clientes:
             <ul>
               ...
               <li><!--LYNX-773-->Adição de uma nova consulta <a href="https://developer.adobe.com/commerce/webapi/graphql/schema/customer/queries/segments/"><code>customerSegments</code></a> para habilitar recursos de personalização.</li>
        ...
        <tr>
           <td><strong>Pedidos</strong></td>
           <td>Essa versão inclui as seguintes usuário melhorias no gerenciamento solicitar:
             <ul>
              <li><!--LYNX-476-->O mensagens de erro aprimorado para incluir valores de inventário disponíveis. Fornece aos usuários insights mais claros sobre os níveis de estoque durante as atualizações de solicitar.</li>
              <li><!--LYNX-470-->Adicionado um novo <code>date_of_first_order</code> campo ao <a href="https://developer.adobe.com/commerce/webapi/graphql-api/index.html#definition-CustomerOrders"><code>CustomerOrders</code></a> tipo.</li>
              ...
              <li><!--LYNX-470-->Foi adicionado um filtro solicitar histórico com base no <code>created_at</code> campo e exposto <code>date_of_first_order</code> com o <a href="https://developer.adobe.com/commerce/webapi/graphql-api/index.html#definition-CustomerOrders"><code>CustomerOrders</code></a> tipo.</li>
        ...
        <td>
            <ul>
              ...
              <li><!--LYNX-540-->Adicionado um campo <code>quantity</code> em <a href="https://developer.adobe.com/commerce/webapi/graphql-api/index.html#definition-ProductInterface"><code>ProductInterface</code></a> para fornecer detalhes sobre o nível de estoque. Ele exibe o estoque disponível ou nulo com base nas configurações do administrador.</li>
    ...
    <tr>
        ...
        <td>
            <ul>
              <li><!--LYNX-713-->Atualização dos <a href="https://developer.adobe.com/commerce/webapi/graphql-api/index.html#definition-GiftOptionsPrices"><code>GiftOptionsPrices</code></a> tipos a <a href="https://developer.adobe.com/commerce/webapi/graphql-api/index.html#definition-OrderTotal"><code>OrderTotal</code></a> serem incluídos e <code>price_including_tax</code> os <code>price_excluding_tax</code> campos de invólucro do presente (por item e solicitar) e cartões impressos. Esse aprimoramento garante uma exibição de imposto precisa com base nas definições de configuração da loja para consultas de carrinho e pedido.</li>
              <li><!--LYNX-695-->Atualização da <a href="https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/store-config/"><code>storeConfig</code></a> query para incluir <code>printed_card_priceV2</code> o retorno de um objeto de preço em vez de uma string.</li>
...
<table style="table-layout:auto">
    <tbody>
        ...
        <tr>
            ...
            <ul>
              <li><strong>Aplicar pagamento</strong><!-- BUNDLE-3409 -->: quando os clientes fazem check-out usando o Apple Pay Express, a extensão do Braintree agora mostra os itens de linha do carrinho no modal do Apple Pay. Ao clicar na dica de ferramenta (?) ao lado do preço, os clientes podem visualização itens de carrinho individuais, descontos, remessa e detalhes fiscais, removendo a necessidade de sair do modal para confirmar seus itens.</li>
              <li><strong>Google Pay</strong><!-- BUNDLE-3408 -->: Quando os clientes fazem check-out usando o Google Pay Express, a extensão do Braintree agora exibe os itens de linha carrinho diretamente no modal do Google Pay. Ao clicar na dica de ferramenta (?) ao lado do preço, o cliente pode visualização itens de carrinho individuais, descontos, remessa e detalhes fiscais, eliminando a necessidade de sair do modal para verificar seus itens.</li>
            </ul>
            </td>
        </tr>
    </tbody>
