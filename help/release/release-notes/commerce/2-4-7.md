---
title: Adobe Systems Comércio 2.4.7 notas de versão
description: Saiba mais sobre novos recursos, melhorias, correções de bugs e problemas conhecidos na versão 2.4.7 Adobe Systems Comércio.
...
* Adição de suporte para o novo método de envio USPS Ground Advantage. Essa é uma integração integrada com o novo método de envio da USPS, o USPS Ground Advantage, que foi lançado em julho de 2023. Essa nova integração pode ser usada para recuperar taxas de envio e agendar entregas e devoluções por meio do serviço de envio do USPS. O método de envio USPS Ground Advantage substitui esses métodos de envio, que foram retirados quando o método de envio USPS Ground Advantage foi lançado:
...
* **Gerenciamento aprimorado do indexador**. O novo `indexer:set-status` comando oferece suporte ao gerenciamento dinâmico do status do indexador. Usuários administradores podem usar este comando para alterar o status do indexador para `suspended`, `invalid`ou `valid`. Esse recurso é particularmente útil para gerenciar o desempenho do sistema durante operações extensas em massa, como importações ou atualizações de produtos, permitindo o controle sobre quando os indexadores são automaticamente acionados pelos trabalhos de cron do sistema. Consulte [Gerenciar os indexadores](https://experienceleague.adobe.com/docs/commerce-operations/configuration-guide/cli/manage-indexers.html).

* **A listagem de produtos página para produtos complexos com muitas opções**. O tempo de carregamento melhorou para páginas da lista de produtos que incluem produtos complexos com mais de 100 opções. O desempenho das solicitações de GraphQL para lista produtos por categoria também melhorou. <!-- ACPT-1683 -->

* **Formato JSON compatível com a API](https://developer.adobe.com/commerce/webapi/rest/modules/import/)** rest [Importar. Os comerciantes agora podem importar até 100.000 registros por minuto para Adobe Systems Comércio no formato JSON. <!-- ACPT-943 -->

* **As vendas regra melhorias** de desempenho. Desempenho aprimorado de implantações empresariais com muitas (aproximadamente 100.000) regras de vendas ativas. Implantações empresariais que implementar promoções frequentemente implantar muitas regras de carrinho ativas. Esses tipos de implantações empresariais executadas Comércio 2.4.7 não verão degradação de desempenho relacionada ao número de regras de preço carrinho configuradas durante as operações de checkout. <!-- ACPT-1198 -->

* **Operações de salvamento mais rápidas de configurações de nível armazenamento para implantações com muitas lojas**. Salvar configurações em implantações com mais de 500 lojas pode ser demorado. O novo módulo [Configuração Assíncrona](../../../performance/configuration.md#asynchronous-configuration-save) habilita operações assíncronas de salvamento de configuração executando um trabalho cron que usa um consumidor para processar a operação de salvamento em uma fila de mensagens. AsyncConfig está desabilitado por padrão.
...
* **Suporte ao GraphQL para todos os [!DNL Braintree] métodos de pagamento, exceto[!DNL Venmo]**: Mais configurações são expostas na API GraphQL. Isso é particularmente útil para aplicativos sem perdoméricos.

* **[!DNL Vaulting]pagamentos na área** de conta - Os clientes conectados agora [podem abobadá-armazenamento](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/payments/braintree#step-3-complete-the-advanced-settings) novos cartões de crédito/débito e [!DNL PayPal] contas na área de conta do Cliente. Anteriormente, os clientes só podiam abobadar/armazenamento ao salvar seus pagamentos para uso posterior ao concluir uma transação no check-out, agora eles podem abotoar novos cartões de crédito/débito e contas PayPal sem precisar criar uma nova transação.

* **Transações sem atritos**— As transações sem atritos aceleram o processo de pagamento, reduzindo a quantidade de cliques/etapas do cliente para concluir uma transação cartão de crédito/débito online. Anteriormente (quando o 3DS era ativado), cada cliente era desafiado por 3DS. Com o novo recurso Transações sem atritos, os clientes só são desafiados para o 3DS quando o banco o solicita. Isso reduz carrinho desistência, aumenta as taxas de conversão e leva a mais vendas.
...
* **Os itens do pedido agora incluem imagens** do produto. `OrderItemInterface` expõe as imagens do produto, que permite que imagens sejam associadas a produtos solicitados e carreguem com mais eficiência.  [GitHub-32369](https://github.com/magento/magento2/issues/32369) <!-- AC-7918 -->

* **Suporte expandido para resolver armazenamento em cache**. Os seguintes resolvedores de query GraphQL agora podem ser armazenados em cache no cache de Resultados do Resolvedor de GraphQL, o que melhora o desempenho quando as consultas são enviadas com solicitações POST:
...
* **Suporte para cancelamento** de solicitar. A [mutação cancelOrder](https://developer.adobe.com/commerce/webapi/graphql/schema/orders/mutations/cancel-order/) permite que um cliente cancele uma solicitar, passando seu identificador e um motivo de cancelamento. <!-- LYNX-197 -->
...
* **Desempenho de analisador GraphQL aprimorado**. O desempenho do analisador de GraphQL foi melhorado reduzindo o número de vezes que o método de análise é chamado por solicitação. Agora é chamado uma vez. Anteriormente, o analisador era chamado pelo menos três vezes.
...
* [`clearCart`](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/clear-cart/) mutação
...
O Gerenciamento de inventário (v1.2.7) fornece ferramentas para gerenciar inventário de produtos. Este recurso desenvolvido comunidade é agrupado com Adobe Systems Comércio e Magento Open Source código principal.

### Pagamentos

* O suporte ao GraphQL foi adicionado para operações principais para todos os métodos de pagamento, exceto para Venmo. Novo terminais GraphQL para serviços de pagamento são descritos no [Checkout](https://developer.adobe.com/commerce/webapi/graphql/schema/checkout/). <!-- BUNDLE-3326 -->

* O cartão abobadamento de cartão foi aprimorado para todos os métodos de pagamento, exceto para Venmo. Os compradores agora podem abobadá-los ou salvar seus pagamentos da ACH e visualização ou excluí-los de suas conta página. Os comerciantes podem habilitar e desativar pagamentos abobadados da ACH do Administrador. <!-- BUNDLE-3324 -->

* O fluxo de Trabalho de check-out agora inclui uma seção de pagamento expresso que fornece botões PayPal, Google Pay e Apple Pay **[!UICONTROL Express]** . <!-- BUNDLE-3322 -->

[O Opções](https://experienceleague.adobe.com/docs/commerce/payment-services/payments-checkout/payments-options.html) de pagamento introduz métodos de pagamento compatíveis.
...
Novo endpoints REST suportam a visualização, adição e exclusão de vários cupons associados a uma carrinho.
...
Substituir códigos cupom em uma carrinho:

`PUT /rest/{store_code}/V2/carts/{cartId}/coupons`
`PUT /rest/{store_code}/V2/carts/mine/coupons`

Remover cupom códigos de uma carrinho:

`POST /rest/{store_code}/V2/carts/{cartId}/coupons/deleteByCodes`
`POST /rest/{store_code}/V2/carts/mine/coupons/deleteByCodes`

**Endpoints adicionais**
Esta versão apresenta dois novos endpoints REST que fornecem uma solução alternativa para uma limitação com a API `GET` REST e `POST V1/products/attributes` endpoints. Esses endpoints retornam o mesmo valor para o `is_filterable` atributo para as `Filterable(with results)` opções e `Filterable(no results)` as opções da opção **[!UICONTROL Use in Layered Navigation]** . (A propriedade do atributo `is_filterable` é do tipo `Boolean`, o que não permite definir essa propriedade como `Filterable(no results)`.)

Dois novos endpoints de REST foram implementados como solução alternativa:

* `PUT /V1/products/attributes/{attributeCode}/is-filterable/{isFilterable}`. Parâmetros de caminho: () e `isFilterable` (`int` os valores são: 0 é Não; 1 é `Filterable (with results)`; 2 é `Filterable (no results)`).`String``attributeCode`
* `GET /V1/products/attributes/{attributeCode}/is-filterable`. Parâmetros de caminho: `attributeCode` (`String`).<!-- ACP2E-2275 -->

## Problemas corrigidos

Corrigimos centenas de problemas no Adobe Systems Comércio código principal 2.4.7. Um subconjunto de problemas corrigidos incluídos nesta versão é descrito abaixo. As correções incluídas em Comércio versões 2.4.7-beta anteriores também são descritas.
...
* `bin/magento setup:upgrade` foi refatorizado para ser executado com sucesso ao instalar uma nova módulo que instala tanto tabelas quanto indexadores associados `mview` . [GitHub-37304](https://github.com/magento/magento2/issues/37304)
...
* O menu Vendas de administrador agora exibe o submenu corretamente. Anteriormente, a quebra de coluna não funcionava corretamente e alguns submenus não eram exibidos. [GitHub-37812](https://github.com/magento/magento2/issues/37812)

<!-- AC-3122 -->

* **[!UICONTROL Select]** As opções do menu suspenso agora estão visíveis no Admin **[!UICONTROL Content]** > **[!UICONTROL Pages]** quando várias páginas são selecionadas na grade. [GitHub-35386](https://github.com/magento/magento2/issues/35386)

<!-- AC-9209 -->

* Corrigiu o caminho para o valor padrão das `system/dashboard/enable_charts` configurações. Os gráficos agora são exibidos no Admin, conforme esperado. [GitHub-37668](https://github.com/magento/magento2/issues/37668)
...
* Títulos de submenu agora indicam todos os irmãos independentemente do nível de acesso atribuído ao usuário. Anteriormente, essas informações eram oculto de usuários administrador com acesso limitado, mesmo que existissem vários grupos irmãos para o submenu.
...
* A grade de solicitar arquivadas é atualizada de forma assíncrona com novos status de solicitar conforme esperado. Uma abordagem baseada em evento para atualizar solicitar status na grade de arquivos resulta em um status solicitar preciso, incluindo reembolsos de memorandos de postagem crédito.

<!-- ACP2E-2049 -->

* O menu suspenso Ações da grade de administração agora pode passar o mouse como esperado em uma link selecionada para **[!UICONTROL Content]** > **[!UICONTROL Pages]** (e catálogos compartilhados no módulo de B2B). Anteriormente, esse menu era principalmente oculto e administrador usuários tinham que rolar para ver as opções. [GitHub-36710](https://github.com/magento/magento2/issues/36710)

### Analytics

<!-- ACP2E-2266 -->

* O detalhe do produto página `meta itemprop="image"` e `meta itemprop="description"` as propriedades agora estão disponíveis para a imagem principal do produto. [GitHub-37497](https://github.com/magento/magento2/issues/37497)

<!-- ACP2E-2314 -->

* A seção Relatório da grade Produtos em Carrinhos agora exibe o preço correto do produto quando **[!UICONTROL Catalog Price Scope]** está definido como **[!UICONTROL Website]**. Anteriormente, o preço do produto era exibido como zero quando o produto era atribuído a um site diferente do padrão.
...
* `price` e `quantity` os valores na camada de dados do Gerenciador de tags do Google agora são enviados como valores numérico conforme o esperado para Google Analytics. Anteriormente, esses valores eram enviados como cadeias de caracteres.

<!-- ACP2E-2030 -->

* Erros ao manipular dados de cabeçalho ao enviar dados de implantação para o Novo módulo de relíquia foram resolvidos.

<!-- ACP2E-1833 -->

* Google Analytics 3 tags agora são acionadas como esperado quando as Google Analytics 4 tags são desativadas.
...
* A `customerCart` query retorna todos os descontos aplicados em produtos de pacote conforme o esperado. Anteriormente, os descontos totais que eram aplicados a um produto pacote eram retornados como zero.
...
* Os produtos do pacote agora são importados com êxito sem duplicar SKUs nas opções do produto. Anteriormente, Adobe Systems Comércio criava várias SKUs duplicado nas opções de produtos quando você importou pacote produtos com entradas duplicado.
...
* As categorias agora estão visíveis para os compradores nas lojas onde o cache está definido como Fastly CDN (**[!UICONTROL Caching Application=Fastly CDN]**). Isso afetou os compradores convidados e os clientes registrados.

<!-- ACP2E-2070 -->

* Os cookies de cache após fazer logon agora são os mesmos após navegarem no site. Anteriormente, o controlador de fazer logon enviava o cookie de cache errado e as páginas poderiam ter sido armazenadas em cache várias vezes.

<!-- ACP2E-1840 -->

* As respostas do GraphQL GET operações em páginas CMS que contêm blocos CMS agora são apagadas conforme esperado no cache Fastly. As atualizações de programação subsequentes agora mostram um conteúdo preciso e atualizado dessas páginas.

<!-- ACP2E-1524 -->

* Página cache agora é limpo como esperado para um produto principal quando um de seus produtos secundários foi salvo do Administrador. Uma verificação plug-in foi introduzida para atingir a invalidação de cache para variantes configuráveis, semelhante a outros tipos de produtos, independentemente do estado do indexador (programado ou em tempo real). Anteriormente, os preços não eram atualizados corretamente na vitrine depois que uma variação configurável foi atualizada do Administrador.
...
* O plug-in cujo objetivo é adicionar o cache configurável tag quando um produto simples associado a um produto configurável é salvo agora funciona conforme o esperado. Todas as tags de cache configuráveis relevantes são removidas conforme esperado quando um produto simples é atualizado.`POST V1/products` Anteriormente, nem todos os preços eram atualizados na loja e os caches tinham que ser limpos manualmente. [GitHub-36726](https://github.com/magento/magento2/issues/36726)
...
* A geração de carrinho regras de filtragem de texto para atributos do produto foi aprimorada. Produtos individuais não são mais carregados várias vezes.

<!-- AC-8920 -->

* Adobe Systems Comércio agora exibe uma mensagem de erro informativa quando ocorre um erro durante o check-out e não retorna mais a consumidor para uma etapa anterior de checkout. Anteriormente, o consumidor era retornado ao página de envio.
...
* Comércio agora exibe mensagens informativas quando uma ação de adicionar à carrinho é acionada. Anteriormente, o `The requested qty is not available` não era sempre exibido como necessário na vitrine.
...
* Os cupons agora são aplicados com êxito a um pedido quando um comprador aplica o cupom depois de selecionar o método de envio de taxa uniforme em que uma regra de preço do carrinho com método de envio definido como uma condição é aplicável. Anteriormente, o carrinho página exibia o Cupom não era um erro válido e o cupom não era aplicado. [GitHub-34866](https://github.com/magento/magento2/issues/34866)

<!-- AC-8596 -->

* A **[!UICONTROL My billing and shipping address are the same]** caixa de seleção agora permanece marcada por padrão quando uma consumidor usa Cromo preenchimento automático para preencher suas endereço de entrega e, em seguida, clica Próximo **antes que o bloco de métodos** de envio seja recarregado. [GitHub-33725](https://github.com/magento/magento2/issues/33725)

<!-- ACP2E-2005 -->

* O `applied_rule_ids` valor na `quote_item` tabela agora inclui uma lista correta das IDs de regra aplicadas. Anteriormente, esse valor continha apenas a última ID de regra aplicada.

<!-- ACP2E-2025 -->

* Os dados do carrinho e do cliente no armazenamento local agora são redefinidos quando uma sessão é removida do servidor. Anteriormente, esses dados ainda apareciam quando o arquivo da sessão era removido.
...
* Quando o status do estoque de produto for automaticamente atualizado para fora do estoque com base na configuração de estoque (quantidade), o status do estoque será posteriormente atualizado em estoque quando o estoque quantidade for atualizado. Anteriormente, a `stock_status_changed_auto` configuração era ignorada quando a configuração de estoque de um produto era atualizada para em estoque.

<!-- AC-8904 -->

* A classificação da barra de ferramentas agora funciona conforme esperado nas páginas de Search quando a **[!UICONTROL Remember Category Pagination]** configuração está ativada. [GitHub-33220](https://github.com/magento/magento2/issues/33220)
...
* Usuários administradores com acesso limitado ao site agora podem adicionar imagens a um produto que se enquadra em seus escopo. Se o administrador usuário não tiver direitos de acesso ao produto, Comércio exibirá uma mensagem informativa. Anteriormente, administrador usuários com acesso limitado ao site não podiam adicionar imagens a produtos aos quais tinham acesso.

<!-- ACP2E-1956 -->

* Valores e `special_to_date``news_from_date``news_to_date` definição `special_from_date`do usuário não são substituídos quando um produto é editado em escopo de armazenamento-visualização.

<!-- AC-6761 -->

* Comércio agora exibe o nome correto após classificar produtos por nome na Administração > **[!UICONTROL Category]** > **[!UICONTROL Products in category]** em implantações de vários armazenamentos onde os nomes dos produtos variam de acordo com armazenamento. [GitHub-36208](https://github.com/magento/magento2/issues/36208)

<!-- ACP2E-1975 -->

* A `addProductsToCart` mutação agora retorna opções personalizáveis com dados. Anteriormente, retornava apenas opções personalizáveis vazias. [GitHub-37599](https://github.com/magento/magento2/issues/37599)

<!-- ACP2E-1859 -->

* A posição do produto e a seleção da ID agora são exibidas corretamente quando um usuário administrador adiciona um novo produto a uma categoria após limpar o cache, mas não recarregar a página da categoria. Anteriormente, a posição e a seleção do produto eram incompatíveis e exibidas incorretamente.
...
* Acessar produtos baixáveis do Administrador não resulta mais em erro quando o usuário administrador altera a exibição da loja da exibição usada durante a criação do produto para outra exibição da loja. Os preços dos produtos são convertidos em flutuação com sucesso quando uma administrador usuário muda armazenamento visualização de todas as exibições armazenamento para o visualização de armazenamento padrão. Anteriormente, o Adobe Commerce emitia um erro. [GitHub-37519](https://github.com/magento/magento2/issues/37519)
...
* A contagem de produtos na árvore de categorias (Administrador **[!UICONTROL Catalog]** > **[!UICONTROL Category]**) agora é precisa. Anteriormente, a contagem de produtos do categoria não podia ser recuperada da `catalog_category_product_index` tabela. O erro de digitação que acionou esse bug foi corrigido. [GitHub-35417](https://github.com/magento/magento2/issues/35417)
...
* Adobe Systems Comércio agora gera um 301 redirecionar para um produto quando a categoria à qual pertence foi transferida para um novo pai. [GitHub-37039](https://github.com/magento/magento2/issues/37039)
...
* Os comerciantes agora podem selecionar **[!UICONTROL Skip quantity at this time]** e **[!UICONTROL Skip image uploading at this time]** ao configurar um produto configurável sem afetar a origem dos produtos simples associados. Anteriormente, a seleção  **[!UICONTROL Skip quantity at this time]** resultou no desaparecimento de fontes de produtos.

<!-- AC-7099 -->

* O **[!UICONTROL As low as]** rótulo não é mais exibido por um preço configurável do produto quando todas as opções têm o mesmo preço.

<!-- ACP2E-2464 -->

* Usuários administradores com permissões de site agora podem salvar um produto filho que faça parte de um produto configurável em um contexto armazenamento diferente. Os processos de reindexação agora ignoram esse tipo de produto configurável.

<!-- ACP2E-2003 -->

* Produtos configuráveis com dois produtos secundários não são mais marcados como desativados quando um produto filho é desativado por uma atualização programada.

<!-- AC-8060 -->

* O desempenho das operações de salvamento para produtos configuráveis com várias opções melhorou. Anteriormente, os tempos limite podiam ocorrer durante as operações de salvamento do produto. [GitHub-36928](https://github.com/magento/magento2/issues/36928)

<!-- ACP2E-1764 -->

* A exibição na vitrine dos preços configuráveis do produto agora muda conforme esperado quando um consumidor seleciona uma opção de produto. [GitHub-37378](https://github.com/magento/magento2/issues/37378)

### Cupons

<!-- ACP2E-2495 -->

* Os códigos de cupom com um número limitado de usos por cliente agora podem ser usados pela segunda vez quando o solicitar para o qual foi usado anteriormente falha. Anteriormente, o código promocional não era lançado quando o solicitar anterior era cancelado.

<!-- ACP2E-2428 -->

* Códigos de cupom que contêm caracteres de espaço não são mais invalidados. Anteriormente, se um código de cupom continha um caractere de espaço (antes ou depois do código real), a validação falhava. [GitHub-38048](https://github.com/magento/magento2/issues/38048)
...
* Código refatorado em toda a base de código que foi criada quando a prática de codificação ditava que qualquer seção de cache incluída no arquivo `app/etc/env.php` incluiria uma seção de front-end. Esse redirecionamento aborda possíveis problemas ao atualizar Comércio implantações pré-2.4.4. [GitHub-38363](https://github.com/magento/magento2/issues/38363)

<!-- AC-8005 -->

* Esclarecimento da mensagem de erro associada `StockItemValidator.php` quando uma ID de estoque com um valor de 0 é fornecida durante uma chamada de API. [GitHub-31107](https://github.com/magento/magento2/issues/31107)

<!-- AC-10583 -->

* Foi adicionada uma matriz vazia padrão para `$tagGenerators` `CompositeTagGenerator` impedir erros durante a compilação de injeção de dependência. Anteriormente, executar o `bin/magento setup:upgrade` comando quando os módulos e `Magento_StoreGraphQl` os `Magento_DirectoryGraphQl` módulos estavam desativados resultava em um erro. [GitHub-38165](https://github.com/magento/magento2/issues/38165)
...
* O `var/log/system.log` arquivo não contém `main.INFO error` mais mensagens. O log de entrada do menu foi movido de informações de nível para DEBUG. Anteriormente, o `var/log/system.log` arquivo era inundado de mensagens curtir o seguinte: `Add of item with id Magento_Theme::design_config was processed … `.

<!-- AC-8809 -->

* O carregamento automático não falha mais quando `exec()` retorna false. A verificação foi adicionada ao processo de pré-carregamento para impedir que `array_combine` receba `false` como segundo argumento. Como resultado, o pré-carregamento continuará a ser executado quando `exec()` retornar `false`. [GitHub-37509](https://github.com/magento/magento2/issues/37509)
...
* O desempenho dos indexadores para grandes lojas com bancos de dados de produtos extremamente ativos melhorou. Agora, os indexadores indexam as informações mais recentes do produto uma vez para cada índice, em vez de várias vezes. Anteriormente, os produtos desaparecevam do site e os dados do produto podiam estar desatualizados no site. [GitHub-30012](https://github.com/magento/magento2/issues/30012)

<!-- ACP2E-1971 -->

* `mview_state` O status do registro agora indica com precisão o estado baseado no changelog atividade quando uma conexão de banco de dados subordinada está disponível. Anteriormente, esses registros permaneceram em um `waiting` estado nas implantações nuvem mesmo quando não havia novas entradas no changelog.
...
* Os usuários administradores agora podem gerar uma fatura quando suas Comércio instância estão conectadas ao S3 armazenamento.Anteriormente, Comércio exibia esse erro: `Cannot create image resource. File not found`. [GitHub-35706](https://github.com/magento/magento2/issues/35706)
...
* Mensagem de erro aprimorada para o cenário em que usuário tenta acessar uma inválido URL do Administrador. [GitHub-35682](https://github.com/magento/magento2/issues/35682).
...
* Adobe Systems Comércio não lança mais essa exceção do sistema de arquivos quando você tenta liberar o cache JavaScript/CSS: `No such file or directory`.

<!-- ACP2E-1358 -->

* O valor de `value_id` in `customer_entity_int` agora é incrementado corretamente. Uma atualização query é executada ao invés de `insert on duplicate key update` atualizar uma entidade que incorpore esses atributos. Como resultado, as colunas de aumento automático usadas no modelo EAV agora crescem de forma linear. Anteriormente, o processo de incremento automático para colunas pulava valores devido a falhas na inserção de queries. [GitHub-28387](https://github.com/magento/magento2/issues/28387)
...
* Os compradores agora podem acessar páginas configuradas como nós de hierarquia quando as restrições de acesso estão ativadas. Anteriormente, um loop de redirecionar infinito acontecia, e o navegador exibia este erro: `ERR_TOO_MANY_REDIRECTS`.

<!-- ACP2E-2404 -->

* Mensagens de erro informativas foram adicionadas aos validação de campo personalizado nas páginas de detalhes do produto. [GitHub-38006](https://github.com/magento/magento2/issues/38006)

<!-- ACP2E-2511 -->

* Caracteres especiais agora podem ser usados em campos de nome. Anteriormente, o caractere e comercial (&amp;) era restrito no validador de nome, que bloqueava as operações de adição ou edição do cliente quando o nome do cliente continha e comercial. [GitHub-38080](https://github.com/magento/magento2/issues/38080)

<!-- ACP2E-2469 -->

* Usuários administradores com acesso restrito a uma armazenamento específica agora podem usar uma ação de atualização em massa para atualizar as avaliações do produto. Anteriormente, Comércio lançou esta exceção: `report.CRITICAL: TypeError: array_intersect(): Argument #1 ($array) must be of type array, null given in app/code/Magento/AdminGws/Model/Models.php:439`. .

<!-- AC-8600 -->

* A vitrine Exibir da página fazer logon Senha funcionalidade agora exibe senhas conforme o esperado. Anteriormente, as senhas não eram exibidas. [GitHub-37432](https://github.com/magento/magento2/issues/37432)

<!-- ACP2E-2062 -->

* A comparação de produtos lista agora é sempre específica do site para compradores convidados e não contém produtos que foram adicionados a partir de outros sites, incluindo produtos que foram atribuídos a ambos os sites.

<!-- ACP2E-2087 -->

* Os blocos de conteúdo CMS que contêm emojis não são mais truncados após o emoji quando salvos no banco de dados. Anteriormente, conteúdo era truncado após o emoji porque a configuração padrão do banco de dados não suportava caracteres de quatro bytes.
...
* A grade de página CMS agora filtros páginas com base nas visualização de armazenamento atribuídas ao usuário de administrador. Anteriormente, função escopo não era aplicada.
...
* O atributo Redefinir Senha Token do sistema da entidade do cliente (`rp_token`) validação foi removido do validação de atributo durante a criação do conta do cliente, e as validação de diacríticas são incluídas apenas no email do cliente resultante. Anteriormente, o Commerce não salvava o registro do cliente e exibia este erro: `Something went wrong while saving the customer`.
...
* O campo Estado/província **do endereço** do cliente administrador agora retém o último valor salvo conforme esperado. Anteriormente, quando o valor era removido desse campo, Comércio continuava a exibir o valor excluído no **campo Estado/Província** . [GitHub-36846](https://github.com/magento/magento2/issues/36846)
...
* Os usuários administradores agora podem adicionar um novo endereço de cliente do Administrador quando esse cliente for criado para outro site cuja ID da loja não corresponde à ID do site. Anteriormente, Adobe Systems Comércio exibia essa mensagem pop-up: `Something went wrong`. [GitHub-36582](https://github.com/magento/magento2/issues/36582)

<!-- AC-8247 -->

* O bloco CMS editor página agora exibe o valor correto do widget `block_id` . [GitHub-29644](https://github.com/magento/magento2/issues/29644)

### Cartões de presente e invólucro

<!-- AC-7372 -->

* Os comerciantes não podem mais criar um cartão de presente com um valor negativo.

<!-- ACP2E-1890 1806-->

* A `setGiftOptionsOnCart` mutação agora remove o invólucro do presente de uma solicitar conforme esperado. O valor de `gift_wrapping_id` agora não está definido no nível do carrinho. Anteriormente, o `gift_wrapping_id` era atualizado para seu valor anterior depois que o pedido era feito, mesmo quando esse valor não estava definido no nível do carrinho.
...
* O rótulo do registro de `Event Information` presente agora é traduzido como esperado.

<!-- ACP2E-1586 -->

* Os subtotais agora são calculados corretamente para valores de cartão de presente personalizados.

<!-- ACP2E-1350 -->

* Os cartões de presente agora são aplicados corretamente durante o check-out com vários endereços. Anteriormente, os valores da cartão do presente eram aplicados incorretamente para envios de vários endereços, o que resultou em inválido valores cartão do presente.
...
* O ícone admin favicon upload formulário agora oferece `.ico` suporte a tipos de arquivos. [GitHub-34858](https://github.com/magento/magento2/issues/34858)
...
* As taxas da tabela de remessa agora são atualizadas conforme esperado no arquivo CSV de importação de taxa de tabela. O arquivo de upload da taxa de tabela é armazenado temporariamente até que o trabalho cron do processador de configuração assíncrono seja executado. Este trabalho cron seleciona o novo local do arquivo e o processa adequadamente. Anteriormente, as taxas de tabela não eram atualizadas como esperado porque o processador assíncrono procurava o arquivo no local errado.

<!-- ACP2E-2298 -->

* A grade de Exportação arquivos mostra todos os arquivos exportados, independentemente do carimbo de data e hora. Anteriormente, a grade não exibia todos os arquivos exportados que tinham o mesmo carimbo de data e hora. [GitHub-36951](https://github.com/magento/magento2/issues/36951)

<!-- ACP2E-2028 -->

* Produtos com opções personalizáveis agora podem ser importados com sucesso. Anteriormente, os dados de opções eram perdidos durante a importação. [GitHub-37598](https://github.com/magento/magento2/issues/37598)
...
* O processo de importação do produto não é mais interrompido quando as imagens do produto estão ausentes. Anteriormente, quando uma imagem de produto estava ausente durante a importação de um serviço de terceiros usando um arquivo de CSV, Adobe Systems Comércio exibia esse erro: `Maximum error count has been reached or a system error is occurred!`.
...
* `AUTO_INCREMENT` da tabela `inventory_source_item` não aumenta mais com cada operação `UPDATE`. Anteriormente, cada atualização aumentava o `AUTO_INCREMENT` desta tabela, o que eventualmente fazia com que o valor `AUTO_INCREMENT` ficasse fora do intervalo ao adicionar um novo registro ao `inventory_source_item table`. Como resultado, administrador usuários receberam o seguinte erro quando tentaram criar um novo produto com o administrador: `Numeric value out of range...`.

<!-- ACP2E-1378 -->

* Agora, os usuários administradores podem adicionar um produto a um carrinho do cliente da Administração para um armazenamento visualização escopo com o Gerenciamento de inventário. Anteriormente, a ID de estoque não era determinada corretamente, e administrador usuários não podiam adicionar o produto ao carrinho do cliente.

<!-- ACP2E-1399 -->

* A sincronização entre Inventário e catálogo não depende mais da **[!UICONTROL Synchronize with Catalog]** configuração. Anteriormente, os produtos só eram sincronizados corretamente quando esta configuração estava ativada.

<!-- ACP2E-1382 -->

* Adobe Systems Comércio não gera mais um erro após página recarregar quando um consumidor deduz um produto quantidade de um solicitar que se enquadra na gama de produtos disponíveis ao selecionar a fonte de envio.

<!-- ACP2E-1259 -->

* O status do estoque do produto configurável é atualizado corretamente quando um produto configurável e seu valor de produtos `Stock Availability` secundários são atualizados por uma atualização inventário em massa. Anteriormente, não era possível retornar um produto configurável e seus produtos secundários para o estoque por atualização em massa.

<!-- ACP2E-2096 -->

* O tipo de dados de fontes não padrão foi atualizado para oferecer suporte a `DECIMAL (12, 4)` até oito dígitos inteiros, que é o mesmo limite implementado para o estoque padrão. Esse valor agora está em sincronizar com a administração adicionar e editar páginas de produtos, que oferecem suporte para entrada validação de até oito dígitos para todos os tipos de fontes de inventário. Anteriormente, a entrada do **[!UICONTROL Quantity]** campo (para fontes não padrão) tinha suporte somente para até seis dígitos.

<!-- ACP2E-2054 -->

* Agora, as fontes padrão e não padrão exibem as informações corretas para as quantidade saláveis do produto quando uma fonte não padrão solicitada na vitrine é processada para envio. Anteriormente, Adobe Systems Comércio exibia o status incorreto de estoque para fontes de produtos.

<!-- ACP2E-1950 -->

* `\Magento\Catalog\Model\Product::getIsSalable()` retorna o status do estoque com base no escopo definido no objeto do produto (`\Magento\Catalog\Model\Product::getStoreId()`). Anteriormente, `getIsSalable()` retornava o status do estoque com base no escopo atual, independentemente do objeto do produto.
...
* Agora, os clientes são redirecionados para o fazer logon página depois de ativar um conta conforme esperado. Anteriormente, os clientes faziam logon automaticamente.

### Galeria de mídia

<!-- ACP2E-2568 -->

* Os comerciantes agora podem upload imagens com o mesmo nome, mas com casos diferentes (uma mistura de caracteres de letras maiúsculas e minúsculas) ao Media Gallery. Anteriormente, arquivo de imagem nomes não diferenciavam maiúsculas de minúsculas em Page Builder, e arquivos com os mesmos nomes diferentes de maiúsculas e minúsculas eram substituídos Comércio mídia armazenamento que eram hospedados em sistemas de arquivos com distinção entre maiúsculas e minúsculas.

### Boletim informativo

<!-- AC-10806 -->

* Comércio agora verifica se o boletim informativo assinatura funcionalidade está ativado antes de aceitar novos assinantes em um boletim informativo. [GitHub-33040](https://github.com/magento/magento2/issues/33040)

### Ordens

<!-- AC-8857 -->

* Os produtos movidos do carrinho de compras de administração para o lista de itens para solicitar agora são excluídos conforme esperado do carrinho de compras. [GitHub-37538](https://github.com/magento/magento2/issues/37538)

<!-- ACP2E-2535 -->

* Os comerciantes agora podem criar memorandos de crédito conforme esperado para o retorno parcial de pedidos que foram pagos em parte por pontos de recompensa. Anteriormente, quando um comerciante criava um retorno parcial, solicitar status era identificado incorretamente como fechado, e o administrador solicitar página não exibia opções de memorandos de crédito.

<!-- ACP2E-2632 -->

* A rotação do log de ação de administrador agora funciona como esperado quando o arquivamento de logs de rotação frequência é definido como intervalos semanais ou mensais. Anteriormente, quando os intervalos semanais ou mensais eram definidos, os logs não eram girados.

<!-- ACP2E-2583 -->

* Comércio agora aplica a quantia de desconto correta em pedidos parcialmente cancelados. Anteriormente, o valor do desconto calculado na `base_discount_cancelled` coluna era calculado incorretamente quando uma solicitar era alterada. O suporte a valores negativos foi adicionado a faturas de pedidos com desconto.

<!-- ACP2E-2585 -->

* O terminal REST `V1/orders` agora aplica cartões de presente conforme esperado ao importar pedidos. Anteriormente, as alterações nos atributos de cartão extensão neste terminal não eram salvas.

<!-- ACP2E-2412 -->

* Comércio classifica os atributos personalizados de endereço do cliente com base na classificação solicitar fornecida quando os usuário administrador criaram os solicitar na Administração.

<!-- ACP2E-2400 -->

* A página Grade de Pedidos de Vendas do Administrador agora é carregada como esperado quando um comerciante inicia uma pesquisa a partir desse página. Anteriormente, quando um comerciante tentava pesquisa ordens desta tabela, os página congelaram e Comércio exibiam um `Request-URI Too Long` erro no console de navegador.

<!-- ACP2E-2294 -->

* O status do pedido está agora marcado como fechado em vez de concluído quando todo o valor da solicitar foi reembolsado com armazenamento crédito mais um método de pagamento adicional. Anteriormente, solicitar status era marcado como concluído porque o valor reembolsado era calculado incorretamente.

<!-- ACP2E-2213 -->

* Aprimoramento do desempenho do `OrderRepository::get()` método, reduzindo o número de vezes em que ele carrega uma solicitar do banco de dados. Anteriormente, esse método carregava uma solicitar várias vezes. [GitHub-36636](https://github.com/magento/magento2/issues/36636)

<!-- ACP2E-1918 -->

* Os comerciantes agora podem gerar memorandos de crédito para pedidos que contêm alguns itens com um total zero quando outros solicitar itens estão disponíveis para reembolso. Anteriormente, os comerciantes não podiam realizar vários reembolsos quando um cliente tinha um desconto de 100% em alguns itens no solicitar.

<!-- AC-8254 -->

* Os links entre produtos secundários e principais são exibidos na página de solicitar durante a re-solicitar do produto filho do Administrador. [GitHub-37028](https://github.com/magento/magento2/issues/37028)

<!-- AC-1756 -->

* A seção do cabeçalho da solicitar página agora contém as informações esperadas sobre a fatura enviada, memorando de crédito e envio. [GitHub-27474](https://github.com/magento/magento2/issues/27474)

<!-- AC-7632 -->

* Os metadados de crédito para pedidos que contêm apenas um produto configurável agora são gerados corretamente. Anteriormente, a `isLast()` função não retornava `true` como esperado. [GitHub-36722](https://github.com/magento/magento2/issues/36722)
...
* Os compradores não podem mais solicitar um produto usando um preço personalizado quando uma solicitar é gerada pela primeira vez pelo Administrador usando o preço personalizado único. Anteriormente, se um solicitar continha um item com preços personalizados, esse preço personalizado era aplicado em outros pedidos feitos com o re-solicitar funcionalidade.

<!-- ACP2E-1421 -->

* Os compradores não podem mais salvar status de solicitar incorretos na solicitar página. O status do pedido não é mais alterado de Todos os Apps para Processamento quando um item é enviado. Anteriormente, Adobe Systems Comércio não verificava o status atual da solicitar antes de salvar solicitar status depois que um consumidor entrou em um comentário. [GitHub-36562](https://github.com/magento/magento2/issues/36562)

<!-- ACP2E-1387 -->

* Adobe Systems Comércio não jogar mais um erro na vitrine solicitar retornar página quando o produto configurável é excluído. Anteriormente, Adobe Systems Comércio registrava esse erro: `report.CRITICAL: Error: Call to a member function getShipmentType() on null in magento2ee/app/code/Magento/Rma/view/frontend/templates/return/create.phtml:52`.

<!-- ACP2E-1367 -->

* As vendas solicitar grade agora são sincronizadas de forma assíncrona com todos os pedidos conforme esperado. [GitHub-36562](https://github.com/magento/magento2/issues/36562)

<!-- ACP2E-1197 -->

* O funcionalidade filtrar por data de compra na seção de pedidos dos clientes página agora funciona conforme o esperado. Anteriormente, um erro de JavaScript ocorria quando o fuso horário conversão lógica exibia um erro quando o mesmo fuso horário era fornecido ao método.

<!-- ACP2E-1294 -->

* O Adobe Systems Comércio painel agora exibe as estatísticas corretas dos pedidos na primeira carga. Anteriormente, o painel exibia informações incorretas solicitar, mas exibia as informações corretas depois que os período eram atualizados.

<!-- ACP2E-1762 -->

* Agora, os usuários administradores são redirecionados conforme o esperado para o solicitar página depois de selecionar um armazenamento visualização durante a criação solicitar nova do Administrador.
...
* Faturas, remessas e demonstrações de crédito agora podem ser retiradas com êxito do arquivo junto com os pedidos associados quando as IDs que incluem diferem das IDs do solicitar. Anteriormente, se um solicitar fosse movido para o arquivo e depois restaurado, depois fatura, envio, registros de memorandos de crédito estavam faltando a partir desse solicitar se os números de ID não correspondessem. [GitHub-36847](https://github.com/magento/magento2/issues/36847)
...
* Page Builder reprodução automática de elemento de vídeo agora funciona como esperado em navegadores móveis safari no iOS quando o elemento está vinculado diretamente a um arquivo de vídeo remoto. Anteriormente, nenhuma imagem pré-visualização era exibida.

<!-- ACP2E-1974 -->

* Page Builder funciona conforme o esperado na navegador de Cromo. Anteriormente, conteúdo alterações feitas naquela navegador nem sempre eram salvas.

<!-- ACP2E-2086 -->

* Carregar uma edição de produto administrador página um produto não gera mais um erro de JavaScript intermitente `Uncaught TypeError` .
...
* Os compradores agora podem selecionar um método de envio ao aplicar cupom código durante o check-out. Anteriormente sob essas condições, Comércio ocorreu um erro e atribuiu o frete de taxa única ao solicitar.
...
* A Adobe Commerce não lança mais um erro ao criar uma segunda fatura parcial para um pedido pago pela Braintree quando o **[!UICONTROL Enable Vault for Card Payments]** está habilitado. Anteriormente, quando um comerciante tentava criar uma segunda fatura, Adobe Systems Comércio exibia esse erro: `Your payment could not be taken. Please try again or use a different payment method. Payment method token is invalid`.

<!-- ACP2E-1229 -->

* A opção do método de pagamento Braintree Vault não é mais desmarcada automaticamente no fluxo de Trabalho de checkout quando um consumidor atualiza seu endereço faturamento. Anteriormente, o método de pagamento era desmarcado quando o consumidor desmarcou o **[!UICONTROL My billing and shipping address are the same]** botão.

<!-- BUNDLE-3248 -->

* Solução de problemas com o check-out com PayPal por meio de Braintree após a atualização do Adobe Systems Comércio 2.4.5-p2. [GitHub-37242](https://github.com/magento/magento2/issues/37242)
...
* Remoção do nãoutilizado `Magento\Catalog\Controller\Adminhtml\Product\Initialization\Helper` de `Magento\Catalog\Model\ProductRepository`. Anteriormente, `ProductRepository` carregava este auxiliar, que é um `Adminhtml` controlador. [GitHub-37864](https://github.com/magento/magento2/issues/37864)
...
* Os usuários administradores agora podem pesquisa por registros com emails que contenham `+` (mais) sinais do Relatório de carrinhos abandonados página. [GitHub-36560](https://github.com/magento/magento2/issues/36560)

<!-- AC-7394 -->

* Os nomes das transações do GraphQL agora são exibidos corretamente depois que Novo Relic é desabilitado do administrador Comércio. [GitHub-36112](https://github.com/magento/magento2/issues/36112)

### Opiniões

<!-- ACP2E-1998 -->

* Os compradores não podem mais enviar rapidamente duplicado avaliações de produto quando o reCAPTCHA v3 invisível está ativado.
...
* O valor do **[!UICONTROL Enable RMA]** atributo agora corresponde ao valor especificado na configuração quando a **[!UICONTROL Use Config]** caixa de seleção na página de edição do produto está ativada.

<!-- ACP2E-1230 -->

* O **[!UICONTROL Enable RMA]** atributo no nível produto individual agora respeita a configuração armazenamento de RMA. Anteriormente, essa configuração era desativada por padrão quando você adicionava um produto, independentemente das configurações armazenamento.

### Procurar {#search-heading}

<!-- ACP2E-2272 -->

* O preenchimento automático pesquisa sugestões no mini-pesquisa frontend do cabeçalho agora funcionam conforme o esperado. Anteriormente, o preenchimento automático não preencha esse campo quando esse recurso estava habilitado. [GitHub-37557](https://github.com/magento/magento2/issues/37557)
...
* A vitrine filtros funcionar como esperado ao classificar entradas de catálogo com atributos personalizados suspensos.

<!-- ACP2E-1499 -->

* Search resultados agora exibem resultados relevantes com base pesquisa peso dos atributos conforme esperado.

<!-- AC-7505 -->

* Texto informativo no `Search Weight` campo de entrada do `Storefront Properties` atributo na Administração **[!UICONTROL Stores]** > **[!UICONTROL Attributes]** > **[!UICONTROL Product]** agora descreve os possíveis valores. [GitHub-36646](https://github.com/magento/magento2/issues/36675)

### Transporte

<!-- ACP2E-2515 -->

* As regras de preço do carrinho agora funcionam como esperado quando um produto pacote é adicionado ao carrinho quando atributos de preço dinâmicos são desativados. O frete grátis e a taxa de tabela correta são aplicados independentemente do atributo dinâmico estar ativado ou desativado. Anteriormente, o frete grátis não era aplicado conforme esperado.
...
* O Adobe Commerce não lança mais um erro SQL ao criar uma alteração de agendamento para um produto enquanto salva outro produto. Anteriormente, Adobe Systems Comércio exibia esse erro: `The product with the SKU "%1" couldn't be added to the current update`.
...
* Remoção das referências a classes inexistentes nos testes de unidade. [GitHub-37636](https://github.com/magento/magento2/issues/37636)
...
* Redirecionamento `Magento\GraphQl\Catalog\ProductSearchTest::testFilterProductsBySingleCategoryId` de teste para lidar com categoria aleatórios solicitar em `products` respostas query.
...
* Correção do aviso de desaprovação para `StorefrontDeleteItemFromRequisitionListPrintView`.
...
* Agora, os usuários administradores podem alterar o logotipo da vitrine quando um único armazenamento modo é ativado. Anteriormente, Comércio exibia o logotipo de fallback padrão. [GitHub-35952](https://github.com/magento/magento2/issues/35952)

<!-- ACP2E-2045 -->

* Os valores de configuração de mixins não estão mais vazios e as misturas do jQuery agora funcionam independentemente da posição de carregamento. Anteriormente, as misturas de jQuery não eram carregadas de forma consistente nos navegadores.
...
* Adobe Systems Comércio agora exibe os preços corretos para produtos de pacote fixos com preços especiais.
...
* Adição dos `taxes`atributos , `itemized_taxes`e `additional_itemized_taxes` extensão a `Magento\Sales\Api\Data\OrderInterface`. Esses atributos permitem economizar impostos aplicados com chamadas REST do pedido.

<!-- ACP2E-2610 -->

* A API REST de preços de nível agora suporta quantidades decimais. Anteriormente, essa API não permitia quantidades decimais mesmo quando a **[!UICONTROL Use Decimal Quantity]** configuração estava ativada.

<!-- AC-8998 -->

* O ponto de extremidade REST `V1/orders/{id}/comments` não altera mais o status de um pedido ao tentar adicionar um comentário ao status de um pedido pendente. Agora dá um erro informativo. Anteriormente, esse terminal alterava o status da solicitar ao adicionar um comentário.

<!-- ACP2E-2275 -->

* A API `GET` REST e `POST V1/products/attributes` o terminal não retornam mais o mesmo valor para o `is_filterable` atributo tanto para quanto `Filterable(with results)` `Filterable(no results)` para as opções da opção **[!UICONTROL Use in Layered Navigation]** . Anteriormente, esse terminal retornava o mesmo valor porque o propriedade do atributo era do tipo, o `is_filterable` que não permitia definir esse propriedade como`Filterable(no results)`.`Boolean` [GitHub-37979](https://github.com/magento/magento2/issues/37979)

<!-- AC-9117 -->

* O campo ausente `media_gallery_entries` foi adicionado ao endpoint REST `V1/configurable-products/sku/children` . Anteriormente, essa função não tinha o código necessário para armazenamento e exibição `media_gallery_entries`. [GitHub-37607](https://github.com/magento/magento2/issues/37607)

<!-- ACP2E-1986 -->

* O método REST `V1/carts/mine/estimate-shipping-methods` agora valida todos os níveis de dados do campo de carga e retorna um código HTTP 400 para dados inválido. Anteriormente, somente o primeiro nível da carga da API era validado e, se os dados inválidos eram retornados em níveis não validados da carga, o Commerce retornava um código de status 500.
...
* O `POST /V1/products/tier-prices` terminal agora retorna uma mensagem de erro mais informativa quando um cliente da inválido grupo nome é usado durante a criação do preço de nível. Anteriormente, o Adobe Commerce retornava um erro como este: `Notice: Undefined index: XXXXX in ~/public_html/vendor/magento/module-catalog/Model/Product/Price/Validation/TierPriceValidator.php on line 474"`. [GitHub-36431](https://github.com/magento/magento2/issues/36431)

<!-- ACP2E-1776 -->

* Agora é possível executar uma API em massa assíncrona autorizada solicitação para criar clientes com uma ID de grupo não padrão. Anteriormente, essas solicitações redefiniam a ID do grupo para o valor padrão.
...
Você pode instalar o Adobe Commerce 2.4.7 usando o [Composer](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
