...
O DHL introduziu schema versão 6.2 e descontinuará schema versão 6.0 em breve. Adobe Systems Comércio 2.4.4 e versões anteriores que oferecem suporte à integração do DHL apenas na versão 6.0. Os comerciantes que implantam essas versões devem se aplicar `AC-3022.patch` o mais cedo possível para continuar oferecendo DHL como uma operadora de transporte. Consulte o [Aplicar um correção de continuar oferecendo o DHL como artigo da Knowledge Base da operadora](https://support.magento.com/hc/en-us/articles/7707818131597-Apply-a-patch-to-continue-offering-DHL-as-shipping-carrier) de transporte para obter informações sobre como baixar e instalar o correção.
...
**Novo `use_application_lock` modo de indexação**. O `use_application_lock` modo permite ativar a reindexação por meio do uso de variáveis de ambiente ou pela configuração do `app/etc/env.php` arquivo. Não é mais necessário redefinir manualmente o indexador após uma falha com esse modo ativado. Consulte [Usando o modo de bloqueio do aplicativo para processos de reindexação](https://developer.adobe.com/commerce/php/development/components/indexing/#using-application-lock-mode-for-reindex-processes).
...
* O biblioteca KnockoutJS foi atualizado para v3.5.1 (a versão mais recente). <!-- MC-40694-->

* O biblioteca TinyMCE v3 obsoleto foi removido. Os `Magento_Tinymce3Banner` testes de módulo e MFTF relacionados ao TinyMCE v3.x foram removidos de Adobe Systems Comércio. <!-- MC-42199 42170 -->

* Versão 2.4.3 foi testada e confirmada como compatível com a Redis 6.0.12. (a versão 2.4.x continua compatível com a Redis 5.x.)

* As dependências biblioteca Laminas foram atualizadas para versões compatíveis com PHP 8.x. Algumas dependências redundantes foram removidas do arquivo `composer.json`. **O Adobe Commerce 2.4.3 usa o Laminas 3.4.0**. <!-- MC-39513-->
...
* **Listas de desejos**. A [mutação](https://developer.adobe.com/commerce/webapi/graphql/schema/wishlist/mutations/add-items-to-cart/) addWishlistItemsToCart move itens do desejo especificado lista para o carrinho do cliente. <!-- magento/partners-magento2ee#449-->

* **Registros de presentes**. As tarefas abordadas incluem: <!-- PWA-1290-->

   * Use o [query](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/queries/gift-registry/) giftRegistry para retornar o conteúdo dos registros de presentes do cliente.

   * Search para registros de presentes por [tipo](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/queries/type-search/), [email](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/queries/email-search/) ou [ID](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/queries/id-search/).

   * [&#128279;](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/create/)Criar, [atualize](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/update/) ou [exclua](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/remove/) registros de presentes.

   * [Atualize](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/update-items/) ou [remova](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/remove-items/) itens de um registro de presente.

   * [Mova todos os itens](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/move-cart-items/) do carrinho para o registro de presentes. <!-- ENGCOM-9082 9087-->

   * [Adicione](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/add-registrants/), [atualize](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/update-registrants/) ou [remova](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/remove-registrants/) os registrantes de um registro de presente.

   * [Compartilhe](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/share/) um registro de presentes com convites.

* **Aspas** negociáveis. Consulte o e [`negotiableQuotes`](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/negotiable-quote/queries/quotes/) as [`negotiableQuote`](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/negotiable-quote/queries/quote/) consultas.<!-- PWA-1292-->
...
* O aplicativo agora exibe uma mensagem informativa e uma link às Lojas **&#x200B;**&#x200B;de administração > **>** de configuração **Avançado** > **System** página no Search para Adobe Stock página em **que a Chave da API (ID do cliente)** e o **Segredo** do cliente não são definidos. Anteriormente, o aplicativo exibia esse erro: `We couldn't find any records` e nenhuma link.
...
* O `varnish6.vcl` arquivo foi atualizado para ignorar armazenamento em cache do página do cliente.
...
* O CAPTCHA agora funciona conforme esperado na página de finalização. Anteriormente, depois que um consumidor respondia corretamente a um desafio captcha, o loader na finalização página nunca concluído e o aplicativo exibia esse erro: `captchaData[formId] is undefined`. (Este erro ocorria somente quando o consumidor usado o mesmo navegador do qual haviam acessado anteriormente uma implantação executada 2.3.5-p1.)

<!-- ENGCOM-8630-->

* `_.isEmpty()`verificações no `defaultCaptcha.js` arquivo agora concluídas com êxito. Anteriormente, essas verificações não eram concluídas e, como resultado, o check-out página falhava ao carregar após a atualização. [GitHub-31641](https://github.com/magento/magento2/issues/31641)
...
* Os links para registros de presentes agora persistem conforme esperado ao editar um produto no carrinho de compras. Anteriormente, esses links desapareciam quando você clicava no **botão do Carrinho** de atualização.
...
* Os usuários administradores agora podem ver espaços duplos nos campos **Nome** e **SKU** na grade de produtos. Anteriormente, o applicationcollapsed vários espaços em um único espaço.

<!-- MC-39869-->

* Os produtos agora são exibidos como fora de estoque na vitrine quando as quantidade saláveis no Administrador são 0. Anteriormente, esses produtos eram listados como em estoque na vitrine e os aplicativo exibiam um Add to carrinho **botão ativo**. [GitHub-31117](https://github.com/magento/magento2/issues/31117)

<!-- MC-39809-->

* Os administradores agora podem adicionar produtos com opções `(File)` personalizáveis à grade de Itens solicitados na seção carrinho de compras (coluna Atividades do cliente) do página do cliente administrador. Anteriormente, o aplicativo não adicionava o item à lista porque o valor não era formatado corretamente antes de ser inserido `\Magento\Catalog\Model\Product\Type\AbstractType::_prepareOptions`.

<!-- MC-39619-->

* A aplicativo não solicita mais que os compradores selecionem uma opção de produto para um produto empacotado que tenha apenas uma opção.

<!-- MC-39617-->

* O aplicativo exibe todas as subcategorias em atualizações de layout (categorias âncora e não âncora) durante a criação de um novo widget.

<!-- MC-39481-->

* A `product` query não substitui mais os valores padrão de todas as exibições armazenamento em implantação multi-armazenamento quando um nome de produto é atualizado por um armazenamento visualização somente. [GitHub-31083](https://github.com/magento/magento2/issues/31083)
...
* Os compradores agora podem adicionar produtos configuráveis às suas carrinho a partir de um visualização de armazenamento não padrão. Anteriormente, quando o comprador em um modo de exibição de loja não padrão tentava adicionar um produto configurável, o aplicativo exibia este erro: `Could not add item to cart. Please check required options and try again`. [GitHub-31660](https://github.com/magento/magento2/issues/31660)
...
* O aplicativo agora registra um erro conforme esperado quando ocorre uma exceção, à medida que um cliente tenta enviar um email do formulário Contact Us. [GitHub-23645](https://github.com/magento/magento2/issues/23645)

### Estruturas

<!-- MC-41712-->

* As classes pai no Admin agora são verificadas em busca de docblock anotação junto com a classe original e interfaces herdadas. Anteriormente, como as classes principais não eram verificadas, executar ações relacionadas ao cliente no Administrador que acionavam um evento resultava em um erro. O aplicativo registrou este erro no log de exceções: `report.CRITICAL: Method's return type must be specified using @return annotation.`.
...
* A aplicativo agora converte todas as cadeias de caracteres conversíveis conforme esperado para o página de carrinho de administração. Anteriormente, o carregamento da tradução acontecia tarde demais e ignorava todos os observadores que estavam inscritos na ação do controlador antes do envio. [GitHub-31849](https://github.com/magento/magento2/issues/31849)

<!-- MC-39776-->

* O validação de formulário na Criar Novo conta do cliente página funciona com sucesso quando a **configuração Logon como Cliente** habilitar a extensão é desabilitada. Anteriormente, o aplicativo exibia um erro de JavaScript.

<!-- MC-39537-->

* O uso `bin/magento setup:db-schema:split-sales` para mover tabelas de banco de dados agora funciona conforme o esperado. Tables são criadas e preenchidas com dados corretamente. Anteriormente, como a verificação da existência da tabela estava incorreta, a aplicativo ocorreu `SQLSTATE` erros quando você correu `bin/magento setup:db-schema:split-sales`.

<!-- MC-39193-->

* O tratamento de exceção para processos secundários bifurcionados por `ProcessManager` foi aprimorado. Quando ocorre uma exceção agora, o processo principal é fechado e uma mensagem de erro é exibida apenas uma vez. Anteriormente, várias falhas do indexador eram registradas e várias mensagens eram exibidas. [GitHub-30622](https://github.com/magento/magento2/issues/30622)

<!-- MC-39132-->

* Plug-ins de Adobe Systems Comércio globais (por exemplo, `webapi_rest` e `graphql` ) não são mais acionados para um novo tipo de área personalizada quando o `di.xml` arquivo dessa área não contém plug-ins registrados.
...
* O aplicativo agora exibe o número correto de estrelas no página do My Product Resenhas e no MyAccount página comentários recentes. Anteriormente, o aplicativo aplicava estrelas de revisão apenas à primeira revisão e deixava as outras revisões na página sem estrelas.
...
* O aplicativo não exibe mais produtos desativados nos registros de presentes. Anteriormente, se um produto estivesse desativado após ser adicionado a um registro de presente, os aplicativo exibiam essa mensagem quando um consumidor acessado o registro: `Error: Call to a member function getId() …`. [GitHub-510](https://github.com/magento/partners-magento2ee/issues/510)
...
* A `products` query não expõe mais o preço especial de um produto quando o período de preço especial é definido para uma data futura. [GitHub-30210](https://github.com/magento/magento2/issues/30210), [GitHub-29631](https://github.com/magento/magento2/issues/29631)
...
* Agora é possível definir os atributos do pacote required_options e has_options conforme esperado ao criar ou atualizar um produto do pacote usando o ponto de extremidade POST `/V1/product/:sku`. Anteriormente, esses atributos personalizados eram definidos como 0 (zero), apesar dos esforços para defini-lo como 1 (um).

### Importar/exportação

<!-- MC-41576-->

* O filtro de IDs de Categoria para entidades de produtos na grade de Atributos de página de entidade de exportação agora funciona conforme o esperado.

<!-- MC-41439 ENGCOM-9005-->

* O status do estoque de produtos agrupados agora é atualizado conforme o esperado para ficar em estoque quando todos os produtos secundários estão sem estoque. [GitHub-32647](https://github.com/magento/magento2/issues/32647)

<!-- MC-41184-->

* O status do estoque de produtos configuráveis agora é atualizado automaticamente conforme esperado, quando o status do estoque do produto filho é atualizado por importação. Anteriormente, o status do estoque de produtos não era atualizado automaticamente quando o status do estoque de produtos secundários era atualizado por importação.
...
* Os administradores agora podem alterar com êxito o `name` de um pacote produto `bundle_values` do Administrador. Anteriormente, o aplicativo exibia o produto como em estoque na vitrine depois que você alterou o nome do Administrador. Os produtos também foram mesclados com a mesma SKU em uma única seção pacote opção.

<!-- MC-40333-->

* O aplicativo agora entra em conta usuário escopo ao exportar dados de clientes. Anteriormente, quando tentava exportar clientes, o aplicativo exportava os dados do cliente de todos os sites.

<!-- MC-40305-->

* O aplicativo agora remove os relacionamentos de produtos entre produtos de venda ativa, venda cruzada e produtos relacionados durante CSV importação de arquivos conforme esperado, quando `__EMPTY__VALUE__` é especificado no arquivo de CSV.

<!-- MC-40012-->

* A exportação de dados de endereço personalizados para muitos clientes (Admin **Sistema** > **Exportar**) não resulta mais em um erro de memória de rotina. Anteriormente, ao exportar dados de endereços personalizados, o aplicativo tentou carregar todos os dados do cliente, o que resultou em esgotamento da memória, e o aplicativo ocorreu um erro de falha na alocação de memória.

<!-- MC-39977-->

* Preços duplicados de nível não são mais importados durante o processo de importação CSV padrão. Anteriormente, faltava validação para impedir a importação de preços duplicado nível, e quando ocorreram preços duplicado nível, os comerciantes não conseguiam salvar produtos. Os comerciantes também viram esse erro quando tentaram programar uma mudança de produto: `SQLSTATE[23000]: Integrity constraint violation: 1062 Duplicate entry…`.

<!-- MC-39609  magento/magento2#31397-->

* Todas as imagens do produto agora são validadas durante a importação. Anteriormente, o aplicativo validava apenas a primeira imagem quando um produto tinha várias imagens. [GitHub-28236](https://github.com/magento/magento2/issues/28236)
...
* O `ArrayIterator` objeto PHP foi atualizado para funcionar conforme o esperado com o PHP 7.4. [GitHub-32088](https://github.com/magento/magento2/issues/32088)

<!-- ENGCOM-8469 -->

* O aplicativo não gera mais um erro quando um cliente tenta completar uma solicitar quando nenhuma operadora de transporte está disponível. Em vez disso, ele exibe a página de check-out e esta mensagem: `Sorry, no quotes are available for this order at this time`. . Anteriormente, o aplicativo exibia uma página de check-out em branco e gravava essa mensagem no log de exceção: `array_keys() expects parameter 1 to be array, null given`. [GitHub-30830](https://github.com/magento/magento2/issues/30830)
...
* O aplicativo agora atende às configurações de habilitação do informativo (**Lojas** > **Configurações** > **Configuração** > **Clientes** > **Informativo** > **Opções Gerais**). Anteriormente, essas configurações sempre eram recuperadas do escopo padrão em implantações em várias armazenamento. [GitHub-31188](https://github.com/magento/magento2/issues/31188)
...
* A aplicativo agora salva uma solicitar modificada conforme esperado quando salva um saldo de cliente reembolsado. [GitHub-393](https://github.com/magento/partners-magento2ee/issues/393)

### Métodos de pagamento

<!-- MC-41525-->

* O aplicativo agora renderiza blocos de pagamento independentemente `frontend` da área a partir da qual o email foi enviado. (A área atual agora é emulada como `frontend` antes da renderização do bloco de pagamento.) Anteriormente, os blocos de pagamento eram renderizados na área de onde o email era enviado. Como resultado, independentemente de o email de vendas ter sido acionado pelo Administrador ou pela API REST, as URLs de ativos tentaram carregá-los da área errada (`webapi_rest` ou `adminhtml`).
...
* Os métodos de pagamento lista agora são atualizados como esperado quando um convidado consumidor muda os endereço de entrega de um solicitar para um país diferente durante o check-out. Anteriormente, a alteração do endereço de faturamento não acionava uma atualização dos métodos de pagamento possíveis.
...
* O desempenho do Admin SKU pesquisa em catálogos grandes melhorou. Dicas do otimizador de consulta agora forçam o uso do índice durante query execução.

<!-- MC-41440-->

* O desempenho do `catalog_product_alert` `cron` processo em execução em tabelas grandes (vários milhões de linhas) foi aprimorado. Anteriormente, `catalog_product_alert` todos os alertas do produto eram carregados, o que causava uma exceção de falta de memória.

<!-- ENGCOM-8752-->

* A aplicativo não carrega mais todas as páginas de CMS quando precisa de apenas uma edição página para renderizar um formulário de administrador.  Agora, essas páginas são carregadas com mais rapidez. [GitHub-30936](https://github.com/magento/magento2/issues/30936)

<!-- ENGCOM-8510-->

* Valores numéricos em `WHERE IN` expressões agora são lançados como número, não como sequência de caracteres, o que melhora query desempenho em algumas versões do MariaDB. [GitHub-31135](https://github.com/magento/magento2/issues/31135)

* O `use_application_lock` modo permite ativar a reindexação por meio do uso de variáveis de ambiente ou pela configuração do `app/etc/env.php` arquivo. Não é mais necessário redefinir o indexador manualmente após a falha com esse modo ativado. Quando este modo não está habilitado, você deve redefinir manualmente o indexador após a falha. Consulte [Usar aplicativo modo de bloqueio para processos](https://developer.adobe.com/commerce/php/development/components/indexing/#using-application-lock-mode-for-reindex-processes) de reindexação.

### Precificação

<!-- MC-41767-->

* O aplicativo agora atualiza corretamente o preço de um produto com preços agrupados quando um consumidor atualiza quantidade de produtos na vitrine. [GitHub-32669](https://github.com/magento/magento2/issues/32669)
...
* O detalhe de pedido de vitrine página agora exibe a quantidade correta do produto enviado. Anteriormente, as quantidades de produtos estavam incorretas porque o modelo para o Envio de pedido página Qty **Shipped** como um `int`. Isso foi alterado para `float`.

<!-- MC-39026-->

* O aplicativo não envia mais cópias de emails de uma fatura do solicitar quando a **Cópia de email da botão de fatura** está desmarcada. Anteriormente, o aplicativo enviou um email para **Cliente** e **Enviar cópia de email de fatura para**. [GitHub-28511](https://github.com/magento/magento2/issues/28511)
...
* Agora os desenvolvedores podem alterar mapeamentos Elasticsearch. Anteriormente, os modelos dinâmicos enviados ao servidor do Elasticsearch nos mapeamentos padrão eram codificados, o que impedia os desenvolvedores de indexar no Elasticsearch dados extras associados a um módulo personalizado. [GitHub-24363](https://github.com/magento/magento2/issues/24363)
...
* O aplicativo agora exibe o preço de envio ajustado correto quando alguns itens no carrinho se qualificam para frete gratuito. Anteriormente, quando um subconjunto de itens no carrinho se qualificava para grátis envio, o aplicativo não ajustava o preço de envio e exibia o preço total de envio ao consumidor.

<!-- MC-41186-->

* Os administradores restritos a um site específico agora podem criar um carregamento para um solicitar colocado no mesmo site. Anteriormente, o aplicativo lançou essa exceção quando um administrador que não tinha permissão à armazenamento padrão visualização tentou enviar um solicitar que foi colocado em uma armazenamento visualização ao qual o administrador tinha acesso: `Notice: Undefined offset: 1 in /app/code/Magento/Catalog/Model/Product/Attribute/Backend/GroupPrice/AbstractGroupPrice.php on line 293`.

<!-- MC-41069-->

* A aplicativo agora leva em conta descontos relevantes carrinho de preço regra ao determinar se um solicitar atende às condições para grátis envio de DHL.
...
* O aplicativo não gera mais um erro quando um comerciante tenta enviar um solicitar usando DHL quando a **caixa de seleção Criar etiqueta** de envio está ativada e o nome do produto contém caracteres unicode. Anteriormente, o aplicativo exibia esse erro ao solicitar a criação do rótulo: `The response is in wrong format`. [GitHub-31032](https://github.com/magento/magento2/issues/31032)

<!-- ENGCOM-8952 -->

* Você pode colocar um solicitar do Administrador em um implantação de vários sites no qual `United States` é ativado em um site e`Disable all countries` é ativado como o escopo padrão no outro site. Anteriormente, o aplicativo não colocava a solicitar e exibia esse erro: `Please check the shipping address information. "regionId" is required. Enter and try again`. [GitHub-30577](https://github.com/magento/magento2/issues/30577)
...
* As cadeias de texto no modelo usadas para gerenciar métodos de pagamento armazenados (**Minha conta**  >  **Métodos** de pagamento armazenado ) agora são traduzidas. [GitHub-31211](https://github.com/magento/magento2/issues/31211)

<!-- MC-30798 ENGCOM-8717-->

* Emails de pedido enviados do administrador agora usam a localidade armazenamento não a localidade associada ao conta do administrador. [GitHub-26521](https://github.com/magento/magento2/issues/26521)
...
* A paginação para fontes agora está presente como esperado durante a criação do carregamento do administrador.
...
* As anomalias com a exibição do carrinho de compras quando ampliadas foram resolvidas. Anteriormente, os elementos de exibição se sobrepunham quando essa página era ampliada.
...
* O aplicativo agora exibe um valor de hora correto quando a opção `timeOnly` do componente `datetime` está definida como **sim**. [GitHub-23157](https://github.com/magento/magento2/issues/23157)
...
* As reescritas do URL do produto para um site específico em um implantação de vários sites agora são geradas como esperado depois que os produtos são atribuídos a um site por atualização em massa.
...
É possível instalar Adobe Systems Comércio 2.4.3 usando [o Composer](../../../installation/composer.md).

## Kits de ferramenta migração
