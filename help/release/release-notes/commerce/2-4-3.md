...
Esta versão inclui mais de 370 novas correções para código principal e 33 melhorias de segurança. Todos os problemas conhecidos identificados nas notas de versão do Adobe Commerce 2.4.2 foram corrigidos nesta versão.
...
* As dependências da biblioteca Laminas foram atualizadas para versões compatíveis com PHP 8.x. Algumas dependências redundantes foram removidas do `composer.json` arquivo. **O Adobe Commerce 2.4.3 usa o Laminas 3.4.0**. <!-- MC-39513-->
...
[Live Search](https://docs.magento.com/user-guide/live-search/overview.html) alimentado por [Adobe Sensei](https://www.adobe.com/sensei.html) O oferece uma experiência de pesquisa intuitiva usando inteligência artificial e algoritmos de aprendizado automatizado para realizar uma análise profunda dos dados agregados do visitante. Consulte [Notas de versão do Live Search](https://experienceleague.adobe.com/docs/commerce-merchant-services/live-search/release-notes.html).

### GraphQL

Esta versão adiciona suporte GraphQL para os seguintes recursos:

* **Catálogos compartilhados**<!-- PWA-1294-->

* **Listas de desejos**. O [mutação addWishlistItemsToCart](https://devdocs.magento.com/guides/v2.4/graphql/mutations/add-wishlist-items-to-cart.html) move itens da lista de desejos especificada para o carrinho do cliente. <!-- magento/partners-magento2ee#449-->

* **Registros de presentes**. As tarefas abrangidas incluem: <!-- PWA-1290-->

   * Use o [consulta de presenteRegistro](https://devdocs.magento.com/guides/v2.4/graphql/queries/gift-registry.html) para devolver o conteúdo dos registros de presentes do cliente.
   ...
   * [Atualizar](https://devdocs.magento.com/guides/v2.4/graphql/mutations/update-gift-registry-items.html) ou [remove](https://devdocs.magento.com/guides/v2.4/graphql/mutations/remove-gift-registry-items.html) itens de um registro de presentes.

   * [Mover todos os itens](https://devdocs.magento.com/guides/v2.4/graphql/mutations/move-cart-items-to-gift-registry.html) do carrinho ao registro do presente. <!-- ENGCOM-9082 9087-->

   * [Adicionar](https://devdocs.magento.com/guides/v2.4/graphql/mutations/add-gift-registry-registrants.html), [atualizar](https://devdocs.magento.com/guides/v2.4/graphql/mutations/update-gift-registry-registrants.html)ou [remove](https://devdocs.magento.com/guides/v2.4/graphql/mutations/remove-gift-registry-registrants.html) registrantes de um registro de presentes.
...
* **Aspas negociáveis**. Consulte a [`negotiableQuote`](https://devdocs.magento.com/guides/v2.4/graphql/queries/negotiable-quote.html) e [`negotiableQuotes`](https://devdocs.magento.com/guides/v2.4/graphql/queries/negotiable-quotes.html) consultas. <!-- PWA-1292-->
...
A versão 2.4.3 apresenta B2B v1.3.2. Essa versão inclui várias correções de erros. Consulte [Notas de versão B2B](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html).
...
Esta versão inclui aprimoramentos no suporte ao Amazon Simple Storage Service (AWS S3) e aos serviços gerenciados em nuvem Amazon Aurora. Ele oferece suporte certificado para AWS ElastiCache, AWS Elasticsearch e AWS Managed Queues (Rabbit MQ). (Testamos a funcionalidade, o desempenho e a integração desses serviços com a Adobe Commerce.)
...
* [Amazon Pay](https://docs.magento.com/user-guide/payment/amazon-pay.html). O Amazon Pay foi descontinuado e será removido em uma versão 2.4.x posterior. A versão 2.4.3 e superior conterá apenas atualizações para compatibilidade e correções para erros principais.
...
* As regras de preço do carrinho criadas anteriormente agora são exibidas na página do painel Preparação de conteúdo depois que uma implantação é atualizada do Magento Open Source para o Adobe Commerce.
...
* O GWS administrador agora usa `int` para `website_id` Condição SQL em Coleções de administração para administradores com permissões personalizadas.
...
* GraphQL `setGuestEmailOnCart` agora, a mutação atualiza corretamente o email de convidado. Anteriormente, as tabelas de endereço de cotação e cotação não eram atualizadas.
...
* Os dados do produto do pacote que estavam faltando anteriormente agora estão incluídos no processo de preparo. Isso soluciona inconsistências no comportamento do produto quando os compradores compraram um produto de pacote na página de listagem de produtos, em vez de adicioná-lo diretamente de uma página de produto.
...
* Os pedidos não omitem mais um nome de cliente fornecido com um endereço de entrega. Anteriormente, os nomes eram omitidos porque a variável `same_as_billing` O sinalizador não foi salvo no banco de dados.

<!-- MC-40946-->

* Os links para os registros de presentes agora persistem como esperado ao editar um produto no carrinho de compras. Anteriormente, esses links desapareciam quando você clicava no **Atualizar carrinho** botão.

<!-- MC-40696-->

* Todas as mensagens da fila para o consumidor `quoteItemCleaner` agora altere seu status para `complete` conforme esperado após a exclusão de vários produtos. Anteriormente, somente uma mensagem para esse consumidor alterava seu status para `complete`e o restante alterou o status para `in progress`.
...
* Agora você é redirecionado para a página de check-out conforme esperado após adicionar um produto de pacote ao carrinho por uma pré-visualização Schedule Update e clicar no carrinho. [GitHub-447](https://github.com/magento/partners-magento2ee/issues/447)
...
* O aplicativo agora esvazia o carrinho de compras conforme esperado depois que um administrador conclui um pedido do Administrador criado por um comprador na loja. Anteriormente, quando o cliente se conectava depois que o administrador concluía o pedido, o carrinho de loja ainda continha o conteúdo do pedido. [GitHub-30262](https://github.com/magento/magento2/issues/30262)
...
* Agora é possível duplicar com êxito um catálogo compartilhado que contém SKUs somente numéricas. Anteriormente, o aplicativo exibia um erro ao tentar duplicar um catálogo compartilhado porque a função `\Magento\Catalog\Model\ProductIdLocator` A classe não funcionava corretamente com SKUs somente numéricas.
...
* O Adobe Commerce atualiza a contagem total de páginas conforme esperado quando você altera o valor por página da lista de Produtos relacionados a administração, Vendas adicionais e Vendas cruzadas. [GitHub-31059](https://github.com/magento/magento2/issues/31059)
...
* Agora você pode atribuir uma nova página a vários nós a partir da guia Editar hierarquia de página do CMS. Anteriormente, ocorria uma violação de restrição exclusiva ao tentar atribuir a página a um nó. [GitHub-363](https://github.com/magento/partners-magento2ee/issues/363)
...
* O aplicativo agora exibe corretamente os atributos personalizados de endereço do cliente nas páginas de ordem de loja e de Administração. Anteriormente, a opção selecionada do atributo suspenso estava ausente na seção Address information e o valor do atributo de entrada continha o código do atributo. [GitHub-508](https://github.com/magento/partners-magento2ee/issues/508)
...
* O email de crédito de armazenamento agora leva em conta o escopo selecionado e é enviado do endereço de email correto.
...
* O aplicativo agora exibe produtos relacionados com base nos segmentos do cliente na loja, como esperado. Anteriormente, o aplicativo não exibia essa seção quando uma regra de produtos relacionada era criada para segmentos específicos do cliente.

<!-- MC-40503-->

* Blocos dinâmicos agora são mostrados para clientes registrados em seus carrinhos de compras quando o segmento do cliente correspondente é aplicado para convidados e clientes registrados. Anteriormente, o aplicativo exibia o bloco somente para convidados.

<!-- MC-40385-->

* Os segmentos do cliente agora são atualizados automaticamente após um pedido ser feito pelo Administrador. Anteriormente, depois de criar um pedido de Administrador, o usuário Admin tinha que atualizar manualmente os dados do segmento relacionado navegando até segmentos de clientes relacionados e clicando no link **Atualizar dados do segmento** botão.

<!-- MC-39490-->

* O aplicativo agora exibe blocos dinâmicos no carrinho de compras para todos os clientes nos segmentos relevantes do cliente. Anteriormente, os convidados não visualizavam um bloco mesmo quando o segmento do cliente incluía convidados e clientes registrados.

<!-- MC-39464-->

* Problemas de desempenho do banco de dados que resultam das regras de segmentação do cliente usando o `Product was Ordered` foi resolvida.

<!-- MC-38895-->

* Um observador de eventos do segmento do cliente para a API REST/SOAP foi adicionado. Como resultado, os segmentos de clientes agora são atualizados automaticamente conforme esperado se os critérios definidos nos segmentos forem correspondidos quando os pedidos forem feitos por meio da API REST/SOAP.

<!-- MC-38346-->

* Um processador de eventos de segmento do cliente foi adicionado para solicitações GraphQL. Anteriormente, os descontos de regras de preço do carrinho não eram aplicados ao carrinho de compras quando um segmento de cliente era usado em uma condição de regra de preço do carrinho. [GitHub-371](https://github.com/magento/partners-magento2ee/issues/371)
...
* Os blocos dinâmicos agora são exibidos somente na página especificada. Anteriormente, o aplicativo exibia todos os banners que eram atribuídos ao widget em todas as páginas se nenhum bloco dinâmico fosse especificado.
...
* A mensagem de email que o aplicativo envia quando você compartilha um registro de presente do Administrador agora contém um link válido para o local do Registro. Anteriormente, tomar esse link resultava em um erro 404. [GitHub-504](https://github.com/magento/partners-magento2ee/issues/504)
...
* Usando `bin/magento setup:db-schema:split-sales` mover tabelas de banco de dados agora funciona como esperado. As tabelas agora são criadas e preenchidas com os dados corretamente. Anteriormente, como a verificação da existência da tabela estava incorreta, o aplicativo exibia `SQLSTATE` erros ao executar `bin/magento setup:db-schema:split-sales`.
...
* Plug-ins globais do Adobe Commerce (por exemplo, `webapi_rest` e `graphql` ) não são mais acionadas para um novo tipo de área personalizada quando a variável `di.xml` deste arquivo de área não contém plug-ins registrados.
...
* O aplicativo agora salva datas de evento do catálogo em fusos horários UTC. Anteriormente, o aplicativo salvava datas de evento do catálogo no fuso horário do usuário.
...
* O aplicativo agora exibe uma mensagem precisa e informativa ao clicar em **Gerenciar itens** na página Registro do presente da minha conta e os produtos do registro estão esgotados. Anteriormente, o aplicativo exibia esse erro: `Error: Call to a member function getId() …`. [GitHub-490](https://github.com/magento/partners-magento2ee/issues/490)
...
* Os encargos de pagamento pendentes agora são removidos de um cartão-presente, conforme esperado, quando um cartão-presente é cancelado. Anteriormente, o aplicativo exibia um erro quando `cron` executado depois que o cartão-presente foi excluído.

<!-- MC-37658-->

* Os compradores agora podem remover cartões-presente de um pedido na página Revisar pedido ao verificar vários endereços. Anteriormente, o aplicativo não removia o cartão-presente do pedido quando o comprador selecionava o cartão e clicava **Remover**.

<!-- MC-34407-->

* O aplicativo agora exibe o novo preço de um cartão-presente no carrinho de compras quando você altera o valor do cartão-presente depois de adicioná-lo ao carrinho.
...
* O `setGiftOptionsOnCart` agora, a mutação cria corretamente mensagens-presente. [GitHub-388](https://github.com/magento/partners-magento2ee/issues/388)

### Registro do presente

<!-- MC-33057 magento/partners-magento2ee#467-->

* **Data do evento** agora são os mesmos na loja e no Administrador durante a criação ou edição do registro de presentes. Anteriormente, cada vez que você editava e salvava o registro do presente, o aplicativo ajustava o valor **Data do evento** um dia depois, na loja e no administrador. [GitHub-466](https://github.com/magento/partners-magento2ee/issues/466)

<!-- MC-32587  magento/partners-magento2ee#511-->

* O aplicativo não exibe mais os produtos desativados nos registros de presentes. Anteriormente, se um produto era desativado após ser adicionado a um registro de oferta, o aplicativo exibia essa mensagem quando um comprador acessava o registro: `Error: Call to a member function getId() …`. [GitHub-510](https://github.com/magento/partners-magento2ee/issues/510)

### Quebra de presente

<!-- MC-38260-->

* A tradução de rótulos de quebra automática de oferta para exibições de loja agora funciona conforme o esperado.

<!-- MC-36749-->

* O encapsulamento de presente agora é aplicado conforme esperado para pedidos enviados para vários endereços.

<!-- MC-34820-->

* O aplicativo agora remove os encargos de embalagem do presente do carrinho de compras quando todos os produtos foram removidos.

<!-- MC-32259 magento/partners-magento2ee#500-->

* Os totais de Aviso de Crédito agora estão corretos quando o aviso de crédito contém encargos de produto de quebra automática e configuráveis. Anteriormente, a quantia do imposto de quebra automática nem sempre era incluída no total do aviso de crédito. [GitHub-34](https://github.com/magento/partners-magento2ee/issues/34)

### Google Analytics

<!-- ENGCOM-8450-->

* O aplicativo não gera mais um erro quando um administrador tenta carregar uma página na loja quando o Google Analytics está ativado. Uma entrada CSP para `connect-src` foi adicionado para permitir solicitações de AJAX para www.google-analytics.com. [GitHub-30880](https://github.com/magento/magento2/issues/30880)
...
* GraphQL `products` agora retorna opções de atributo que são classificadas na mesma ordem de classificação usada na página de edição de atributo.

<!-- MC-41473-->

* A resposta ao `{ category(id: 2){ children { name children { name } } } }` A solicitação GraphQL agora inclui uma árvore de categoria classificada corretamente.
...
* Solicitações vazias para GraphQL agora exibem o código de resposta 200 em vez de 500. Anteriormente, o analisador GraphQL exibia uma exceção antes do resultado da consulta ser gerado.
...
* Os produtos desativados não são mais incluídos na resposta GraphQL quando GraphQL é usado para vincular produtos de venda adicional.
...
* GraphQL `product` query agora retorna os preços corretos do grupo de clientes.

<!-- MC-38822-->

* GraphQL `products` a resposta da consulta agora classifica agregações de acordo com a posição do atributo do produto.

<!-- ENGCOM-8462-->

* O aplicativo não gera mais erros do tipo durante consultas GraphQL quando os sufixos de URL de produto e categoria contêm valores nulos. [GitHub-30909](https://github.com/magento/magento2/issues/30909)
...
* O desempenho do `catalog_product_alert` `cron` o processo ao executar em tabelas grandes (vários milhões de linhas) foi aprimorado. Anteriormente, `catalog_product_alert` carregou todos os alertas do produto, o que causou uma exceção de falta de memória.
...
* Todos os relatórios do sistema agora podem ser visualizados e baixados do projeto Adobe Commerce on cloud Infrastructure. Anteriormente, não era possível baixar relatórios do sistema para Logs, Data, Design ou Lojas.
...
* RMA `resolution` não é mais exibido como obrigatório quando o valor do campo de atributo `required` está definida como **não**.

<!-- MC-41808-->

* Agora você pode selecionar vários pedidos para devolução pelo Administrador. Anteriormente, o aplicativo exibia um erro ao clicar **Enviar** depois de selecionar várias retornos e selecionar Fechamento no menu suspenso Ações .

<!-- MC-41759-->

* O aplicativo agora envia uma notificação por email aos clientes, conforme esperado, depois que você altera o status de um retorno usando a REST API.

<!-- MC-39981  magento/partners-magento2ee#442 -->

* O aplicativo agora filtra registros de acordo com valores de filtro na grade de retornos do cliente administrador . Anteriormente, o aplicativo direcionava você à página Todos os clientes administradores e clicando no botão **Redefinir filtro** também é redirecionado para essa página. [GitHub-436](https://github.com/magento/partners-magento2ee/issues/436)
...
* Os comerciantes agora podem cancelar um pedido feito total ou parcialmente usando pontos de recompensa depois que o cliente que fez o pedido tiver sido removido do sistema.

<!-- MC-40037-->

* Os comerciantes agora podem remover pontos de recompensa das regras de preço do carrinho. Anteriormente, quando um comerciante tentava definir **Pontos de premiação** como 0 ao criar uma regra de preço do carrinho, o valor anterior para a quantidade do ponto de recompensa persistiu.

<!-- MC-39890-->

* Os pontos de recompensa para revisões de produtos agora estão associados ao site relevante quando várias revisões são aprovadas pelo Administrador **Marketing** > **Revisões pendentes** grade. Anteriormente, os pontos de recompensa não eram exibidos na loja e não eram vinculados ao site relevante no Administrador.
...
* Os administradores que usam o Safari agora podem adicionar um produto a um pedido do Administrador com êxito. Anteriormente, quando o administrador clicava no **Adicionar produtos selecionados ao pedido** , o aplicativo exibia o ícone de carregamento giratório e a página suspensa. [GitHub-30265](https://github.com/magento/magento2/issues/30265)
...
* GraphQL `cart` agora retorna o total geral correto para a etapa de faturamento de um carrinho quando um cupom é aplicado ao pedido.
...
* Agora é possível adicionar um mapeador de campo Elasticsearch personalizado para `Magento\Elasticsearch\Model\Adapter\FieldMapper\Product\FieldProvider\FieldName\Resolver\CompositeResolver`. [GitHub-24350](https://github.com/magento/magento2/issues/24350)
...
* Os campos de pesquisa (pesquisa de formulário) agora funcionam como esperado quando as Sugestões de Pesquisa estão desativadas. [GitHub-30584](https://github.com/magento/magento2/issues/30584)
...
* Os administradores que são restritos a um site específico agora podem criar uma remessa para um pedido colocado no mesmo site. Anteriormente, o aplicativo exibia essa exceção quando um administrador que não possuía permissão para exibição de loja padrão tentava enviar um pedido colocado em uma exibição de loja ao qual o administrador tinha acesso: `Notice: Undefined offset: 1 in /app/code/Magento/Catalog/Model/Product/Attribute/Backend/GroupPrice/AbstractGroupPrice.php on line 293`.
...
* O aplicativo não desmarca mais a caixa de seleção **Anexar comentários** caixa de seleção quando um comprador clicar **Obter métodos de envio e taxas** e seleciona um método de envio ao criar um pedido pelo Administrador. [GitHub-30256](https://github.com/magento/magento2/issues/30256)
...
* A quantidade do produto agora permanece inalterada como esperado depois que um comprador altera a quantidade na página de endereço múltiplo de Entrega e clica no botão Voltar do navegador. [GitHub-31956](https://github.com/magento/magento2/issues/31956)
...
* O recurso Agendar atualização de bloco agora funciona como esperado em implantações que implementam Fastly. Anteriormente, o aplicativo não atualizava o conteúdo do bloco porque as tags de cache Fastly não eram invalidadas e Fastly continuava a veicular conteúdo desatualizado.

<!-- MC-41142-->

* A visualização de armazenamento temporário da regra de preço do carrinho agora é carregada no site apropriado quando atribuída a várias visualizações de armazenamento de site. Anteriormente, a visualização de preparo da regra de preço do carrinho era carregada na visualização de loja padrão independentemente dos sites atribuídos.

<!-- MC-40916-->

* Preços especiais para produtos na exibição de loja não padrão agora são definidos como o valor padrão, como esperado, após a conclusão de uma atualização programada.

<!-- MC-40850  magento/partners-magento2ee#489-->

* Os comerciantes agora podem salvar com êxito um produto para download com uma amostra vinculada da página de produto para download depois de criar uma atualização de preparo futura com uma data de término. Anteriormente, o aplicativo não salvava o produto e exibia esta mensagem: `The downloadable sample isn't related to the product. Verify the link and try again`. [GitHub-488](https://github.com/magento/partners-magento2ee/issues/488)

<!-- MC-40734-->

* O aplicativo não executa mais consultas de banco de dados grandes que incluem todas as IDs de produto durante a pré-visualização de uma atualização de agendamento de uma categoria. Em vez disso, o filtro de categoria é aplicado à coleção de produtos para fazer uma tabela temporária para pré-visualizações da categoria. Anteriormente, o aplicativo inseria todos os produtos em uma tabela temporária para a pré-visualização de preparo, o que degradava o desempenho.

<!-- MC-40714-->

* O `Magento_SalesRuleStaging` O módulo do não aciona mais erros durante a atualização da 2.3.x.

<!-- MC-40697-->

* O aplicativo agora exibe as condições da regra de catálogo com dados corretos quando você exibe uma atualização de agendamento. Anteriormente, se você visualizava uma atualização de agendamento criada com condições, o aplicativo não exibia as condições ou exibia as condições com dados incorretos.

<!-- MC-40604-->

* O **Usar valor padrão** a caixa de seleção de um produto permanece marcada depois que uma atualização agendada é desmarcada antes da atualização.

<!-- MC-40588 magento/partners-magento2ee#463-->

* As regras de preço do catálogo agora se aplicam conforme esperado para produtos com valores de atributo indefinidos. Anteriormente, os descontos que eram criados por meio de uma regra de preço de catálogo não eram aplicados a produtos com atributos indefinidos. [GitHub-461](https://github.com/magento/partners-magento2ee/issues/461)

<!-- MC-40283-->

* As regras de catálogo agora funcionam conforme esperado em implantações de vários sites com lojas em diferentes fusos horários. Anteriormente, o aplicativo aplicava ou desativava regras de catálogo em todos os sites de uma só vez.

<!-- MC-40054-->

* Os preços de camada não são mais removidos quando uma atualização de programação é criada para um site específico.

<!-- MC-39784 magento/partners-magento2ee#475-->

* Agora é possível salvar um produto na página Produto disponível para download depois de criar uma atualização de preparo futura com uma data final. Anteriormente, o aplicativo não salvava o produto e exibia esta mensagem: `The downloadable link isn't related to the product. Verify the link and try again`. [GitHub-474](https://github.com/magento/partners-magento2ee/issues/474)
...
* O aplicativo agora exibe um **Número do IVA** na página de registro do cliente quando `customer/create_account/vat_frontend_visibility` estiver ativado. [GitHub-31326](https://github.com/magento/magento2/issues/31326)
...
* Adição de um teste para este cenário: Usuários administradores podem editar uma conta de cliente quando o cliente assinar um informativo em fila. [GitHub-30645](https://github.com/magento/magento2/issues/30645)

<!-- ENGCOM-8484-->

* Remoção do `cache:flush` dos testes para melhorar o tempo de execução do `Catalog`, `CatalogUrlRewrite`e `LoginAsCustomer` módulos. [GitHub-31056](https://github.com/magento/magento2/issues/31056)
...
* Os colchetes adicionados às strings não são mais removidos quando a tradução em linha é configurada. Anteriormente, o `escapeHtmlAttr` O método converteu os colchetes em códigos de entidade HTML. [GitHub-32000](https://github.com/magento/magento2/issues/32000)
...
* Sequências de texto no modelo usadas para gerenciar métodos de pagamento armazenados (**Minha conta**  >  **Métodos de pagamento armazenados** ) agora são traduzíveis. [GitHub-31211](https://github.com/magento/magento2/issues/31211)

<!-- MC-30798 ENGCOM-8717-->

* Os emails de pedido enviados do Administrador agora usam a localidade da loja, não a localidade associada à conta do administrador. [GitHub-26521](https://github.com/magento/magento2/issues/26521)
...
* Problemas de alinhamento com as guias na página de Convite do administrador (Administrador) **Marketing** > **Vendas privadas** > **Convites**) foram resolvidas. Anteriormente, a exibição de Guia para convites que tinham sido aceitos por um cliente estava desalinhada. [GitHub-333](https://github.com/magento/partners-magento2ee/issues/333)

<!-- MC-38226 magento/magento2#32008-->

* A página de revisão do pedido exibida durante o check-out com o PayPal Express Checkout agora é carregada com êxito. Anteriormente, o modelo continha o não usado **Atualizar método de delivery** , que só estava parcialmente oculto pelo script durante a renderização da página. [GitHub-32006](https://github.com/magento/magento2/issues/32006)

<!-- MC-37941 magento/magento2#31892-->

* As estrelas de classificação e o texto de revisão na seção Revisões do cliente da página do produto agora são espaçados corretamente quando nomes de classificação longos estão presentes. [GitHub-31890](https://github.com/magento/magento2/issues/31890)
...
* Os redirecionamentos de URL que são criados a partir do Administrador usando um URL personalizado agora funcionam com êxito. Anteriormente, GraphQL armazenava a resposta em cache a partir de um GraphQL `urlResolver` e retornou o valor antigo após a atualização de reescrita do URL.
...
* O aplicativo agora usa o modelo de email personalizado correto ao gerar emails para novos administradores. Anteriormente, o aplicativo usava o modelo padrão antigo, que omitia o nome e sobrenome do administrador.
...
* Os administradores com funções restritas agora podem adicionar, atualizar ou duplicar com êxito as páginas às quais ele tem acesso que são atribuídas a um nó de hierarquia. Anteriormente, o pedido apresentava um erro fatal nestas condições.
...
* O desempenho de carregamentos de página da categoria de administrador com o Visual Merchandiser foi aprimorado. O carregamento de páginas de categoria de Administrador não gera mais consultas de banco de dados grandes.
...
* Os compradores agora são redirecionados para a página que estavam tentando acessar antes de serem solicitados a fazer logon. Anteriormente, em implantações onde as restrições de site eram ativadas e o Modo de restrição era definido como **Vendas privadas: Somente logon**, os compradores foram redirecionados para a página inicial da loja.
...
* O widget de link do nó da hierarquia CMS no TinyMCE agora renderiza os links da hierarquia como esperado quando a variável **Ancorar texto personalizado** campo está vazio.
...
**Problema**: GraphQL `category` e `categoryList` As consultas ignoram as permissões de Categoria que determinam se as categorias em um catálogo compartilhado estão ocultas ou expostas. A loja exibe atualmente todas as categorias que foram atribuídas ou não ao catálogo compartilhado. Isso afeta as implantações do Adobe Commerce 2.4.3 que implementam uma loja de PWA para a qual os catálogos compartilhados B2B foram habilitados. <!-- MC-42528-->
...
Você pode instalar o Adobe Commerce 2.4.3 usando [Composer](../../../installation/composer.md).
