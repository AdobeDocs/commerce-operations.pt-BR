...
Esta versão adiciona suporte do GraphQL aos seguintes recursos:
...
* Um processador de eventos de segmento do cliente foi adicionado para solicitações do GraphQL. Anteriormente, os descontos de regras de preço do carrinho não eram aplicados ao carrinho de compras quando um segmento de cliente era usado em uma condição de regra de preço do carrinho. [GitHub-371](https://github.com/magento/partners-magento2ee/issues/371)
...
**Problema**: A GraphQL `category` e `categoryList` As consultas ignoram as permissões de Categoria que determinam se as categorias em um catálogo compartilhado estão ocultas ou expostas. A loja exibe atualmente todas as categorias que foram atribuídas ou não ao catálogo compartilhado. Isso afeta as implantações do Adobe Commerce 2.4.3 que implementam uma loja de PWA para a qual os catálogos compartilhados B2B foram habilitados. <!-- MC-42528-->
