...
# Instalação no local de início rápido

Nós usamos [Composer](https://getcomposer.org/) para gerenciar componentes Adobe Commerce e Magento Open Source e suas dependências. Usar o Composer para obter o Adobe Commerce e o Magento Open Source [metapackage](https://glossary.magento.com/metapackage) oferece as seguintes vantagens:

- Reutilizar bibliotecas de terceiros sem agrupá-las com o código-fonte
- Reduza conflitos de extensão e problemas de compatibilidade usando uma arquitetura baseada em componentes com gerenciamento de dependência robusto
- Adhere to [Grupo de Interoperabilidade PHP-Framework (FIG)](https://www.php-fig.org/) padrões
- Reempacote o Magento Open Source com outros componentes
- Usar o software Adobe Commerce ou Magento Open Source em um ambiente de produção

>[!NOTE]
>
>Os desenvolvedores que contribuem para o Magento Open Source devem usar a variável [baseado em git](https://developer.adobe.com/commerce/contributor/guides/install/) método de instalação.
...
- Get [chaves de autenticação](prerequisites/authentication-keys.md) para o repositório do Adobe Commerce e do Magento Open Source Composer.

## Fazer logon como proprietário do sistema de arquivos

Saiba mais sobre a propriedade, as permissões e o proprietário do sistema de arquivos em nosso [Visão geral de propriedade e tópico de permissões](prerequisites/file-system/overview.md).

Para alternar para o proprietário do sistema de arquivos:

1. Faça logon no servidor de aplicativos como, ou alterne para, um usuário com permissões para gravar no sistema de arquivos.
...
1. Para executar comandos CLI de qualquer diretório, adicione `<app_root>/bin` ao seu sistema `PATH`.
   ...
   - `cd <app_root>/bin` e execute-os como `./magento <command name>`
   - `app_root>/bin/magento <command name>`
   - `<app_root>` é um subdiretório do docroot do servidor da Web

## Obter a metapackage

Para obter o metapackage do Adobe Commerce ou do Magento Open Source:
...
1. Altere para o diretório raiz do servidor da Web ou um diretório que você configurou como um ponto de raiz do host virtual.
1. Crie um projeto do Composer usando o metapackage do Adobe Commerce ou do Magento Open Source.
   ...
   **Adobe Commerce**
   ...
   Quando solicitado, insira as chaves de autenticação. Chaves públicas e privadas são criadas e configuradas no [Commerce Marketplace](https://marketplace.magento.com/customer/account/login/).

   Se encontrar erros, como `Could not find package...` ou `...no matching package found`, certifique-se de que não haja erros de digitação em seu comando. Se ainda encontrar erros, talvez você não esteja autorizado a baixar o Adobe Commerce. Contato [Suporte Adobe Commerce](https://support.magento.com/hc/en-us) para obter ajuda.
...
Você deve definir permissões de leitura/gravação para o grupo de servidores da Web antes de instalar o Adobe Commerce ou o Magento Open Source. Isso é necessário para que a linha de comando possa gravar arquivos no sistema de arquivos.
...
Este exemplo supõe que o diretório de instalação tenha o nome `magento2ee`, o `db-host` está na mesma máquina (`localhost`) e que a `db-name`, `db-user`e `db-password` são todas `magento`:
...
>[!TIP]
>
>Você pode personalizar o URI do administrador com a variável `--backend-frontname` opção. No entanto, recomendamos omitir essa opção e permitir que o comando de instalação gere automaticamente um URI aleatório. Um URI aleatório é mais difícil para hackers ou software mal-intencionado explorar. O URI é exibido no console quando a instalação é concluída.

>[!TIP]
>
>Para obter uma descrição completa das opções de instalação da CLI, consulte [Instale o aplicativo a partir da linha de comando](advanced.md).
...
A tabela a seguir resume os comandos disponíveis. Os comandos são mostrados somente no formulário de resumo. Para obter mais informações sobre um comando, clique no link na coluna Comando .
...
| `magento maintenance:{enable/disable}` | Ativa ou desativa o modo de manutenção (no modo de manutenção, somente endereços IP isentos podem acessar o Administrador ou a loja). | Aplicativo instalado |
| `magento setup:config:set` | Cria ou atualiza a configuração da implantação. | Nenhum |
| `magento module:{enable/disable}` | Ative ou desative os módulos. | Nenhum |
| `magento setup:store-config:set` | Define opções relacionadas à vitrine, como URL base, idioma, fuso horário. | Configuração de implantação |
| `magento setup:db-schema:upgrade` | Atualiza o schema do banco de dados. | Configuração de implantação |
...
Os argumentos a seguir são comuns a todos os comandos. Esses comandos podem ser executados antes ou depois da instalação do aplicativo:
...
>[!NOTE]
>
>Parabéns! Você concluiu a instalação rápida. Precisa de ajuda mais avançada? Veja nossa [Instalação avançada](advanced.md) guia.
