...
O procedimento usado é diferente, dependendo se você usa [hospedagem compartilhada](#set-permissions-for-one-user-on-shared-hosting) e tem um usuário ou se usa um [servidor privado](#set-ownership-and-permissions-for-two-users) e tem dois usuários.
...
>[!NOTE]
>
>Para restringir ainda mais as permissões após a instalação do aplicativo, você pode [configurar uma máscara](../../next-steps/set-umask.md).
...
>[!NOTE]
>
>Pule para [Localize o grupo de usuários do servidor Web](#find-the-web-server-user-group) se planeja usar uma conta de usuário existente.
...
>[!WARNING]
>
>Se você não tiver privilégios de `root` no servidor de aplicativos, poderá usar outra conta de usuário local. Verifique se o usuário tem uma senha forte e continue com [Coloque o proprietário do sistema de arquivos no grupo de servidores Web](#step-3-put-the-file-system-owner-in-the-web-servers-group).
...
>[!WARNING]
>
>Como o objetivo da criação deste usuário é fornecer segurança adicional, certifique-se de criar uma [senha forte](https://en.wikipedia.org/wiki/Password_strength).
...
>[!NOTE]
>
>As opções `-a -G` são importantes porque adicionam `apache` ou `www-data` como um grupo *secundário* à conta de usuário, o que preserva o grupo *primário* do usuário. Adicionar um grupo secundário a uma conta de usuário ajuda a [restringir a propriedade e as permissões do arquivo](#set-ownership-and-permissions-for-two-users) para garantir que os membros de um grupo compartilhado tenham acesso somente a determinados arquivos.
