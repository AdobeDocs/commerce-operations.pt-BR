...
| `--language` | Código de idioma a ser usado no Administrador e na loja.<br><br>(Se você ainda não tiver feito isso, poderá exibir a lista de códigos de idioma inserindo `magento info:language:list` no diretório `bin`.) | Não |
...
| `--admin-use-security-key` | `1` faz com que o aplicativo use um valor de chave gerado aleatoriamente para acessar páginas no Administrador e em formulários. Esses valores principais ajudam a impedir ataques de falsificação de script entre sites. Este é o padrão.<br/><br/>`0` desativa o uso da chave. | Não |
