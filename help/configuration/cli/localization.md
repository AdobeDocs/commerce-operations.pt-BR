...
As traduções comerciais permitem personalizar e localizar sua loja para várias regiões e mercados gerando:

- **Dicionários de tradução**, que são uma maneira conveniente de personalizar ou traduzir _alguns_ palavras e frases, como as de um módulo ou tema personalizado.
...
Você pode gerar um [dicionário de tradução] para personalizar strings existentes, traduza palavras e frases em um módulo personalizado, localize um tema ou crie pacotes de idioma.
...
1. Você pode criar um pacote dos dicionários de tradução em um pacote de idiomas e fornecê-lo ao administrador da loja de Commerce.
...
>[!INFO]
>
>Para criar um pacote de idiomas a partir de um dicionário de tradução, você _deve_ use o `-m|--magento` opção.
...
- Ao criar dicionários para códigos de idiomas, use as cadeias de caracteres padrão do Commerce.
- Ao traduzir, preste atenção aos espaços reservados: `%1`, `%2`

O Commerce usa os espaços reservados para inserir valores de contexto; eles são _não_ usado para traduções. Por exemplo:
...
A frase resultante deve conter pelo menos um de cada espaço reservado. Por exemplo, suponha que haja marcadores de posição de `%1` para `%3` na frase original. A tradução pode ter muitos desses espaços reservados em qualquer ordem, mas deve haver pelo menos uma ocorrência de `%1`, `%2`, e `%3`. A conversão não pode conter valores de espaços reservados ausentes no valor original (por exemplo, `%4`, `%5`e assim por diante).
...
Ao contrário de um dicionário de tradução, você pode traduzir qualquer uma ou todas as palavras e frases no aplicativo do Commerce usando um pacote de idiomas. Você pode traduzir um componente específico, como um módulo ou um tema, usando um dicionário de tradução. [Saiba mais sobre pacotes de idiomas].
...
Os pacotes de idiomas estão localizados em um diretório em `app/i18n/<VendorName>` no sistema de arquivos do Commerce com os seguintes conteúdos:

- Arquivos de licença necessários
- `composer.json`
- `registration.php` que [registros] o pacote de idioma
...
1. Adicionar [`composer.json`] que especifica as dependências do pacote de idioma.
1. Registrar o pacote de idioma com [`registration.php`]
1. Adicionar `language.xml` arquivo de metainformações conforme discutido na próxima seção.

#### Pacote de idioma language.xml

Ao declarar um pacote de idioma no `language.xml` arquivo de configuração, você deve especificar a sequência da herança de idioma para esse pacote.

A herança de idioma permite criar uma tradução chamada _filho_ com base em uma tradução existente chamada _pai_. As traduções secundárias substituem as principais. No entanto, se a tradução secundária não for carregada ou exibida, ou se não tiver uma frase ou palavra, o Commerce usará o local principal. [Exemplos de herança do pacote de idioma](#example-of-language-inheritance).
...
- `code`—Idioma local do pacote (obrigatório)
- `vendor`—Nome do fornecedor do módulo (obrigatório)
- `package`—Nome do pacote de idioma (obrigatório)
- `sort_order`—Prioridade de upload de um pacote quando há vários pacotes de idioma disponíveis para uma loja
- `use`—Local do pacote de idioma principal do qual herdar dicionários
...
Se um pacote de idioma herdar de dois pacotes, suas `language.xml` pode ser semelhante ao seguinte:
...
Se o aplicativo Commerce não puder encontrar a palavra ou frase no `en_GB` pacote, ele procura em outros pacotes na seguinte sequência:
...
Especificar todas as heranças entre os pacotes de idioma pode resultar na criação de cadeias de herança circulares. Uso [Magento\Test\Integrity\App\Language\CircularDependencyTest] teste para localizar e fixar essas cadeias.
...
Para ativar um pacote adicional para um idioma existente, nomeie o novo pacote com qualquer nome, exceto para um nome de código de idioma existente (para evitar confusão). Especifique as configurações de um pacote no do idioma `language.xml` arquivo de metainformações conforme discutido na próxima seção.
...
1. Colete frases do módulo:
   ...
   >[!INFO]
   >
   >O nome do arquivo CSV deve _corresponder exatamente_ a localidade, incluindo a caixa dos caracteres.

1. Traduzir as palavras e frases usando [estas diretrizes](#translation-guidelines).
1. Se necessário, copie `xx_YY.csv` para `/var/www/html/magento2/app/code/ExampleCorp/SampleModule/i18n` ou para o diretório de temas do módulo (dependendo se o dicionário de tradução é para um módulo ou um tema).

### Exemplo: criar um pacote de idioma

Semelhante ao exemplo anterior, gere um arquivo CSV, mas em vez de especificar um módulo ou diretório de tema, especifique todo o diretório raiz do aplicativo do Commerce. O CSV resultante contém qualquer frase que o comando possa encontrar no código.
